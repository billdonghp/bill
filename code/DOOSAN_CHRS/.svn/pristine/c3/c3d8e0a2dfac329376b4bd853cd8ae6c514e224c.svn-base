...
function ImportExcel() {
    document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&content=viewAffirmBusArrangeInfo&excelFlag=excel";
	document.form1.submit();

}
function search(){
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&content=viewAffirmBusArrangeInfo";
	document.form1.submit();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<table width="100%" height="30" border="0" cellspacing="1" cellpadding="0" >
		...
		<ait:message messageID="display.mutual.search_criteria" module="ess"></ait:message>		
		  ...
		  <ait:message messageID="display.mutual.end_date" module="ess"></ait:message> 
		        ...
		        <ait:codeClass codeClass="BusAreaType" name="busAreaType" selected="${busAreaType}" all="all"/>
		          </td> 
		   ...
		   <ait:message messageID="display.mutual.status" module="ess"></ait:message>            	  
		        ...
		        <tr>
		          ...
		          <td class="info_content_00" nowrap="nowrap">  
		          <ait:codeClass codeClass="BusTypeCode" name="busTypeCode" selected="${busTypeCode}" all="all"/>
		          </td>     
		        
		          <td  class="info_title_01" nowrap="nowrap"><!-- 关键字 -->
		     <ait:message messageID="display.mutual.key_word" module="ess"></ait:message>                
		          </td> 
		          <td class="info_content_00" nowrap="nowrap">  <input type="text" name="key" value="${key}"  title="输入代请人姓名"/></td>       
		         ...
		         <td class="info_content_00" nowrap="nowrap">
				     <select name="arriveType">
				         <option value="2" <c:if test="${arriveType==2}">selected</c:if>>全部</option>
				         <option value="0" <c:if test="${arriveType==0}">selected</c:if>>正常</option>   
				         <option value="1" <c:if test="${arriveType==1}">selected</c:if>>异常</option>
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF">
		    ...
		    <tr>
		    <c:forEach items="${busArrangeListList}" var="list" varStatus="i">
		    <tr align="center">			     
		      <td nowrap="nowrap" align="center" class="info_content_01" >
   			 ...
   			 <c:if test="${list.ARRIVE_TIME_FLAG==1}">
   			   <font color="red">${list.ARRIVE_TIME }&nbsp;异常</font>
   			   
   			  </c:if>
   			  <c:if test="${list.ARRIVE_TIME_FLAG==0}">
   			  <font color="#00CC00">${list.ARRIVE_TIME }&nbsp;正常</font>
   			  </c:if>
			 ...
			 <div id="opition_${i.index}" style="display: none;">
							<c:forEach items="${list.allBusArrangeListList}" var="view">
									  <p>
   			 ...
   			 <td nowrap="nowrap" class="info_content_01">
			     <c:forEach items="${list.allBusArrangeListList}" var="view" varStatus="j">
	...
	<ait:page       
             ...
             parameters="&operation=gaAffirm&content=viewAffirmBusArrangeInfo&menu_code=${param.menu_code}&qryType=${qryType }&endDate=${endDate }&startDate=${startDate }&key=${key }&driverOtType=${driverOtType }&OTTypeCode=${OTTypeCode}"
             total="${busArrangeListInt}"
