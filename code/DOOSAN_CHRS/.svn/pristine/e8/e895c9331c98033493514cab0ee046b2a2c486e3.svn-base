...
function Save()
{
	document.form1.action="/arControlServlet?operation=ar_vacation_update&menu_code=<%=request.getParameter("menu_code")%>";
		...
		<form name="form1" method="post" action="">
		  ...
		  <table width="100%" border="1"cellspacing="0" cellpadding="5" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
		    <tr>
		      ...
		      <ait:codeClass name="vac_tp" codeClass="VacationType" selected="${result.VAC_TP}" remove="VacType20"/></td>
		    </tr>
		    <tr>
		      <td class="info_title_01"><!--开始月-->
					<ait:message  messageID="display.att.setting.dayoff.start_month" module="ar"/></td>
		      <td class="info_content_00"><input name="strt_month" type="text" value="${result.STRT_MONTH}" size="10" maxlength="10" required numeric/></td>
		    </tr>
		    <tr>
		      <td class="info_title_01"><!--结束月-->
					<ait:message  messageID="display.att.setting.dayoff.end_month" module="ar"/></td>
		      <td class="info_content_00"><input name="end_month" type="text" size="10" value="${result.END_MONTH}" maxlength="10" required numeric/></td>
		    </tr>
		    <tr>
		      <td class="info_title_01"><!--休假时间-->
					<ait:message  messageID="display.att.setting.dayoff.time" module="ar"/></td>
		      <td class="info_content_00"><input name="vac_day_cnt" type="text" size="10" value="${result.VAC_DAY_CNT}" maxlength="10" required numeric/></td>
		    </tr>
		  </table>
		  <input type="hidden" name="vac_no" value="${result.VAC_NO}"/>
		</form>
