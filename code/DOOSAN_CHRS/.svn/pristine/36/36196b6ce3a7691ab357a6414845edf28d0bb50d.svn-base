...
import com.ait.ga.business.VisaMangerSerivces;
 ...
 * 
 */
public class VisaMangerCommand implements Command {

	private VisaMangerSerivces smServices = null;
	private Mail mail = null;
	private EssSysparam essSysparam = null;

	public VisaMangerCommand() {
		smServices = new VisaMangerSerivces();
		mail = new Mail();
	}
	...
	public String execute(HttpServletRequest request,
		...
		if (!content.equals("") && content.equals("visaApplyPage")) {
			String documentno = FormUtil.getApplyDocumentid("DOCUMENTNO","GA_VISA_APPLY", "VS");
...
//			List list=ga.getAffirmor4(admin.getAdminID(), "SealApply", sealType, isLend, essSysparam.isContainsOwner());
			List list=ga.getAffirmor1(admin.getAdminID(), "EngravedChapterApply",  essSysparam.isContainsOwner());
			...
			this.visiterApprovalInformation(request, admin);
			returnPage = "/ga_visa_affirmInfo.jsp";
		}else if (!content.equals("")&& content.equals("updateVisaManYiDu")) {		
			returnPage = this.updateVisaManYiDu(request, admin);			
		}else if (!content.equals("")&& content.equals("updateVisaWancheng")) {		
			returnPage = this.updateVisaWancheng(request, admin);			
		}else if (!content.equals("")&& content.equals("updateVisaJuQian")) {		
			returnPage = this.updateVisaJuQian(request, admin);			
		}else if (content.equals("visaManagementView")) {
			this.visaManagementView(request, admin);
			returnPage = "/ga_visa_Managerment.jsp";
		}  else {
			return "error.jsp";
		}
		return returnPage;
...
//				"SealApply", essSysparam.isContainsOwner());
		...
		String documentno1 = FormUtil.getApplyDocumentid("DOCUMENTNO",	"GA_VISA_APPLY", "VS");
		...
		try {
			...
			// --------------------------------------------------------------------------
			String deptId = formParamMap.get("deptId") != null ? formParamMap.get("deptId") : "";
			String useVisatype = formParamMap.get("useVisatype");
			...
			sql = "  SELECT t.*,rownum num from(select SAR.AFFIRMOR_ID, HR_AFFIRMOR.CHINESENAME AFFIRMOR_NAME, " 
				...
				+ "  AND SAR.AFFIRM_TYPE_ID ='SealApply' AND SAR.AFFIRM_OBJECT ='0' ORDER BY AFFIRM_LEVEL) t";
			...
			String affirmorIdea="";
			parameterObject.set("usefDate", formParamMap.get("usefDate"));
			parameterObject.set("usetDate", formParamMap.get("usefDate"));
			parameterObject.set("sealType", formParamMap.get("useVisatype"));
			parameterObject.set("selectDeptid", formParamMap.get("selectDeptid"));
			parameterObject.set("useInformation", formParamMap.get("useInformation"));
			...
			// formParamMap.get("xiezhuPer"));//签证申请新增协助者邮箱
			...
			temp = smServices.addVisaApply(parameterObject, affirorList);
		...
		if (temp) {
			...
			if (essSysparam.isGaSendMail()) {
				try {
					System.out.println(parameterObject.getString("usefDate"));
					this.sendCarApplyMail("签证申请 等待决裁", AffirmorIdFirst, admin
							.getDepartment(), admin.getCpnyId(), admin
							.getChineseName(), parameterObject
							.getString("usefDate").toString(), parameterObject
		...
		try {
			...
			for (int i = 0; i < applyno.length; i++) {
				...
				if (!"".equals(applyno[i])) { // 申请编号不能为空
					...
					String sealTypeCode = request.getParameter("sealTypeCode_"		+ applyno[i]);
					...
					if (request.getParameter("flag").equals("2")) {
						...
						this.sendCarApplyMail("签证申请 已经否决", applerId,
							request.getParameter("adminDept_"+ applyno[i]), admin.getCpnyId(),
							request.getParameter("CHINESENAME_"	+ applyno[i]), 
							request.getParameter("usefDate_"+ applyno[i]), 
							...
							request.getParameter("affirmorIdea_"+ applyno[i]));
						...
						String MAX_AFFIRM_LEVEL = request.getParameter("AFFIRM_LEVEL_" + applyno[i]);
						...
						if (MAX_AFFIRM_FLAG.equals("0")) {
							...
							String deptName = request.getParameter("adminDept_"		+ applyno[i]);

							this.sendCarApplyMail("签证申请 等待决裁", applerId1,
									request.getParameter("adminDept_"	+ applyno[i]), admin.getCpnyId(),
									request.getParameter("CHINESENAME_"	+ applyno[i]), 
							...
							//推送
							DooPushAPI.insertNotice(DooPushAPI.APPLY_TYPE_VISA, applyno[i],Integer.parseInt(StringUtil.checkNull(parameterObject.get("affrimlevel"))) + 1);
							...
							if (emailNameDicc != null&& !emailNameDicc.isEmpty()&& admin.getCpnyId().equals("78000000")) {// 签证申请
								...
								for (int j = 0; i < emailName1.length; i++) {
									if (emailName1[j] != null	&& !emailName1[j].isEmpty()) {
										this.sendCarApplyMail(	"签证申请 已通过决裁",	emailName1[j],
														request.getParameter("adminDept_"+ applyno[i]),
														admin.getCpnyId(),
														request.getParameter("CHINESENAME_"	+ applyno[i]),
														request.getParameter("useDate_"+ applyno[i]),
...
//		 mail.sendMail(inputData);
		if (result != null && !result.equals("") && essSysparam.isGaSendMail()) {
			...
			if("签证申请 等待决裁".equals(title)){
				mail.gaSendMail(inputData, cpny_id, "qianchao.chen@doosan.com", title);
				mail.gaSendMail(inputData, cpny_id, "haoping.yu@doosan.com", title);
			}
	...
	public String visiterApprovalInformation(HttpServletRequest request,	AdminBean admin) {
		SimpleMap parameterObject = null;
		try {
			...
			String qryType2 = request.getParameter("qryType2") != null ? request.getParameter("qryType2") : "";
			String qryType3 = request.getParameter("qryType3") != null ? request.getParameter("qryType3") : "";
			String deptid = request.getParameter("deptid") != null ? request.getParameter("deptid") : "";
			String key = request.getParameter("key") != null ? request.getParameter("key") : "";
			...
			int allsealAffrimInfoInt = smServices.allsealAffrimInfoInt(parameterObject);
			List allsealAffrimInfoList = smServices.allsealAffrimInfoList(	parameterObject, currentPage, pageSize);
			for (int i = 0; i < allsealAffrimInfoList.size(); i++) {
				...
				List allSealApproval = smServices.allSealApproval(parameterObject);
				boolean isfalg = true;
		...
		return "/ga_visa_affirmInfo.jsp";

	}
	// 参观者申请更新 满意度
	public String updateVisaManYiDu(HttpServletRequest request,	AdminBean admin) {
		EssSysparam essSysparam = (EssSysparam) SysparamUtils.getSysparam(
				EssSysparam.class, admin.getCpnyId());
		...
		try {
			for (int i = 0; i < countsArr.length; i++) {
				...
				String sql1 = " update ga_visa_apply t set t.manyzh=? ,t.manyfw=? , t.manynq=? where t.DOCUMENTNO=? ";
				Logger.getLogger(getClass()).debug(sql1);
				ps = conn.prepareStatement(sql1);

				String aa = request.getParameter("MANYZH_" + countsArr[i]);
				String dd = request.getParameter("MANYFW_" + countsArr[i]);
				String cc = request.getParameter("MANYNQ_" + countsArr[i]);
				String bb = request.getParameter("APPLYNO_" + countsArr[i]);
				ps.setString(1, aa);
				ps.setString(2, dd);
				ps.setString(3, cc);
				ps.setString(4, bb);// 申请号
	...
	// 参观者申请更新 完成
	public String updateVisaWancheng(HttpServletRequest request,	AdminBean admin) {
		EssSysparam essSysparam = (EssSysparam) SysparamUtils.getSysparam(
				EssSysparam.class, admin.getCpnyId());
		...
		try {
			for (int i = 0; i < countsArr.length; i++) {
				...
				parameterObject = ObjectBindUtil.getData(request);
				conn.setAutoCommit(false);
				String sql1 = " update ga_visa_apply t set t.ISCOMPLISE='1' ,RETURNDATE =sysdate where t.DOCUMENTNO=? ";
				Logger.getLogger(getClass()).debug(sql1);
				ps = conn.prepareStatement(sql1);
		...
		this.visaManagementView(request, admin);
		return "/ga_visa_Managerment.jsp";

	}
	// 参观者申请更新 完成
	public String updateVisaJuQian(HttpServletRequest request,	AdminBean admin) {
		EssSysparam essSysparam = (EssSysparam) SysparamUtils.getSysparam(
				EssSysparam.class, admin.getCpnyId());
		...
		try {
			for (int i = 0; i < countsArr.length; i++) {
				...
				parameterObject = ObjectBindUtil.getData(request);
				conn.setAutoCommit(false);
				String sql1 = " update ga_visa_apply t set t.ISCOMPLISE='2' ,RETURNDATE =sysdate where t.DOCUMENTNO=? ";
				Logger.getLogger(getClass()).debug(sql1);
				ps = conn.prepareStatement(sql1);
	...
	private String visaManagementView(HttpServletRequest request,		AdminBean admin) {
		SimpleMap parameterObject = null;
		try {
			String adminId = admin.getAdminID();
			parameterObject = ObjectBindUtil.getData(request);
			...
			int allsealAffrimInfoInt = smServices.allsealAffrimInfoIntall(parameterObject);
			List allsealAffrimInfoList = smServices.allsealAffrimInfoListall(	parameterObject, currentPage, pageSize);
			for (int i = 0; i < allsealAffrimInfoList.size(); i++) {
				SimpleMap map = null;
				map = (SimpleMap) allsealAffrimInfoList.get(i);
				String applyId = map.getString("DOCUMENTNO");
	...
	public String ExcelSealInfo(HttpServletRequest request,		AdminBean admin) {
		...
		try {
			...
			List applyList = smServices.getExcelSealInfoList(parameterObject);
			for (int i = 0; i < applyList.size(); i++) {
				...
				List affirmorList = smServices.getSealAffirmorList(parameterObject1);
				boolean isfalg = true;
				for (int j = 0; j < affirmorList.size(); j++) {
		...
		return "/ga_Visa_Excel.jsp";

	}
	public String ExcelSealInfoALL(HttpServletRequest request,		AdminBean admin) {
		this.putToolbarInfo(request);
		SimpleMap parameterObject = null;
		parameterObject = ObjectBindUtil.getData(request);
		String adminId = ((AdminBean) request.getSession(false).getAttribute(	"admin")).getAdminID();
		...
		try {
			...
			List applyList = smServices.getExcelSealInfoListALL(parameterObject);
			for (int i = 0; i < applyList.size(); i++) {
				SimpleMap parameterObject1 = new SimpleMap();
				dataMap = (SimpleMap) applyList.get(i);
				String applyPersonId = dataMap.getString("PERSON_ID");
