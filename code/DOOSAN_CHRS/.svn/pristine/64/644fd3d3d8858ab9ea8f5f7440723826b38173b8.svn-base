...
public class InsertOTPlanCmd extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			//parameterObject.set("planMonth", parameterObject.getString("year") + parameterObject.getString("month"));
			...
			int result = NumberUtil.parseInt(services.validateOTPlan(parameterObject));
			if (result != 0) {
//				此人的当前的加班计划已存在
				messageSource = new MessageSource("ar",admin.getLocale(), "UTF-8");
				request.setAttribute("alert", messageSource.getMessage("alert.att.otPlan_exist"));
				request.setAttribute("reload","history.back();");
				return "/inc/alertMessage.jsp";
			}
					
			// insert OT Plan
			services.insertOTPlan(parameterObject);

		} catch (Exception e) {

			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("insert OT Plan Exception. ", e);
		}
//		添加成功
