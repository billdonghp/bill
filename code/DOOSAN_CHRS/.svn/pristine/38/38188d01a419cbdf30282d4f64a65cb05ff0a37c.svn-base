...
function search(){
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=visaManger&content=visaAffirm";
	document.form1.submit();
}

 function sealApprovalOKorNO(applyno,flag){
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=visaManger&content=sealApprovalOKorNO&flag="+flag+"&applyno="+applyno;
		...
		if(document.getElementById("READ_FLAG_"+applyno).value=="0" 
			   		...
			   		alert("请查看编号"+applyno+"上传文件后再进行签证决裁");
			   		return false;
		}
	document.form1.submit();
	}
...
function doAffirm(){
	...
	for (var i=0; i<affirmno.length; i++){
		if(affirmno[i].checked == true){
		   ...
		   if(document.getElementById("READ_FLAG_"+affirmno[i].value).value=="0" 
		   		...
		   		alert("请查看编号"+affirmno[i].value+"上传文件后再进行签证决裁");
		   		return false;
		   }
    ...
    if (confirm(msg5)) {
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=visaManger&flag=1&content=sealApprovalOKorNO";
	document.form1.submit();
	}
}
...
function doReject(){
	...
	if (confirm(msg6)) {
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=visaManger&flag=2&content=sealApprovalOKorNO";
	document.form1.submit();
	}
}
function viewFile(fileAddress,documentno){
document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=visaManger&content=ongingRead&documentno="+documentno;
document.form1.submit();
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF">
		 ...
		 <c:forEach items="${applyList}" var="varTest" varStatus="i">
		    ...
		    <td nowrap="nowrap" align="center" class="info_content_01" >
		      ...
		      <td nowrap="nowrap"  align="center" class="info_content_01">${varTest.USEFDATE}
		      <input type="hidden" value="${varTest.USEDFATE}" name="usefDate_${varTest.DOCUMENTNO}">
		      </td>
		      <td nowrap="nowrap"  align="center" class="info_content_01">${varTest.USETDATE}
		      <input type="hidden" value="${varTest.USEDTATE}" name="usetDate_${varTest.DOCUMENTNO}">
		      </td>
		      <td nowrap="nowrap" align="center" class="info_content_01" >${varTest.DEPTNAME}</td>
		      <td nowrap="nowrap" align="center" class="info_content_01" >${varTest.SEALTYPE}</td>
		 ...
		 <tr align="center"> 
		  ...
		  <!-- Page Navigation Start-->
					<ait:page       
		               ...
		               parameters="&operation=visaManger&content=visaAffirm&menu_code=${param.menu_code}&qryType=${qryType }&deptid=${deptid }&endDate=${endDate }&startDate=${startDate }&key=${key }" 
		               total="${resultCount}"
