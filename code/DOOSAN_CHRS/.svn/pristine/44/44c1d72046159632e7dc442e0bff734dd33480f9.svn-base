...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form action="hrmControlServlet" method="post" name="searchForm" onkeydown="if(event.keyCode==13) searchForm.submit();">
		  ...
		  <input type="hidden" name="operation" value="searchPlurality">
...
<!--						<td width="7%" class="info_title_01" nowrap>发令类型</td>-->
<!--						<td width="15%" class="info_content_00"><ait:codeClass style="content" codeClass="TransPlurality" name="TRANSCODE" all="all" filling="align='absmiddle'"/></td>-->
						<td width="7%" class="info_title_01" nowrap>
							<!--是否生效--><ait:message  messageID="display.mutual.activity"/>
							</td>
		...
		<!-- display content -->
		<br>
		<table width="100%" border="0" cellpadding="0" cellspacing="1"  style='word-break:keep-all'   border='0'>
			<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d" style="word-break:break-all">
			<form name="save" action="hrmControlServlet" method="post">
			  <input type="hidden" name="operation" value="delPlurality"/>
			  <input type="hidden" name="menu_code" value="${toolbarInfo.menu_code}" />
		  ...
		  <tr>
		    ...
		    <td class="info_title_01" style='word-wrap:break-word;word-break:break-all' nowrap="nowrap"><!-- 兼职内容 -->
					<ait:message  messageID="display.emp.staff_info.contents_of_concurrent_job" module="hrm"/></td>
		    <td class="info_title_01"  nowrap><!-- 兼职解除日期 -->
					<ait:message  messageID="display.emp.offer_regulation.concurrent_job.termination" module="hrm"/></td>
		    <!-- 
		  ...
		  <c:forEach items="${list}" var="oneResult" varStatus="i">
		      <tr>
		        <td class="info_content_01" >
		        	...
		        	<input type="hidden" name="pluralityNo_${i.index}" value="${oneResult.pluralityNo}"/>
		        ...
		        <td class="info_content_01" >${oneResult.contents}</td>  
		         <!-- 
		        <td class="info_content_01" nowrap>
		        	<img src="../images/a_${oneResult.activity}.gif" width="22" height="25">
		        </td>
		        -->
		        <td class="info_content_01" nowrap>${oneResult.updateDate}</td>  
		        <td class="info_content_01" nowrap="nowrap">
		    	<a href="/hrmControlServlet?operation=exportInfo&type=plurality&empid=${oneResult.empID}" target="_blank"><!-- 查看内容 -->
		    	   excel
		    	</a>                 
		     </td>        
		       </tr>
		...
		<ait:page       
		         ...
		         parameters="&operation=searchPlurality&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}&TRANSCODE=${TRANSCODE}&TRANSNO=${TRANSNO}&ACTIVITY=${ACTIVITY}" 
		         total="${resultCount}"
