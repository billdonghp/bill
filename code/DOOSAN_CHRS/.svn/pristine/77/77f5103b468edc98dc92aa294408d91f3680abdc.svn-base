...
public class GmVegetableOpinionViewCommand extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List gmVegetableOpinionViewList = services.gmVegetableOpinionViewList(parameterObject, currentPage, pageSize);
			
			Object gmVegetableOpinionViewCount = services.gmVegetableOpinionViewCount(parameterObject);
			
			request.setAttribute("resultList", gmVegetableOpinionViewList);
			request.setAttribute("recordCount", gmVegetableOpinionViewCount) ;
			...
			if(!"excel".equals(flag)){
			     url = "/gm_vegetable_opinion_view.jsp?menu_code=" + parameterObject.getString("menu_code");
			}else if("excel".equals(flag)){
			
				 url = "/gm_vegetable_opinion_view_excel.jsp?menu_code=" + parameterObject.getString("menu_code");	
			}
		} catch (Exception e) {

			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("retrieve Vegetable Opinion View List  by paging Exception. ", e);
		}
