...
public class UpdateDeptPostGradeCmd implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (int i = 0 ; i < selectC.length; ++i )
			{
				...
				parameterObject.setString("POST_GRADE_YEAR", request.getParameter(selectC[i] + "_POST_GRADE_YEAR")) ;
				
				updateList.add(parameterObject) ;
			}
			
						
			paService.updateDeptPostGrade(updateList);
		} catch (Exception e) {

			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("update dept hr_post_grade  Exception. ", e);
		}
		//			修改成功
		messageSource = new MessageSource(admin.getLocale(), "UTF-8");
		request.setAttribute("alert", messageSource.getMessage("alert.mutual.modify_successfully"));
		request.setAttribute("url", "paControlServlet?operation=viewdeptpostgradecmd&menu_code=" + request.getParameter("menu_code") + "&DEPTID=" + request.getParameter("DEPTID") 
							);
