...
function defaultLogin(){
 if($('empDiffSize').value==1 && $('grantFlag').value==0){
...
<input type="hidden" id="empDiffSize" name="empDiffSize" value="${empDiffSize}"/>
<input type="hidden" id="grantFlag" name="grantFlag" value="${grantFlag}"/>
<div class="login_div">
		...
		<!-- <form action="" method="get"> -->
			<div class="login_input_div">
				<div class="login_input_div_input">
					<c:if test="${grantFlag == 0}">
						<ait:select name="cpnyId" dataListName="empDiff" style="login_select" selected="${cpnyId}"/>
					</c:if>
					<c:if test="${grantFlag != 0}">
						<tr>
							<td>
							    <select name="loginId" id="loginId" onChange="getEmpDivision();">
							    	<option value="${a}">${chineseName}</option>
						    ...
						    <td>
							    <span id="empDiff">
								<ait:select name="cpnyId" dataListName="empDiff" selected="${cpnyId}"/>
								</span>
						    </td>
						</tr>
					</c:if>
