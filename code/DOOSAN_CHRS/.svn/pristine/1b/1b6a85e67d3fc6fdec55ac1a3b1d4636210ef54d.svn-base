...
<%@page import="com.ait.ga.bean.GaAffirmList"%>
<html>
<head>
...
function Search(){
	...
	document.form1.action="/gaControlServlet?operation=retrieveFestivalPresentAffirm&menu_code=${param.menu_code}";
...
function affirm(affirmFlag, index, jndex){
	...
	$('affirmNo').value = $('affirmNo_'+jndex).value;
	...
	if (confirm("确定对当前记录进行决裁？")) {
		document.form1.action="/gaControlServlet?operation=affirmFestivalPresent&menu_code=${param.menu_code}";
		document.form1.submit();
	}
}
...
function doApproval(){
    ...
    if (confirm(msg5)) {
    	$('AFFIRM_FLAG').value = 1;
    	$('op').value = "batch";
		document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=affirmFestivalPresent";
		document.form1.submit();
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
			...
			<tr>
				...
				<table width="100%" height="30" border="0" cellpadding="0"
					...
					<tr>
						...
						<td width="10%" class="info_content_00"><!-- 请输入姓名查找 --> 
						...
						<td width="18%" class="info_content_00">
							<ait:selDept name="DeptId" selected="${DeptId}" all="all"/>
						</td>
						<td width="8%" class="info_title_01">状态</td>
						<td width="8%" class="info_content_00"><ait:codeClass
							name="affirmType" codeClass="AffirmType" selected="${affirmType}" all="all"/></td>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
			<tr>
				<td align="left" class="title1" colspan="10">节日礼品决裁</td>
			</tr>
		</table>
		<table width="100%" border="1" cellspacing="0" cellpadding="2"
			...
			<c:forEach items="${recordList}" var="oneResult" varStatus="i">
				<tr align="center">
							...
							<c:if test="${oneResult.SEQ_NO eq affirm.APPLY_NO}">
								<tr>
									...
									<input type="hidden" name="affirmNo_${j.index}" value="${affirm.AFFIRM_NO}"/> 
										<c:if test="${affirm.AFFIRM_FLAG==0}">
											<input type="hidden" name = "affirm_flag${i.index}" value="false">
											<font color="#990099">未裁决</font></c:if>
							          	<c:if test="${affirm.AFFIRM_FLAG==1}">
							          		<input type="hidden" name = "affirm_flag${i.index}" value="true">
							          		<font color="#00CC00">已通过</font></c:if>
							          	<c:if test="${affirm.AFFIRM_FLAG==2}">
							          		<input type="hidden" name = "affirm_flag${i.index}" value="true">
							          		<font color="#00CC00">已否决</font></c:if>
							          	<c:if test="${affirm.AFFIRM_FLAG == 0}">
							          		<c:if test="${affirm.AFFIRMOR_ID eq currnetId}">
					...
					<td class="info_content_01">
						<c:if test="${oneResult.AFFIRM_FLAG == 0}">
		...
		<!-- Page Navigation Start--> <ait:page link="/gaControlServlet"
			parameters="&operation=retrieveFestivalPresentAffirm&menu_code=${param.menu_code}&personId=${personId}&empId=${empId}&affirmType=${affirmType}&deptId=${deptId}"
