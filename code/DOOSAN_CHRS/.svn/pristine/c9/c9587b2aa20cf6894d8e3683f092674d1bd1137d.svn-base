...
<title>评价 > 基本设置 > 职业资格等级修改决裁</title>
...
function doAffirm(flag){
  ...
  if (!existSelected){
	...
	///	alert("3");
		...
		document.ApplyForm.action="/evsControlServlet?operation=evszyzgdjupdateaffirm&menu_code=<%=request.getParameter("menu_code")%>";
		   ...
		   <ait:message messageID="display.mutual.status" module="ess"></ait:message>            	  
				  ...
				  <td width="10%" class="info_content_00">
				     <select name="qryType" onchange="Search();">
				         ...
				         <option value="0" <%if(qryType.equals("0")){out.print(" selected ");}%>><!-- 未决裁 -->
				     <ait:message messageID="display.ess.approval.pending" module="ess"></ait:message>  
				     </option>        
				     <option value="1" <%if(qryType.equals("1")){out.print(" selected ");}%>>已决裁         </option>    
				     <option value="2" <%if(qryType.equals("2")){out.print(" selected ");}%>>已否决         </option>            
				     </select>
				  </td>                    
		        </tr>  
			</table>
		...
		<table width="100%" border="1" align="center" cellpadding="0"
			...
			<ait:message	messageID="display.mutual.status_2" module="ess"></ait:message>   
	            ...
	            <%
	                	...
	                	parameterObject.setString("codeFlag", "CT");
	                	parameterObject.setString("sDate", sDate);
	        			parameterObject.setString("eDate", eDate);
					  List levStypecodeid=evsGxjndj.getEvsDJCodenameconfirm(parameterObject);
					 
					  if(levStypecodeid!=null){ 
					     for(int i=0;i<levStypecodeid.size();i++){   
					    	 evsGxjndj=(EvsGxjndj)levStypecodeid.get(i);
					    	 essAffirmorList = evsGxjndj.getAffirmorList();
					    	 
				    	    int applyNo = evsGxjndj.getPkNo1();
						    int affirmNo = 0;
						    String person_id="";
						    int opFlag = evsGxjndj.getOpFlag();
				 ...
				 %>
						<tr
							onClick="HighLightTR('#F0F1F4','black','<%=evsGxjndj.getCodeNo()%>','<%=menu_code%>')">
							 <td class="info_content_09" ><%if (evsGxjndj.getStatus().equals("2") ) {%>
							 	<input type="checkbox" name="affirmNo" value="<%=affirmNo%>" <%=checkState%>>
							  	 <input type="hidden" name="applyNo<%=affirmNo%>" value="<%=applyNo%>">
							  	  <input type="hidden" name="codeflag<%=affirmNo%>" value="<%=evsGxjndj.getCodeflag()%>">
							...
							<td><div align="center" height="30" ><%=evsGxjndj.getEvscodeid()%>&nbsp;
							   <input name="evscodeid" type="hidden" value="<%=evsGxjndj.getCodeNo()%>"/></div>
							</td>
							<td>
					      		  <table>
							      <tr>  
							      <td align="center">
							       <span style="color: green">  <%=evsGxjndj.getDjjffrom()%>(现) </span>
							      </td>
							      </tr>	
							       <tr>  
							        <td align="center">  
							        <span style="color: blue">    <%=evsGxjndj.getDjjffrom1()%>(原) </span>
							      </td>
							      </tr>	  
							      </table>	  	
					        </td>
							<td>
							      ...
							      <tr>  
							      <td align="center">
							       <span style="color: green">  <%=evsGxjndj.getDjjfto()%>(现) </span>
							      </td>
							      </tr>	
							       <tr>  
							        <td align="center">  
							        <span style="color: blue">    <%=evsGxjndj.getDjjfto1()%>(原) </span>
							      </td>
							      </tr>	  
							      </table>	  	
					        </td>
							<td><div align="center" height="30" ><%=evsGxjndj.getDengji()%>&nbsp;</div>
							</td>
							<td><div align="center" height="30" ><%=evsGxjndj.getUpdatetime()%>&nbsp;</div>
							</td>
							<td><div align="center" height="30" ><%=evsGxjndj.getUempid()%>&nbsp;</div>							
							</td>
							<td><div align="center" height="30" ><%=evsGxjndj.getCodeflag()%>&nbsp;</div>							
							</td>
