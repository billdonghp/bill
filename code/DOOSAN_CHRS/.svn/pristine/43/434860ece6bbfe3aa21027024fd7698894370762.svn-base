...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form action="hrmControlServlet" method="post" name="searchForm" onkeydown="if(event.keyCode==13) searchForm.submit();">
		  ...
		  <input type="hidden" name="operation" value="searchSuspend">
					...
					<tr>
						...
						<td width="10%" class="info_title_01" nowrap><!-- 发令类型 -->
			          		...
			          		<td width="10%" class="info_content_00"><ait:codeClass  
								...
								onChange="searchForm.submit();" filling="align='absmiddle'"  all="all"/>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d" style="word-break:break-all">
			<form name="save" action="hrmControlServlet" method="post">
			  <input type="hidden" name="operation" value="delSuspend"/>
			  <input type="hidden" name="menu_code" value="${toolbarInfo.menu_code}" />
		  ...
		  <tr>
		    ...
		    <td class="info_title_01" nowrap="nowrap">
				<!-- 事由 --><ait:message  messageID="display.emp.staff_info.unpaid_leave.reason_of_unpaid_leave" module="hrm"/>
			</td>
		     <td class="info_title_01"><!-- 复职日期 -->
		     <ait:message  messageID="display.emp.offer_regulation.unpaid_leave.returned" module="hrm"/>
		     </td>
		  ...
		  <c:forEach items="${list}" var="oneResult" varStatus="i">
		      <tr>     
		        <td class="info_content_01" >
		        	<c:if test="${oneResult.activity eq 0 && oneResult.flag eq 1}" >  
			          
			        	<input type="checkbox" name="selectedTag" value="${i.index}" class="check"/>
			        	
		        	</c:if>
		        	<input type="hidden" name="suspendNo_${i.index}" value="${oneResult.suspendNo}"/>
		        ...
		        <td class="info_content_01"   >${oneResult.suspendReason}</td>
		       
		        <td class="info_content_01" nowrap> ${oneResult.updateDate}</td> 
		        <td class="info_content_01" nowrap="nowrap">
		    	<a href="/hrmControlServlet?operation=exportInfo&type=suspend&empid=${oneResult.empID}" target="_blank"><!-- 查看内容 -->
		    	   excel
		    	</a>                 
		     </td>          
		...
		<ait:page       
		         ...
		         parameters="&operation=searchSupspend&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}&TRANSCODE=${TRANSCODE}&TRANSNO=${TRANSNO}&ACTIVITY=${ACTIVITY}" 
		         total="${resultCount}"
