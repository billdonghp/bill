...
<title>LOTTE&gt;提示信息&gt;预转正信息浏览</title>
</head>
<SCRIPT type="text/javascript">
function Search()
{
 document.searchForm.action="/hrmControlServlet?operation=viewexpiredprobationcmd&menu_code=hr0407";
  document.searchForm.submit();
}

function ImportExcel()
{
   document.searchForm.action="/hrmControlServlet?operation=tipmessagereportcmd&gowhere=exporedprobat";
  document.searchForm.submit();
}
</SCRIPT>
<body>
	...
	<tr>
		...
		<table width="100%" height="30" border="0" cellspacing="0"
			...
			<jodd:form fromRequest="true">
				<input type="hidden" name="operation" value="viewexpiredprobationcmd">    
				 <input type="hidden" name="menu_code" value="hr0407">                   
				<tr>
				...
				<tr>
					<td>
						...
						<!-- 部门 --><ait:message  messageID="display.mutual.dept"/></td>
							...
							<td width="15%" class="info_title_01" nowrap><!-- 预转正日期 -->
					<ait:message  messageID="display.emp.offer_regulation.recruit.internal_info.off_probation" module="hrm"/></td>
			          		<td width="25%" class="info_content_00">
			          			<input type="text" name="sDate" class="content" style="width:80px " onClick="setday(this);">
			            	~
			          			<input type="text" name="eDate" class="content" style="width:80px " onClick="setday(this);">
			          		</td>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
			<tr>
 				...
 				<!-- 预转正信息 --><ait:message  messageID="display.emp.statistics.off_probation_info" module="hrm"/>
 				</td>
			</tr>
		</table>
		<table width="100%" border="0" cellpadding="0" cellspacing="1"
			...
			<tr>
				...
				<td class="info_title_01" nowrap><!-- 预转正日期 -->
					<ait:message  messageID="display.emp.offer_regulation.recruit.internal_info.off_probation" module="hrm"/></td>
			          		</td>
			</tr>
			<c:forEach items="${listdata}" var="VCtr" varStatus="status">
				<tr>
					<td height="30" class="info_content_01" nowrap>
						<ait:content enContent='${VCtr.PINYIN}' zhContent='${VCtr.CHINESENAME}' koContent='${VCtr.KOREANNAME}'/>
						</td>
					...
					<td class="info_content_01" nowrap><c:out
						value="${VCtr.END_PROBATION_DATE}" /></td>
				</tr>
			</c:forEach>  
		...
		<ait:page       
         ...
         parameters="&operation=viewexpiredprobationcmd&DEPTID=${deptid}&sDate=${sDate}&eDate=${eDate}&EMPIDORCHINESENAME=${key}&menu_code=${toolbarInfo.menu_code}" 
         total="${resultCount}"
