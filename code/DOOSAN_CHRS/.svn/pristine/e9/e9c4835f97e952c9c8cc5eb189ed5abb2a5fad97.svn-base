...
public class PaBonusParamItemCmd extends PaAbstractCommand {
	...
	public String list(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<SimpleMap> paParamItimList  = this.services.retrievePaBonusParamList(parameterObject, currentPage, pageSize) ;
			
			resultCount = this.services.retrievePaBonusParamListCnt(parameterObject) ;
		...
		{
			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("retrieve pa bonus param item list Exception. ", e);
		}
		return "/pa_bonus_param_item.jsp?menu_code=" + parameterObject.getString("menu_code") ;
	}
}