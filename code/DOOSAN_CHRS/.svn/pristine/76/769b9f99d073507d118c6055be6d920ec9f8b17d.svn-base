...
public class PaBonusHistoryUpdateCmd extends ArAbstractCommand {
	
	private static final Logger logger = Logger.getLogger(PaBonusHistoryUpdateCmd.class);
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SimpleMap updateMap = new SimpleMap() ;
		...
		try {
			...
			updateSql.append("PA_MONTH = '").append(parameterObject.getString("paMonth"))
				     .append("' WHERE BONUS_TYPE_CODE = '").append(parameterObject.getString("bonusTypeCode")).append("'")
				     .append(" AND BONUS_NO = ").append(parameterObject.getString("bonusNo"))
					 .append(" AND PERSON_ID = '").append(parameterObject.getString("personId")).append("'")
				     .append(" AND PA_MONTH = '").append(parameterObject.getString("paMonth")).append("' ");
			
			paServices.updatePaBonusHistory(updateSql.toString()) ;
			
			if (parameterObject.getString("paCalcFlag") != null && parameterObject.getString("paCalcFlag").length() > 0){
				paServices.callPaBonusHistoryProc(parameterObject) ;
			}
			...
			throw new GlRuntimeException("pa_bonus_history update Exception. ", e);
		}
		...
		request.setAttribute("url","/pa/" + parameterObject.getString("url") + "?&key=" + parameterObject.getString("empId") + "&statTypeCode=" + parameterObject.getString("statTypeCode") 
								 + "&paMonth=" + parameterObject.getString("paMonth") + "&bonusTypeCode=" + parameterObject.getString("bonusTypeCode") + "&bonusNo=" + parameterObject.getString("bonusNo")
		);
