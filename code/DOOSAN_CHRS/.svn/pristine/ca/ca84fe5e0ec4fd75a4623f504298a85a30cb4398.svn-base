...
<jsp:useBean id="sr" class="com.ait.hrm.entity.SexReport"></jsp:useBean>

    <%
        String joinTypeCode=request.getParameter("joinTypeCode");  
		response.setHeader("Content-Type", "application/vnd.ms-excel; charset=UTF-8");
		response.setHeader("Content-Disposition", "attachment; filename=Gender.xls");  
     ...
     <table	border="1" cellpadding="0" cellspacing="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
        ...
        <!--组织人员结构-----员工性别分布-->
        	<ait:message  messageID="report.hrm.sex.distribute.title.name" module="report"/>
                </td>      </tr>
      </table>
     <table	border="1" cellpadding="0" cellspacing="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
     ...
     <%------------------------------ 表头 ----- -----------------------------%>
     	<tr>
        	...
        	<ait:message  messageID="report.hrm.organize.structure.division" module="report"/></td>  
     		<td align="center"    rowspan="2"  colspan="3"><!--性别分布-->
        	<ait:message  messageID="report.hrm.sex.distribute.name" module="report"/></td>
     		<td align="center"    rowspan="2" colspan="2"><!--%性别占有-->
        	<ait:message  messageID="report.hrm.sex.distribute.scale" module="report"/></td>  
   		</tr>    
   		  <tr>
   		<tr>
	   		<td align="center" ><!--男-->
        	...
        	<ait:message  messageID="report.hrm.sex.distribute.small" module="report"/></td>
	   		<td align="center" ><!--男性比例-->
        	<ait:message  messageID="report.hrm.sex.distribute.male.scale" module="report"/></td>
	        <td align="center" ><!--女性比例-->
        	<ait:message  messageID="report.hrm.sex.distribute.female.scale" module="report"/></td>           
   		</tr>
   <%---------------------------------------------------------------------- --%>
         <tr>
           ...
           <%
   				sr = rd.getSexInfo("Z000000_",joinTypeCode);
   			 %>
           <td align="center" ><!--                部小计 -->
        	<ait:message  messageID="report.hrm.organize.structure.office" module="report"/></td>
           <td align="center"><%=sr.getMan() %></td>
   			<td align="center"><%=sr.getWoman() %></td>
        	...
        	for (int i = 0 ; i < oneList.size() ; i ++)        
        	{
        		...
        		for (int j = 0 ; j < twoList.size() ; j ++)
        		{
					...
					for (int k = 0 ; k < threeList.size() ; k ++)
					{
						...
						sr = rd.getSexInfo(threeDept.getDeptID(),joinTypeCode);
						if( j == 0 && k == 0)
						{
			%>
		<tr>
		...
		<tr>	
			<%			
					...
					sr = rd.getSexInfo(twoDept.getDeptID(),joinTypeCode);
			%>
   			<td align="center"><!--                科小计 -->
			...
			<%
        		...
        		sr = rd.getSexInfo(oneDept.getDeptID(),joinTypeCode);            
        	%>	
        <tr>
        ...
        %>
   		...
   		<tr>
        	...
        	<ait:message  messageID="report.hrm.organize.structure.sum" module="report"/></td>
   			<%
   				sr = rd.getSexInfo("Z000000",joinTypeCode);            
   			 %>
   			<td align="center"><%=sr.getMan() %></td>
   			<td align="center"><%=sr.getWoman() %></td>
   			<td align="center"><%=sr.getCount() %></td>
