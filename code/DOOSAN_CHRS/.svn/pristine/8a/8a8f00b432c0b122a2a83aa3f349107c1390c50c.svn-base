...
<SCRIPT type="text/javascript">
 function oingConfirm(applyno){
  document.form1.action="/gmControlServlet?menu_code=${param.menu_code}&operation=expressManger&content=oingConfirm&flag=1&applynoAr="+applyno;
  document.form1.submit();
 }
 function checkAll()
{
    var selected = document.form1.ck_all.value == "0" ? true : false;
    var applyno = document.getElementsByName("applynoAr") ;
...
function Confirm(){
    ...
    if (confirm(msg5)) {
		document.form1.action="/gmControlServlet?menu_code=${param.menu_code}&operation=expressManger&content=oingConfirm&flag=1";
		document.form1.submit();
	}
...
function Search() {
	//重新搜索时应重置当前页数
	document.form1.action = "/gmControlServlet?menu_code=${param.menu_code}&operation=expressManger&content=expressConfirm" ;
	document.form1.submit();
}
	...
	<tr>
	    ...
	    <tr>
		  ...
		  <ait:message messageID="display.mutual.end_date" module="ess"></ait:message> 
		   ...
		   <ait:message messageID="display.mutual.status" module="ess"></ait:message>            	  
	        ...
	        <tr>
	          <td class="info_title_01" nowrap="nowrap"><!-- 快递单号 -->快递单号</td>
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF">
		 ...
		 <c:forEach items="${expressConfirmInfoList}" var="varTest" varStatus="i">
		    <tr align="center">
		      ...
		      <td align="center" nowrap="nowrap" class="info_content_01" > &nbsp;
		      <c:if test="${varTest.ISCONFIRM == '0'}">
		      	 <input type="checkbox" name="applynoAr" value="${varTest.APPLY_NO}" />
		      	 </c:if>
			  </td>
		      ...
		      <!--<td nowrap="nowrap"  align="center" class="info_content_01">${varTest.EMPID2}&nbsp;</td>-->
		      ...
		      <td nowrap="nowrap" align="center" class="info_content_01">
		      	<c:if test="${varTest.ISCONFIRM == '0'}">
		      	<span style="color:red; cursor:pointer;" onClick="oingConfirm(${varTest.APPLY_NO })">确认</span>
		      	</c:if>
		      	<c:if test="${varTest.ISCONFIRM == '1'}"><font color="#00CC00"> 已确认</font>
		      	</c:if>
		      </td>
		      <td nowrap="nowrap"  align="center" class="info_content_06">
		      <textarea name="affirmorIdea_${varTest.APPLY_NO}" style=" height: 40px;width:100px " type="_moz" onfocus="this.style.height='50px'" onblur="this.style.height='40px';">${varTest.CONFIRMIDEA}</textarea></td>	
			 </tr>	
		</c:forEach>
		<input type="hidden" name="currentPage" value="${currentPage}">		
					...
					<ait:page       
		               ...
		               parameters="&operation=expressManger&content=expressConfirm&menu_code=${param.menu_code}&deptID=${deptID}&postNumber=${postNumber}&sendPerson=${sendPerson}&startDate=${startDate}&endDate=${endDate}&qryType=${qryType}&key=${key}" 
		               total="${resultCount}"
