...
tableUtil.appendRow = function(){
	...
	td.innerHTML = "<input type='text' name='medicalDate"+j+"' value=''>";
}
...
tableUtil1.appendRow1 = function(){
	...
	k = Number(document.form1.temp3.value);
	y = k -1 ;
	...
	td = nTr.insertCell() ;
	td.className = "info_content_00" ;
	td.innerHTML = "<input type='text' id='empId"+y+"' name='empId"+y+"' value='' onKeyUp='SearchContent(this.value,this.id,"+y+")'> <input type='hidden' id='personId"+y+"' name='personId"+y+"' value=''>";
...
function Save()
{
	...
	document.form1.action="/puControlServlet?operation=Roomsetup&content=saveUpdateRoomSetup&menu_code=${param.menu_code}&i="+i+"&k="+k+"&id="+<%=request.getParameter("id")%>;
...
function updateValue(cell,no) {
	$('empId'+no).value=cell.childNodes[2].firstChild.nodeValue;
...
<form name="form1" method="post" action="">
		...
		<input type="hidden" name="temp1" value="${equiplistcount}">
		<input type="hidden" name="temp2" value="">
		<input type="hidden" name="temp3" value="${confirmorcount}">
		...
		<table id="operateTable" width="100%" border="1" cellspacing="0" cellpadding="5"
			...
			<tr>
				<td width="20%" class="info_title_01">
	      		  ...
	      		  <td class="info_content_00"><!-- 请输入姓名查找 -->
		          	 	<input type="text" id="roomname" name="roomname"  value="${roomname}" required/>
		         </td>
			</tr>
			<tr>
				<td width="20%" class="info_title_01">可容纳人数</td>
				<td class="info_content_00">
					<input type="text" name="peoples" value="${peoples}" onkeyup="if(isNaN(value)||paseValueToAmount(value))execCommand('undo')" required>
				</td>
			</tr>
			<!-- <tr>
				<td width="20%" class="info_title_01">管理担当</td>
				<td class="info_content_00">
				<input type="text" id="empId" name="empId" size="15" value="${confirmorname}" onkeyup="SearchContent(this.value,this.id)" />
				<input type="hidden" id="personId" name="personId" size="8" value="${confirmorid}" /></td>
			</tr> -->
			<tr>
				<td width="20%" class="info_title_01">
			...
			<c:forEach items="${equiplist}" var="equiplist" varStatus="i">
				<tr>
					<td width="20%" class="info_title_01">设备<input type='radio' name='rowNum'/>
					</td>
					<td class="info_content_00">
						<input type="text" id="medicalDate${i.index}" name="medicalDate${i.index}" value="${equiplist}">&nbsp;
					</td>
				</tr>
			</c:forEach>
			</table>
			...
			<c:forEach items="${confirmorlist}" var="confirmorlist" varStatus="i">
				<tr>
					<td width="20%" class="info_title_01">管理担当<input type='radio' name='rowNum1'/>
					</td>
					<td class="info_content_00">
						
						<input type="text" id="empId${i.index}" name="empId${i.index}"  value="${confirmorlist}" onkeyup="SearchContent(this.value,this.id,0)" />
				        <input type="hidden" id="personId${i.index}" name="personId${i.index}" size="8" value="${confirmorlist}" /></td>
					</td>
				</tr>
