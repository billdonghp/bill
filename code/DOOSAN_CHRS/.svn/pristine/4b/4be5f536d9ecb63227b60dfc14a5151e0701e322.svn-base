...
public class PaControlServlet extends ControlServlet {

	private static final long serialVersionUID = -130239423410918445L;

	private CommandManager manager;

	private static final Logger logger = Logger.getLogger(PaControlServlet.class);

	public void init(ServletConfig config) throws ServletException {
		super.init(config);

		// load command manager
		manager = CommandManager.getInstance(config.getServletContext().getRealPath("/"), "navigation.pa.path");
