...
//  主调用函数是 setDayMuli(this,[object])和setDayMuli(this)，[object]是控件输出的控件名，举两个例子：
//  一、<input name=txt><input type=button value=setDayMuli onclick="setDayMuli(this,document.all.txt)">
//  二、<input onfocus="setDayMuli(this)">
...
document.writeln('<iframe id=meizzDateMuliLayer Author=wayx frameborder=0 style="position: absolute; width: 144; height: 204; z-index: 9998; display: none"></iframe>');
strFrame='<style>';
...
strFrame+='	{var DateLayer=parent.document.all.meizzDateMuliLayer.style;';
...
window.frames.meizzDateMuliLayer.document.writeln(strFrame);
window.frames.meizzDateMuliLayer.document.close();		//解决ie进度条不结束的问题

//==================================================== WEB 页面显示部分 ======================================================
var outObject;
var outButton;		//点击的按钮
var outDate="";		//存放对象的日期
var odatelayer=window.frames.meizzDateMuliLayer.document.all;		//存放日历对象
var index ;
var togetherName ;
function setDayMuli(tt,cnt,name) //主调函数
{
	if (arguments.length >  3){alert("对不起！传入本控件的参数太多！");return;}
	if (arguments.length == 0){alert("对不起！您没有传回本控件任何参数！");return;}
	this.togetherName = name ;
	this.index = cnt ;
	var dads  = document.all.meizzDateMuliLayer.style;
	var th = tt;
...
function closeLayer()               //这个层的关闭
  {
    document.all.meizzDateMuliLayer.style.display="none";
  }
...
function meizzToday()  //Today Button
  {
    ...
    if(outObject){
		...
		if(document.ApplyForm("ck_" + index).checked)
		{
			for(var z = 1 ; z <= size ; z++ )
			{
				if(document.ApplyForm.c.checked == true && document.ApplyForm("ck_" + z).checked)
				{
					document.ApplyForm(togetherName + z).value = outObject.value ;
				}
			}
		}
...
//复选多选
function checkBoxMuli(name,cnt){

	if(document.ApplyForm.c.checked==true){
		var size = document.ApplyForm.applySize.value ;	
		var selected = document.ApplyForm(name + cnt).checked ;
		...
		if(document.ApplyForm("ck_" + cnt).checked)
	    {
		    for(var z = 1 ; z <= size ; z++ )
			{
				if(document.ApplyForm.c.checked && document.ApplyForm("ck_" + z).checked)
				{
					...
					document.ApplyForm(name + z).checked = selected ;
		    	}
			}
	    }
	}
...
//输入多选
function textMuli(name,cnt){

	if(document.ApplyForm.c.checked==true){
		var size = document.ApplyForm.applySize.value ;	
		var value = document.ApplyForm(name + cnt).value ;
		...
		if(document.ApplyForm("ck_" + cnt).checked)
	    {
		    for(var z = 1 ; z <= size ; z++ )
			{
				if(document.ApplyForm.c.checked && document.ApplyForm("ck_" + z).checked)
				{
					...
					document.ApplyForm(name + z).value = value ;
		    	}
			}
	    }
	}
