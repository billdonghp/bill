...
function Confirm(matchno, flag, batchTag) {
	var msg = "通过";
	if (flag == "2") {
		msg = "否决";
	}
...
<div align="left">
<span class="title1">人事确认&gt;值班确认</span></div>
<form name="ApplyForm" action="/essControlServlet">
	<input type="hidden" name="operation" value="view" />
	<input type="hidden" name="content" value="matchconfirm" />
...
<%for(int i=0;i<matchList.size();i++){
  ...
  <tr align="center">
    <td><%if (essMatchBean.getOpFlag()>=0) {%>
    	<input type="checkbox" name="ck_selectedTag" value="<%=essMatchBean.getMatchNo()%>"/>
    	<input type="hidden" name="hidden_otno" value="<%=essMatchBean.getMatchNo() %>" ><%} else {%>&nbsp;<%}%>
    </td>
    ...
    <td align="center" nowrap>
		<font color="<%=(String) colorMap.get(String.valueOf(essMatchBean.getActivity()))%>">
    		...
    		<%if (essMatchBean.getOpFlag() == 0) {%>
    			<br><span style="color:red; cursor:pointer;" onClick="Confirm(<%=essMatchBean.getMatchNo() %>, 1, false);">通过</span>&nbsp;|&nbsp;<span style="color:red; cursor:pointer;" onClick="Confirm(<%=essMatchBean.getMatchNo()%>, 2, false);">否决</span>
    		<%} else if (essMatchBean.getOpFlag() == 1) {%>
    			<br><span style="color:red; cursor:pointer;" onClick="Confirm(<%=essMatchBean.getMatchNo() %>, 1, false);">通过</span>
    		<%} else if (essMatchBean.getOpFlag() == 2) {%>
    			<br><span style="color:red; cursor:pointer;" onClick="Confirm(<%=essMatchBean.getMatchNo() %>, 2, false);">否决</span>
			<%}%>
		</font>
	</td>
	<td align="left" style="width: 80px "><textarea name="text_remark" rows="2"><%=StringUtil.checkNull(essMatchBean.getRemark(), "")%></textarea></td>
  </tr>
<%}%>
</table>
