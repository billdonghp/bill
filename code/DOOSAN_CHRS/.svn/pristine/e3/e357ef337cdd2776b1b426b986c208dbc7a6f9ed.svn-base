package com.ait.evs.evs0126.servlet;
...
import com.ait.sy.sy0103.bean.Biz;
import com.ait.sy.sy0103.bean.Ent;
import com.ait.util.StringUtil;
import com.ait.utils.Func;

public class Esy0103Control extends HttpServlet {
	/**
	 * 
	 */
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (flag != null) {
			...
			if (flag.equals("insert")) { // 执行insert
				try {
					...
					if (parentCode.equals("0")) {
						...
						response
								.sendRedirect("/sy/sy_code_detail.jsp?menu_code="
			...
			if (flag.equals("updateSubCode")){
				...
				response.sendRedirect("/sy/sy_code_detail.jsp?menu_code="
						+ menu_code + "&code_id=" + parentCode);
			}
			...
			if (flag.equals("delete")) { // 执行delete
				try {
					...
					CodeUtil.getInstance().refreshCodes();
					if (!code_id.equals("")) {
						response.sendRedirect("/sy/sy_code_detail.jsp?code_id="
								+ code_id);
