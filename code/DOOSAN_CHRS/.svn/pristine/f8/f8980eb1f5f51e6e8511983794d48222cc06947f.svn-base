...
<script language="javascript">
 var noteflag=0;
function Save(){
   if(document.form1.NOTE0.value==""||document.form1.NOTE0.value==null){
     alert("请输入要修改的项目!");
     return;
    } 
   if(noteflag==1){	
	  alert("备注在250个汉字以内！");
		...
		document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=voitureManger&content=updateMaintenanceCosts";
...
function CheckNumberlength(notevalue){
 var text=notevalue.replace(/[^\x00-\xff]/g,"**");
 if(text.length>500){
 noteflag=1;
 alert("备注在250个汉字以内！");
 return;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr> 
		...
		<%} %>
		  ...
		  <tr>
		   ...
		   <tr>
			 ...
			 <td class="info_content_01" colspan="3"><%=voitureBean.getAS_Duration()!=null?voitureBean.getAS_Duration():"&nbsp;" %>&nbsp;</td>
		  </tr>	   
		  </table>	
		  <table width="100%" border="0" cellpadding="0" cellspacing="1" >
				<tr>
     ...
     <table width="100%"  border="1"cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" id = 'operateTable'>
		    ...
		    <input name="temp1" type="hidden" value="<%=arrylist.size() %>">
		    <%for(int i=0; i<arrylist.size();i++){ 
		    vb=(VoitureBean)arrylist.get(i);%>			 
		    <tr onClick="band('#E7F0EF','black',<%=i%>)" height="30">
		    <input type="hidden" value="<%=vb.getEXPENSENO()%>" name="expenseno<%=i%>" >		     
		      <td nowrap="nowrap" class="info_content_01"><input type="text" name="TIMING_DATE<%=i%>" value="<%=vb.getTIMING_DATE()!=null?vb.getTIMING_DATE():""%>" class="content" style="width:80px "  readonly onClick="setday(this);" required></td>     
		      <td nowrap="nowrap" class="info_content_01"><select name="CAUSES<%=i%>"><%for(int s=0;s<voitureResumelist.size();s++) {voitureResume=(VoitureBean)voitureResumelist.get(s);%><option value="<%=voitureResume.getCode_id() %>" <%=(voitureResume.getCode_id()).equals((vb.getCAUSES()!=null?vb.getCAUSES():""))?"selected=\"selected\"":""%> ><%=voitureResume.getCode_name()%></option><%} %></select></td>
		      <td nowrap="nowrap" class="info_content_01">
		      <select name="UNIT<%=i%>">
		      	<option value="0" <%if(vb.getUnit().equals("0")){ %> selected="selected" <%} %>>元</option>
		      	<option value="1" <%if(vb.getUnit().equals("1")){ %> selected="selected" <%} %>>公里</option>
		      	<option value="2" <%if(vb.getUnit().equals("2")){ %> selected="selected" <%} %>>升</option>
		      </select>
		      </td>
		      <td nowrap="nowrap" class="info_content_01"><input type="text" name="AMOUNT<%=i%>" value="<%=vb.getAMOUNT()!=null?vb.getAMOUNT():""%>" onkeyup="return CheckNumber(this.value)" required></td>
		      <td nowrap="nowrap" class="info_content_01"><textarea name="NOTE<%=i%>" style='height: 25px;width:250px' onblur="CheckNumberlength(this.value)"><%=vb.getNOTE()!=null?vb.getNOTE():""%></textarea></td>
		      <td nowrap="nowrap" class="info_content_01"><font color="red"><%=vb.getCREATE_BY()%></font><input type="hidden" value="<%=vb.getCREATE_BY()%>" name="name10" id="name10"> </td>	
		      <%if(i==0){ %>	
		      <td nowrap="nowrap" class="info_content_01" >
