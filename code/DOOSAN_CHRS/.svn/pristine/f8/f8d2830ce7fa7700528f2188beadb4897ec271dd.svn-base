...
function checkDate(){
	var1 = document.addworkForm.OTdate.value;
	var2 = "<%=yearCurrentDate%>";
	var otDate = new Date(var1.substring(0,4), var1.substring(5,7), var1.substring(8,10));
	var curDate = new Date(var2.substring(0,4), var2.substring(5,7), var2.substring(8,10));
	if (otDate.getTime() < curDate.getTime()) {
		return true;
	}
	else {
		alert("不能申请今天或之后的加班");
		document.addworkForm.OTdate.focus();
		return false;
	}
}
 //-->
...
<div align="left">
<span class="title1">申请信息＞紧急加班申请</span></div>
<%if (result!=null) {
	if (result.equals("0"))
		result = "申请失败";
...
<!--    <tr align="center">
     ...
     <td height="30"  align="left">
		 <select name="otTypeCode"  style="width:85px ">
		   <%
						try{
							Vector vector = SysCodeParser.getCode("TOTTypeCode");
...
-->
    <%
	   ...
	   affirm_list =(ArrayList)affirmdao.getAffirmorList(empidaffirmed,"TOTApply");
