...
<%	
	...
	try{
		if (flag.equals("pa_modify")){
			...
			paParam.modifyPaParam(param_data_no,param_no,return_value,personId,chineseName,pinyinName,korName);
			//"修改已完成"
			%>
			<script lang="javascript">
			alert('<%=edit%>');
			//location.href="pa_personal_v.jsp?menu_code=<%=menu_code%>&pamonth=<%=pamonth%>&empID=<%=personId%>&deptid=<%=deptid%>";
			//location.href="/hrmControlServlet?operation=destinationCmd&menu_code=<%=menu_code%>&destination=/pa_personal&pamonth=<%=pamonth%>&empID=<%=personId%>&deptid=<%=deptid%>";
			location.href="pa0201.jsp?menu_code=<%=menu_code%>";
			</script>
			<%
		}
		else if (flag.equals("pa_bonus_modify")){
			String [] param_data_no = request.getParameterValues("param_data_no");
			String [] param_no = request.getParameterValues("param_no");
			String [] return_value = request.getParameterValues("return_value");
			String personId = request.getParameter("personId");
			...
			String korName = request.getParameter("korName");
			PaParam paParam = new PaParam();
			paParam.modifyPaBonusParam(param_data_no,param_no,return_value,personId,chineseName,pinyinName,korName);
			//"修改已完成"
			%>
			<script lang="javascript">
			alert('<%=edit%>');
			location.href="/paControlServlet?operation=pa_bonus_param&menu_code=<%=menu_code%>";
			</script>
			<%
		}
