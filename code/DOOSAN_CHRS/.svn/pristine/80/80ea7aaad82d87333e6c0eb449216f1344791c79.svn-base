 ...
 * 
 */
public class OpsitionIdCounterpartsDiseaseType implements Command {
	...
	public String execute(HttpServletRequest request,
		...
		PositionInfo positionInfo  = (PositionInfo)jobHealthServices.getDiseaseType(parameterObject);
		
    	response.setContentType("text/xml;charset=UTF-8");
    	response.setHeader("Cache-Control", "no-cache");
		PrintWriter out = response.getWriter();
		StringBuffer str = new StringBuffer(5000) ;
		str.append(positionInfo.getDISEASETYPE());

		str.append("<input type='hidden' name='JOB_DISEASE_TYPE_CODE' value='"+positionInfo.getJOB_DISEASE_TYPE_CODE()+"'>");
