...
<html>
<head>
<!-- pa0105.jsp 工资系统 > 工资查看 > 个人别工资支给明细 -->
...
function ImportExcel()
{
  if($('key').value==""||$('key').value==null){
   ...
   return;
	...
	document.form1.action = "/paControlServlet?operation=paDetailView&content=ImportExcelPerson_F&menu_code=${param.menu_code}";
...
function PrintReport(){
	var url = "" ;
	if (document.all("qufen").value == "C_12067_1330306" || document.all("qufen").value == "C_12067_1330310")
	{
	...
	var url="http://10.40.128.28:8099/<%= admin.getCpnyId() %>_" + url + "?empid="+document.all("key").value+"&startmonth="+(document.all("startyear").value+document.all("startmonth").value)+"&endmonth="+(document.all("endyear").value+document.all("endmonth").value)
       +"&avgthistotalsupport="+document.all("AVG_THIS_TOTAL_SUPPORT").value+"&avgthisactualwage="+document.all("AVG_THIS_ACTUAL_WAGE").value+"&patype="+document.form1.patype.value;
	...
	<tr>
	    ...
	    <tr>
	      ...
	      <tr>	        
	        ...
	        <td align="center" class="info_content_01" nowrap="nowrap" >
	        	区分:&nbsp;
	        	<ait:empDiff name="qufen"  cpnyDiff="<%=admin.getCpnyId()%>" selected="${param.qufen}" />
		...
		<tr>
		            ...
		            <td class="info_title_00" nowrap >岗保扣除</td>
		            ...
		            <td class="info_title_00" nowrap >餐税</td>
		          ...
		          <tr>
		            ...
		            <td class="info_content_00" nowrap >${test.A_POST_RESERVATION_MINUS}</td>
		            ...
		            <td class="info_content_00" nowrap >${test.PERSONAL_HOUSING_FUND}</td>
		             <td class="info_content_00" nowrap >${test.MEAL_DUTY }</td>
