/*
 * @(#)InsertTrainingCommandCmd.java 1.0 2006-12-19 下午05:17:21
 *
 ...
 * @Date 2006-12-19 下午05:17:21
 * @version 1.0
 * 
 */
public class InsertTrainingCommandCmd extends HrmAbstractCommand {
	...
	public String execute(HttpServletRequest request,
		...
		try {
		    ...
		    List<Studying> lStudying = new ArrayList<Studying>();
		    if (null != check) {
			for (int i = 0, j = check.length; i < j; i++) {
			    Studying bean = new Studying();
			    ObjectBindUtil.setFormBean(request, bean, "_" + check[i]);
			    lStudying.add(bean);
			}
		    }
		    HrmServices services = HrmServices.getInstance();
		    services.insertStudying(lStudying);
		...
		//跳转的新页
		request.setAttribute("url", "/hrmControlServlet?operation=viewTrainingCommand&menu_code="+request.getParameter("menu_code"));
