...
<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
  ...
  <tr align="center">
    ...
    <td  height="30"  align="center"><%=(String)record.get("QUAL_NAME_CODE")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)record.get("QUALIFICATION_NAME")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)record.get("QUAL_LOCATION")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)record.get("GRADE_OBTAINED")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)record.get("QUAL_INSTITUTE")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)record.get("END_QUAL_DATE")%>&nbsp;</td>
	        <%
	...
	<%if(act==0){%><a href="/ess/ess0600_t.jsp?menu_code=<%=menu_code%>&NO=<%=(String)record.get("QUAL_NO")%>&operate=qualification">
	删除</a><%}%>&nbsp;
	</td> -->
  ...
  <td width="11%">
  <a href="/EssContorlServlet?command=ArbitramentCircsCommandImp&content=ArbitramentCircsCertificateContentImp&menu_code=ess0603&pageno=<%=(p.getPagenum()/10)-10+1%>">
  ...
  <%for(int i=1;i<=p.getPagenum();i++){%>
	<td>
	<a href="/EssContorlServlet?command=ArbitramentCircsCommandImp&content=ArbitramentCircsCertificateContentImp&menu_code=ess0603&pageno=<%=i%>">
...
<table border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <td width="11%">
  <a href="/EssContorlServlet?command=ArbitramentCircsCommandImp&content=ArbitramentCircsCertificateContentImp&menu_code=ess0603&pageno=<%=(p.getPagenum()/10)*10-10+1%>"><img src="/images/btn/p_last101.gif" width="18" height="12" border="0" align="absmiddle"></a>
  </td>
  <%}%>
  <td>
 <%for(int i=(((p.getCurrent()-1)/10*10)+1);i<=(((p.getCurrent()-1)/10)+1)*10 && i<=p.getPagenum();i++){%>
	<a href="/EssContorlServlet?command=ArbitramentCircsCommandImp&content=ArbitramentCircsCertificateContentImp&menu_code=ess0603&pageno=<%=i%>"><font color="#666666" ><b><%=i%></b></font></a>&nbsp;
 <%}%>
 </td>
  <%if(p.getPagenum()/10!=0 && (((p.getCurrent()-1)/10*10)+1)+10<p.getPagenum()){%>
  <td width="11%">
  <a href="/EssContorlServlet?command=ArbitramentCircsCommandImp&content=ArbitramentCircsCertificateContentImp&menu_code=ess0603&pageno=<%=(p.getPagenum()/10)*10+1%>"><img src="/images/btn/p_next101.gif" width="18" height="12" border="0" align="absmiddle"></a>
  </td>
  <%}%>
  </tr>
</table>
