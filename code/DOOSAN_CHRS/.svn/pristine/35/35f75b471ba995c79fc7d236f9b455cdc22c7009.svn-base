...
public class Update extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request,
		...
		try {
			int result = dao.validateUpdateDate(arDate) ;
			if (result != 0) {
				//"修改的员工区分已存在"
				messageSource = new MessageSource("ar",admin.getLocale(), "UTF-8");
				request.setAttribute("alert", "修改的员工区分已存在");
				request.setAttribute("reload","history.back();");
				return "/inc/alertMessage.jsp";
			}	
			
			dao.editArDate(arDate);
		} catch (Exception e) {
			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("update ar_statistic record  Exception. ", e);
		}
