 ...
 * 
 */
public class ImportParamDataReportCmd extends PaAbstractCommand {
...
//			 附加表字段
			...
			String sqlContent = " SELECT get_emp_id(#FIELD1_VALUE#) FIELD1_VALUE ,#RETURN_VALUE#,T.CHINESENAME,"
							  + "#START_MONTH#,#SD_ED_VALUE#,#END_MONTH#,KPA_PARAM_DATA_SEQ.NEXTVAL," + param_no 
							  + " FROM HR_EMPLOYEE T WHERE T.EMPID = #FIELD1_VALUE# " 
/*							  + " AND EXISTS "
							  + "  ( SELECT C.STAT_TYPE_CODE " 
							  + " FROM KPA_PARAM_ITEM C " 
							  + " WHERE C.PARAM_NO = " + param_no + " ) "*/
							  + " AND NOT EXISTS "
							  + "  ( SELECT D.PARAM_DATA_NO FROM KPA_PARAM_DATA_I D "
			...
			// 添加参数
			parameters.set("tableName", "KPA_PARAM_DATA");
