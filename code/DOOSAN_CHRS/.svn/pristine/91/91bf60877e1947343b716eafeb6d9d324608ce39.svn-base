...
public class ExportSmockProvideReportCmd extends GaAbstractCommand{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List recordList = services.selectSmockProvideList(parameterObject);
			...
			columns.setString("部门", "FOURTHIDEPT");
			...
			columns.setString("预发放日期", "CREATE_DATE");
			...
			columnType.setInt("MODELNUMBER", ReportConstant.CELL_TYPE_TEXT);
			

			// 设置报表参数
			ExcelParameterBean paramBean = new ExcelParameterBean();
			paramBean.setFileName("SMOCKPROVIDE_DATA.xls");
			paramBean.setSheetname("SMOCKPROVIDE_DATA");
...
//			paramBean.setHeadContent("资产记录");
			// 设置内嵌表头
			// LSFC个人年假记录表
			 paramBean.setInLineHeadContent("工作服发放记录信息表");
			...
			throw new GlRuntimeException(
					"export smock provide report Exception. ", e);
		}
