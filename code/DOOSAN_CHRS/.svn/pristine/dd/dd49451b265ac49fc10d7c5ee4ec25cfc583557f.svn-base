...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form action="hrmControlServlet" method="post" name="searchForm" onkeydown="if(event.keyCode==13) searchForm.submit();">
		  ...
		  <input type="hidden" name="operation" value="searchResign">
		  	...
		  	<tr>
				...
				<table width="100%" height="30" border="0" cellpadding="0"
					...
					<tr>
						...
						<td width="15%" class="info_content_00"><ait:codeClass style="content" codeClass="TransResign" name="TRANSCODE" all="all" filling="align='absmiddle'"/></td>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d" style="word-break:break-all">
			<form name="save" action="hrmControlServlet" method="post">
			  <input type="hidden" name="operation" value="delResign"/>
			  <input type="hidden" name="menu_code" value="${toolbarInfo.menu_code}" />
		  ...
		  <tr>
		    ...
		    <td class="info_title_01" height="30" nowrap >
						<!-- 人事令号 --><ait:message  messageID="display.mutual.regulation_no"/></td>
<!--		 //    取消人事令类型、离职原因、转职公司，原人事令种类的内容改为离职类型 -->
		    ...
		    <td class="info_title_01" nowrap><!-- 离职日期 --><ait:message  messageID="display.mutual.leaving_date"/></td>
<!--		    <td class="info_title_01">离职类型</td>-->
		    ...
		    <td class="info_title_01" nowrap><!-- 离职事由 --><ait:message  messageID="display.mutual.dimission_reason"/></td>
		  ...
		  <c:forEach items="${list}" var="oneResult" varStatus="i">
		      <tr>
		        <td class="info_content_01" width="5%">
		        	...
		        	<input type="hidden" name="resignNo_${i.index}" value="${oneResult.resignNo}"/>
		        	<input type="hidden" name="activity_${i.index}" value="${oneResult.activity}"/>
		        </td>
		        <td class="info_content_01" width="10%">${oneResult.transNo}</td>
		        ...
		        <td class="info_content_01" width="10%" nowrap>${oneResult.empID}</td>              
		        <td class="info_content_01" width="10%" nowrap>
		        ...
		        <td class="info_content_01" width="10%" nowrap>${oneResult.resignDate}</td>  
<!--		        <td class="info_content_01" nowrap>${oneResult.transCodeName}</td>-->
		        ...
		        <td class="info_content_01" nowrap>${oneResult.newCompany}</td>
		         -->
		        <td class="info_content_01" >${oneResult.resignDesc}</td>
		         <td class="info_content_01" nowrap="nowrap">
		    	<a href="/hrmControlServlet?operation=exportInfo&type=resign&empid=${oneResult.empID}" target="_blank"><!-- 查看内容 -->
		    	  excel
		    	</a>                 
		     </td>    
		...
		<ait:page       
		         ...
		         parameters="&operation=searchResign&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}&TRANSCODE=${TRANSCODE}&TRANSNO=${TRANSNO}&ACTIVITY=${ACTIVITY}" 
		         total="${resultCount}"
