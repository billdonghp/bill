...
public class EvszyzgdjdeletevaluateCmd  implements Command{

				private static final Logger logger = Logger
				.getLogger(EvszyzgdjdeletevaluateCmd.class);
			...
			public String execute(HttpServletRequest request,
				...
				String codeno = request.getParameter("ID");
			...
			try { 
				...
				//services.deleteZyzgdj(evscodeid);  
			 
				 EvsGxjndj evsCraft = new EvsGxjndj();
					evsCraft.setCodeNo(Integer.parseInt(codeno));
					
					
					evsCraft.getEvsGxjndjByID();
             ...
             "VALUES(Evs_Code_Affirm_Seq.NEXTVAL,?,'0',?,?,SYSDATE,?,'1','delete')";
				...
				for(int j=0;j<affirmList.size();j++){
					...
					insertAffirmPsb.setInt(1, Integer.parseInt(codeno));
					insertAffirmPsb.executeUpdate();
					
					insertAffirmPs = conn.prepareStatement(sqlInsertAffirm);
					insertAffirmPs.setInt(1, Integer.parseInt(codeno));
...
//				if("78000000".equals(admin.getCpnyId()) && affirmEmail !=""){	元
			   if(affirmEmail !=""){	
						...
						content.append("<br>").append("等级:").append(evsCraft.getDengji());
				    	content.append("<br>").append("技能等级删除:").append("");
						content.append("<br>");
						content.append("----------------------------");
			   ...
			   //evsCraft.addEvsGxjndj();
				...
				if(affirmEmail !="")
				{
					...
					content.append("<br>");
					content.append("系统决裁页面位置:技能评价 > 领导决裁 > 技能等级删除决裁");
					content.append("<br>");
					content.append("--------------------------------------------------------------------------");
					content.append("<br>");
			...
			if(menu_code.equals("evs0134"))
			return "/evs0134.jsp";
			else if(menu_code.equals("evs0135"))
				return "/evs0135.jsp";
			else
				return "/evs0134.jsp";
		}
