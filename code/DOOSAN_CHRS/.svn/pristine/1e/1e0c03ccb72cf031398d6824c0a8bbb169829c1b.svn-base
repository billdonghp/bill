...
public class ResetPasswordCmd extends AbstractCmd implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = apiService.resetPassword();
		if (result == 1) {
			response.getWriter().append(R.toJson(R.ok()));
		}else{
			response.getWriter().append(R.toJson(R.error("重置失败")));
		}
		return null;
	}
}
