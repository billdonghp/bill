	...
	if(checks.length>0){
		...
		for(var i=0;i<checks.length;i++){
			...
			if(checks[i].checked==true){
			    ...
			    check = "checked";
			    //发令日期验证
	    		var date=document.getElementById("rewardDate_"+checks[i].value);
	   		
	    		if(date){
	    			if(date.value==""){
		     		//	alert("奖励日期不能为空！");
		     			alert('<ait:message  messageID="alert.emp.offer_regulation.award.award_date_cannot_be_empty" module="hrm"/>');
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table width="100%" height="30" border="0" cellspacing="1" cellpadding="0" >
		  ...
		  <input type="hidden" name="operation" value="viewHortation">
		...
		<table width="100%"  border="0" cellpadding="0" cellspacing="0">
		    <form name="save" action="hrmControlServlet" method="post">
			  <input type="hidden" name="operation" value="insertHortation"/>
		  	...
		  	<tr>
				  ...
				  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d">
				    <tr align="center">
				      ...
				      <td class="info_title_01" nowrap><!-- 奖励日期 --><ait:message  messageID="display.mutual.award_date"/></td>
				      <td class="info_title_01" nowrap><!-- 奖励类型 --><ait:message  messageID="display.mutual.award_type"/></td>
				      <td class="info_title_01" nowrap><!-- 种类 --><ait:message  messageID="display.mutual.awarder"/></td>
					  <td class="info_title_01" nowrap><!-- 人数 --><ait:message  messageID="display.mutual.people"/></td>
				      <td class="info_title_01" nowrap><!-- 功绩内容 --><ait:message  messageID="display.mutual.award_narrative"/></td>
				      <td class="info_title_01" nowrap><!-- 奖励金额 --><ait:message  messageID="display.mutual.award_sum"/></td>
				    </tr>
				    <c:forEach items="${emp_list}" var="emp" varStatus="j">
					    <tr>
					       <td class="info_content_01" height="30">
									...
									<!-- <input type="hidden" name="transCode_${j.index}" value="a" id="tc"/> -->
									...
									<input type="hidden" name="achievementTypeCode_${j.index}" value=""/>
					      ...
					      <td class="info_content_01">
							<input type="text" size="10" id="rewardDate_${j.index}" name="rewardDate_${j.index}" 
						      		onClick="setday(this);setCheckboxChecked('selectedTag',${j.index});textMuli('rewardDate_',this.id)" 
						      		onChange="textMuli('rewardDate_',this.id)" 
						      		onBlur="textMuli('rewardDate_',this.id)"/>
						  </td>
			
					      
					      
					       <td class="info_content_01">
						  	<ait:codeClass style="content"  codeClass="RewardTypeCode" name="rewardTypeCode_${j.index}" onChange="setCheckboxChecked('selectedTag',${j.index});selMuli('rewardTypeCode_',this.id)"/>
						  </td>
					      <td class="info_content_01">
						  	<ait:codeClass style="content" codeClass="RewardSortCode" name="rewardSortCode_${j.index}" onChange="setCheckboxChecked('selectedTag',${j.index});selMuli('rewardSortCode_',this.id)"/>
						  </td>
						 
					      <td class="info_content_01">
					      	<input style="content" type="text" size="20" id="rewardAmount_${j.index}" name="rewardAmount_${j.index}" 
								      		onChange="setCheckboxChecked('selectedTag',${j.index});textMuli('rewardAmount_',this.id)" />
					      </td>
					      
					      <td class="info_content_01">
					      	<input style="content" type="text" size="20" id="rewardContents_${j.index}" name="rewardContents_${j.index}" 
								      		onChange="setCheckboxChecked('selectedTag',${j.index});textMuli('rewardContents_',this.id)" />
					      </td>
					      
					      <td class="info_content_01">
					      	<input style="content" type="text" size="20" id="rewardBonus_${j.index}" name="rewardBonus_${j.index}" 
								      		onChange="setCheckboxChecked('selectedTag',${j.index});textMuli('rewardBonus_',this.id)" />
					      </td>
					    </tr>
				    </c:forEach>
...
<ait:page       
         ...
         parameters="&operation=viewHortation&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}" 
         total="${resultCount}"
