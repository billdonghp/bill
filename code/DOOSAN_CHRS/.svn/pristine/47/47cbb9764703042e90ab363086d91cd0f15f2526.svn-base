...
public class EssControlServlet extends ControlServlet {

	private CommandManager manager;

	private static final Logger logger = Logger
			.getLogger(EssControlServlet.class);
	...
	public void init(ServletConfig config) throws ServletException {
		...
		manager = CommandManager.getInstance(config.getServletContext()
				.getRealPath("/"), "navigation.ess.path");
	}
	...
	protected Command resolveCommand(HttpServletRequest request)
		...
		try {
			...
			logger.error("Resolve ess command Exception. ");
			throw new GlRuntimeException("Resolve ess command Exception. ", e);
		}
