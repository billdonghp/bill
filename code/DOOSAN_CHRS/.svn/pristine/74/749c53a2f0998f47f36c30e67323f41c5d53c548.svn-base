...
function Add() {
     url = "/syControlServlet?operation=getselectlistforpostaddcmd&menu_code=" + MENU_CODE;           
...
function Update(msg) {      
    ...
    {
        url = "/syControlServlet?operation=retrievedatapostforupdate&postid=" + ID + "&menu_code=" + MENU_CODE;
        location.href = url;       
    }
}
function Active() {
...
function Delete(msg) {
    if (ID == "") {
        ...
        if (confirm(msg[2])) {
            url = "/syControlServlet?operation=deletepostcmd&postid="+ID + "&menu_code=" + MENU_CODE;
            location.href = url;  
        } else {           
            return;   
        }
