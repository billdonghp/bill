...
<SCRIPT language=JavaScript>
	function save(){
	 ...
	 var msg=new Array('<ait:message messageID="alert.sys.maintenance.code.basic_empty" module="sys"></ait:message>',
	    ...
	    code=document.sysCodeUpdateForm.codeId.value;
	   name=document.sysCodeUpdateForm.codeName.value;
	   enname=document.sysCodeUpdateForm.basicEnName.value; 
	   ...
	   if(i==0)             
	   {     
       document.sysCodeUpdateForm.submit();                
        }               
...
<%
	...
	Ent ent = Biz.select(codeNo);
%>
<link href="../css/default.css" rel="stylesheet" type="text/css">
<form  name="sysCodeUpdateForm" method="post" action="/E<%=menu_code%>Control?flag=updateSubCode&menu_code=<%=menu_code%>" onSubmit="return CheckForm()" >
