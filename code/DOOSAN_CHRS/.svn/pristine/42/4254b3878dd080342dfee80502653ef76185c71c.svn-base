...
<SCRIPT type="text/javascript">
 function search(){
 document.form1.action="/gaControlServlet?menu_code=ga0502&operation=gaAffirm&content=viewCardAffirmInfo";
...
<!--		      <td nowrap="nowrap" class="info_title_01">
		    ...
		    <c:forEach items="${eatingCardAffirmInfoList}" var="test" varStatus="i">		
		    <tr align="center">
		      ...
		      <td class="info_textarea_content" >${test.COMPANYNAME}&nbsp;</td>
...
<!--		      <td nowrap="nowrap"  align="center" class="info_content_01">${test.END_DATE}</td>		      -->
		      <td  class="info_textarea_content">${test.RMARK} &nbsp;   </td>
			...
			<td nowrap="nowrap" class="info_textarea_content">        
		      ...
		      <c:if test="${test.FIRSTAFFIRMFLAG==0 && test.APPLYERID eq adminid}"><a href="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&content=deleteCardApply&applyNo=${test.APPLY_NO}" onclick="return(confirm('确认删除吗？这将清空所有的相关信息！'))"><font color="red">删除</font></a></c:if>&nbsp;		  
		      </td>
		      </tr>
		   ...
		   <input type="hidden" name="currentPage" value="${currentPage}">	    
					...
					<ait:page       
		               ...
		               parameters="&operation=gaAffirm&content=viewCardAffirmInfo&menu_code=${param.menu_code}&startDate=${startDate}&endDate=${endDate}&qryType=${qryType}&deptid=${deptid}&key=${key}" 
		               total="${resultCount}"
