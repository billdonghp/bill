...
public class Paitem {
  ...
  String LIST_SQL = "SELECT"
      ...
      + " CPNY_ID,NVL(CPNYNAME,'公共') CPNYNAME "
      + " FROM pa_item_V ";
  ...
  //插入新记录的sql语句，"?"代表参数变量
  String INSERT_SQL = "INSERT INTO pa_item ("
      + " pa_item_no, item_name, descr, datatype, item_type, pricision, carry_bit, "
      + " item_id, item_en_name, item_kor_name, RETROACTIVE_TAX_CALC_MARK,calcu_order,CPNY_ID,SUM_MAINTAIN_FLAG  "
      + " )VALUES("
      + " pa_item_seq.nextval, ?, ?, ?, ?, ?,"
      + " ?, ?, ?, ?, ?,"
      + " (select nvl(max(calcu_order),3000) + 1 as calcu_order from pa_item), ?, ?)";

  //更新记录的sql语句，"?"代表参数变量
  String UPDATE_SQL = "UPDATE pa_item SET"
      + " item_name = ?, descr = ?, datatype = ?,"
      + " item_type = ?, pricision = ?, carry_bit = ?,"
      + " item_id = ?, item_en_name = ?, item_kor_name = ?,"
      + " RETROACTIVE_TAX_CALC_MARK = ?, SUM_MAINTAIN_FLAG = ? "
      + " WHERE pa_item_no = ?";

  //删除记录的sql语句，"?"代表参数变量
  String DELETE_SQL = "DELETE pa_item WHERE pa_item_no = ?";
  ...
  //修改计算顺序--向后
  String CHANGE_ORDER_SQL = "UPDATE pa_item SET"
	  + " calcu_order = ? "
	  + " where pa_item_no = ? ";
 String  StringWhere ="  where (CPNY_ID='"+admin.getCpnyId()+"'  OR  CPNY_ID IS NULL)  ";
  ...
  public Vector List() throws Exception { //搜索信息方法(公司号,搜索方式,搜索信息,开始日期,结束日期,分页)
    ...
    Paitem  paitem  = null;
    try {
      pstmt = conn.prepareStatement(LIST_SQL + StringWhere+ ORDER_SQL);
      rs = pstmt.executeQuery();
      while (rs.next()) { //产生搜索信息循环
        paitem = new Paitem(); //产生实体对象
        //按照上面搜索信息依次附值，以下方法如下
  ...
  public Vector List(String searchsql,com.ait.util.PageControl pc) throws Exception { //搜索信息方法(公司号,搜索方式,搜索信息,开始日期,结束日期,分页)
    ...
    try {
      ...
      while ( (pc.getI() < pc.getIntPagedSize()) && rs.next()) { //产生搜索信息循环
        ...
        paitem.setCpnyId(rs.getString("CPNY_ID"));
        paitem.setCpnyName(rs.getString("CPNYNAME"));
        Vlist.addElement(paitem); //形成一条记录，插入记录列表
        pc.setiii();
      }
    }
  ...
  //得到详细信息数据
  public static Paitem Detail(String No) throws Exception {
    Paitem paitem = null;
    ...
    try {
      ...
      String DETAIL_SQL = "SELECT"
          ...
          + " carry_bit, RETROACTIVE_TAX_CALC_MARK, SUM_MAINTAIN_FLAG "
          + " FROM pa_item"
          + " WHERE pa_item_no = ?";
      ...
      if (rs.next()) { //依次得到值
        ...
        paitem.setSumMainTainFlag(rs.getInt(13)) ;
        //形成一条记录，插入记录列表
      }
    }
    catch (SQLException e) {
  ...
  public void Update() throws Exception {
    ...
    try {
      ...
      pstmt.setInt(11, this.getSumMainTainFlag()) ;
      pstmt.setInt(12, this.getPa_item_no());
  ...
  public void Insert() throws Exception {
    ...
    try {
      ...
      pstmt.setString(9, this.getItem_kor_name());
      pstmt.setInt(10, this.getRetroactiveRaxCalcMark()) ;
      pstmt.setString(11, this.getCpnyId());
      pstmt.setInt(12, this.getSumMainTainFlag()) ;
      if (pstmt.executeUpdate() == 0) {
        throw new NotExistException("No data exists");
      }
    }
  ...
  public void ChangOrder(String type,String pa_item_no) throws Exception {
	...
	private int retroactiveRaxCalcMark = 0 ;
	
	private int sumMainTainFlag = 0 ;
	...
	public int getSumMainTainFlag() {
		return sumMainTainFlag;
	}

	public void setSumMainTainFlag(int sumMainTainFlag) {
		this.sumMainTainFlag = sumMainTainFlag;
	}
}
