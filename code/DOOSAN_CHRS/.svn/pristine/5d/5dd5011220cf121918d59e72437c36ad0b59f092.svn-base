	...
	<tr> 
		...
		<form name="form1" method="post" action="" enctype="multipart/form-data">
			...
			<table width="85%" border="0" cellpadding="0" cellspacing="1" >
				<tr>
					<td align="left" class="title1" colspan="10">整改计划反馈</td>
				</tr>
			</table>
          ...
          dataMap=(SimpleMap)AnviewCorrectivePlanList.get(i);%>
			...
			<tr>
				...
				<td class="info_content_00"><%=StringUtil.checkNull(dataMap.get("CORRECTIVEPLAN"),"&nbsp;")%></td>
			</tr>		
			<tr>
			...
			<tr>
				...
				<td class="info_content_00"><%=StringUtil.checkNull(dataMap.get("PLANDATE"),"&nbsp;")%></td>
			</tr>				
			<tr>
				<td width="20%" class="info_title_01">整改完成照片</td>
				<td class="info_content_00"><%if(dataMap.get("ISCOMPLETEDRECTIFICATION").equals("1")){ %><span style="color:red; cursor:pointer;" onClick="ScenePhotos('<%=dataMap.get("SECURITYCHECKSNO")%>')">查看现场照片</span><%}else{ %><font color="red">整改正在进行，暂无照片</font><%} %></td>
