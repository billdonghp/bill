...
{	
    ...
    document.form1.action="/evControlServlet?operation=evaluateSet&content=evaluateItemSetAdd&menu_code=${param.menu_code}&flag="+j;
...
tableUtil.appendRow = function(){	
		...
		td.innerHTML = "<select name='selectItem"+i+"'> <c:forEach items='${itemList}' var='itemList' varStatus='i'><option value='${itemList.ITEM_ID}'>${ itemList.ITEM_NAME }</option></c:forEach></select>" ;
	...
	<tr>
		...
		<table id="operateTable" width="100%" border="1" cellspacing="0" cellpadding="5"
            ...
            <tr>
				...
				<td width="10%" class="info_content_00"><ait:date yearName="year" monthName="month" yearSelected="${year}" monthSelected="${month}" yearPlus="10" /></td>
			</tr>
			<tr>
				...
				<td class="info_content_00">
					 <select name="selectItem">
							...
							<c:forEach items="${itemList}" var="itemList" varStatus="i">
								...
								<option value="${itemList.ITEM_ID}" <c:if test="${itemList.ITEM_ID==selectItem}">selected</c:if>>
									${itemList.ITEM_NAME}
								</option>
							</c:forEach>
						</select>
	        ...
	        <tr>
				<td width="10%" class="info_title_01">评价项目</td>
				<td width="10%" class="info_title_01">项目比例</td>
