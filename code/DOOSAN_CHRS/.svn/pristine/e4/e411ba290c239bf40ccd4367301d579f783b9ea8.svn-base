...
function Search(){
	...
	document.form1.action="/gaControlServlet?operation=retrieveWashConfirmList&menu_code=${param.menu_code}";
...
function Confirm(confirmFlag, index){
	...
	if (confirm("确定对当前记录进行决裁？")) {
		document.form1.action="/gaControlServlet?operation=confirmWash&menu_code=${param.menu_code}";
		document.form1.submit();
	}
}
...
function doApproval(){
    ...
    if (confirm(msg5)) {
    	...
    	$('op').value = "batch";
		document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=confirmWash";
		document.form1.submit();
	}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
			<tr>
				<td align="left" class="title1" colspan="10">洗衣确认</td>
			</tr>
		</table>
				...
				<tr align="center">
					...
					<td class="info_content_01">
						...
						<c:forEach items="${oneResult.detailList}" var="detail" varStatus="j">
								<tr>
									<td class="info_content_07">${j.index + 1} 名称：${detail.WASH_NAME}</td>
									...
									<td class="info_content_07">洗衣人员明细：${detail.REMARK}</td>
								</tr>
						</c:forEach>
						</table>
					</td>
		...
		<!-- Page Navigation Start--> <ait:page link="/gaControlServlet"
			parameters="&operation=retrieveWashConfirmList&menu_code=${param.menu_code}&personId=${personId}&empId=${empId}&startDate=${startDate}&endDate=${endDate}&flag=${flag}&deptId=${deptId}"
