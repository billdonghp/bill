...
public class InsertEssOvertimeParamCmd extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag!=null && flag.equals("add")){
			return this.addEssOvertimeParam(request,admin);
		}else if(flag!=null && flag.equals("view")){
			return this.viewEssOvertimeParamPage(request,admin);
		}else{
			return "error.jsp";
		}
	}
	private String addEssOvertimeParam(HttpServletRequest request,AdminBean admin) {
		...
		try {
            ...
            // validate R_TIEM
			int result = syservice.validateInsertEssOvertimeParam(parameterObject);
			if (result != 0) {
			...
			syservice.insertEssOvertimeParam(parameterObject);

		} catch (Exception e) {

			Logger.getLogger(getClass()).error(e.toString());
			throw new GlRuntimeException("insert EssOvertimeParam Exception. ", e);
		}
//		添加成功
	...
	private String viewEssOvertimeParamPage(HttpServletRequest request,AdminBean admin) {
		// TODO Auto-generated method stub
		SimpleMap parameterObject;
		List list;
		try {
