...
function band(backColor,textColor,deptid,typeid,empid,affirmor,affirmRelationNo)
{
	var t;
	if(typeof(preEl)!='undefined')
	{
...
//---------------end-----------------------
    ...
    function Add()
    {   
        url="/sy/<%=menu_code%>_a.jsp?menu_code=<%=menu_code%>&x="+document.affirmForm.x.value+"&y="+document.affirmForm.y.value;
        location.href=url;
    }
    function Update()
    {   
		...
		url=url+"&deptcodeId="+document.affirmForm.deptid.value+
    ...
    { 
		...
		{        
			if( confirm(msg[2]) ) 
			{
				url='/sy/sy0106_t.jsp?menu_code=<%=menu_code%>&levelflag=deleteandback&affirmRelationNo='+
					document.affirmForm.affirmRelationNo.value+'&affirmid='+document.affirmForm.affirmor.value
					+'&affirmtypeId='+document.affirmForm.typeid.value+'&deptcodeId='+
					document.affirmForm.deptid.value+'&empId='+document.affirmForm.empid.value+
...
function batch(){
    ...
    if(confirm('<ait:message messageID="alert.sys.authority.approval.update" module="sys"/>')){
	  ...
	  ///  var isUpdate=(document.affirmForm.check.checked==true)?1:0;
		...
		url='/sy/sy0106_t.jsp?menu_code=<%=menu_code%>&levelflag=batch&affirmid='+oldPersonId+'&newaffirmid='
...
<%	AffirmDAO affirmdaodept = new AffirmDAO();
	...
	if (empid.equals("") && deptid.equals("")){
		empid_vector = affirmdaodept.getEmpListByAffirm(affirmId,PERSON_ID);
		String affirmSql="";
		if(!affirmId.equals("")){
		...
		pc.setRowCount(SQL);
		deptid_vector = affirmdaodept.getDeptListByAffirm(affirmId,pc,cnpy_ID);
	...
	<tr>
	    ...
	    <tr>
	      ...
	      <table width="100%" height="30" border="0" cellpadding="0"
	        ...
	        <tr>
	          <td class="info_title_01"><!-- 被决裁部门 -->
	        <ait:message messageID="display.ess.authority_management.approval_management.approvee(dept)" module="sys"></ait:message>	  
	          </td>
	          <td class="info_content_00">
	          		<select name="selectdept">
						<option value="">select</option>
						<%
						 for(int i=0; i<deptid_vector_full.size();i++){
						    affirmDept = (Affirm)deptid_vector_full.elementAt(i);
							if (deptid.equals(affirmDept.getAffirmObject()))
								out.println("<option value=\""+affirmDept.getAffirmObject()+"\" selected >");
							else
								out.println("<option value=\""+affirmDept.getAffirmObject()+"\">");
							%>
							<ait:content enContent='<%=StringUtil.checkNull(affirmDept.getAffirmObjectEnName())%>' zhContent='<%=StringUtil.checkNull(affirmDept.getAffirmObjectName())%>'></ait:content>
							<% 
							//out.println(affirmDept.getAffirmObjectName());
							out.println("</option>");
						}%>   
					</select>
			  </td>
	          <td class="info_title_01"><!-- 被决裁者 -->
	          <ait:message messageID="display.ess.authority_management.approval_management.approvee(individual)" module="sys"></ait:message>	
	          </td>
	          <td class="info_content_00">
	          <select name="selectemp">
				<option value="">select</option>
				<%
				 for(int i=0; i<empid_vector_full.size();i++){
				    affirmEmp = (Affirm)empid_vector_full.elementAt(i);
				if (empid.equals(affirmEmp.getAffirmObject()))
					out.println("<option value=\""+affirmEmp.getAffirmObject()+"\" selected >");
				else
					out.println("<option value=\""+affirmEmp.getAffirmObject()+"\">");
				%>
				<ait:content enContent='<%=StringUtil.checkNull(affirmEmp.getAffirmObjectEnName())%>' zhContent='<%=StringUtil.checkNull(affirmEmp.getAffirmObjectName())%>'></ait:content>
				<% 
				//out.println(affirmEmp.getAffirmObjectName());
			     	out.println("</option>");               
				}%>                       
			</select>
	          </td>
	          <td class="info_title_01"><!-- 决裁者 -->
	          <ait:message messageID="display.mutual.approver" ></ait:message>	
	          </td>
	          <td class="info_content_00">
	          <input type="text"  name="affirmId" id="affirmId" value="<%=affirmId%>" size="7" onKeyUp="javascript:selectEMP(this.value,this.id,100,'syAffirmSearchEmployee','personId');" style="width:60;height:18"></td>
	          <input type="hidden" name="personId" id="personId" value="">
	          <td class="info_content_00">
	          <ait:image src="/images/btn/Search.gif"  border="0" align="absmiddle"
          				onclick="javascript:querybydept();" style="cursor:hand" /></td>
	        </tr>
	       </table>
	      </td>
		</tr>
		...
		<table width="100%" border="1" cellpadding="0" cellspacing="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
		 ...
		 <ait:message messageID="display.mutual.no" ></ait:message>	   
		    ...
		    <td height="30"  class="info_title_01" nowrap="nowrap"><!-- (课组/人员)名称 -->
		 		 <ait:message messageID="display.sys.authority_management.authority.name_dept_staff" module="sys"></ait:message>	
		    </td>
		...
		<% List affirm_list= new ArrayList();                
			for(int i=0; i<deptid_vector.size();i++)
				empid_vector.add(deptid_vector.elementAt(i));
			
			for(int i=0; i<empid_vector.size();i++)
			{
				affirmDept = (Affirm)empid_vector.elementAt(i);
		   %>
		  <tr align="center" >
		    ...
		    <td height="30"   align="center" style="color: #666666;" nowrap="nowrap">
		    	<%=StringUtil.editNbsp(affirmDept.getFourthDeptName())%>
		    </td>
		   ...
		   <table  align="center">
		     ...
		     <%}else{ %>
				   ...
				   <td onClick="band('red','black','<%=affirmDept.getAffirmObject()%>','<%=codemap.get("code")%>','<%=affirm.getAffirmorID()%>','<%=affirm.getAffirmorName()%>','<%=affirm.getAffirmRelationNo() %>')"><%=StringUtil.checkNull(affirm.getAffirmorName())%></td>			   
				   </tr>
				   <%}%>
			 <%}%>
		   </table>
		   ...
		   <%}%>
			...
			<%--<% for(int k=0;k<vector.size();k++){		
				codemap = (HashMap) vector.elementAt(k);   
				affirm_list = affirmdaodept.getAffirmRelation(affirmDept.getAffirmObject(),(String)codemap.get("code"),"1");
				out.print("<td height='30'  align='center' style='color: #666666;' ");
				if(affirm_list.size()==0) {
					//如果没有决裁者，输出空格
					out.print("onClick=\"band('#E7F0EF','black','"+affirmDept.getAffirmObject()
					+"','"+codemap.get("code")+"','','','')\" >");
					out.println("&nbsp;");
				}
				else {
					//如果有决裁者，帮定第一个决裁者到下一个页面
					affirm =(Affirm) affirm_list.get(0);
					out.print("onClick=\"band('#E7F0EF','black','"+affirmDept.getAffirmObject()
					+"','"+codemap.get("code")+"','"+affirm.getAffirmorID()+"','"+affirm.getAffirmorName()+"','"+affirm.getAffirmRelationNo()+"')\" >");
				}
				  for (int j=0;j<affirm_list.size();j++){             
				       affirm =(Affirm) affirm_list.get(j);%>
				  	<%=affirm.getAffirmLevel()%>.&nbsp;<ait:content enContent='<%=affirm.getAffirmorEnName()%>' zhContent='<%=StringUtil.editNbsp(affirm.getAffirmorName())%>'></ait:content>&nbsp; <br>
				  <%}%>
			<%}%>
