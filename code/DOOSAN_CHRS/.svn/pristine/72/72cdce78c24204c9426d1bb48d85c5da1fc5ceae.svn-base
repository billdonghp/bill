...
function Search(){
	document.form1.action="/paControlServlet?operation=viewdeptvouchertypecmd&menu_code=${param.menu_code}&deptid=" + document.form1.DEPTID.value ;
...
function Save(){
	...
	if (checkFlag == 0){
		...
		document.form1.action="/paControlServlet?operation=updatedeptvouchertypecmd&menu_code=${param.menu_code}&deptid=" + document.form1.DEPTID.value ;
		document.form1.submit();
	}
...
function deptVoucherTypeChange(postGradeYear)
{
	...
	if(document.form1.c.checked == true){
	    ...
	    for(var z = 0 ; z < size ; z++ )
		{
			...
			{
				var sel = document.form1(document.form1.selectC[z].value + "_VOUCHER_TYPE");
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
	    ...
	    <tr>
	      ...
	      <table width="100%" height="30" border="0" cellpadding="0"cellspacing="1" class="dr_d">
	      		<tr>
					...
					<td class="info_title_01" width="12%">凭证类型</td>
					 <td class="info_content_00" >
					     <select name="VOUCHER_TYPE">
					        <option value="">全部</option> 
					    	<option value="0000">无</option>
							<c:forEach items="${deptvouchertypelist}" var="oneResult">
								<option value="${oneResult.PZ_TYPE}">${oneResult.PZ_TYPE}</option>
							</c:forEach>
						 </select>
					</td>
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" >
			<tr>
				<td align="left" class="title1" >部门凭证类型  (1000制造，3000销售，4000管理)</td>
			</tr>
		</table>                                      
		<table width="100%" height="30" border="0" cellpadding="0" cellspacing="1" class="dr_d">
			<tr>
				...
				<td class="info_title_01" width="20%">凭证类型</td>
			</tr> 
			<c:forEach items="${deptvoucherdata}" var="VCtr" varStatus="status"> 
			<tr>
			    <td class="info_content_01"><input type="checkbox" name="selectC" value="${VCtr.DEPTID}_${VCtr.PZ_TYPE}"/></td>
			    <td class="info_content_01">${VCtr.DEPTNAME}</td>
			    <td class="info_content_01">
				    <select name="${VCtr.DEPTID}_${VCtr.PZ_TYPE}_VOUCHER_TYPE" onchange="deptVoucherTypeChange(this);"> 
				    	<option value="">无</option>
						<c:forEach items="${deptvouchertypelist}" var="oneResult">
							<option value="${oneResult.PZ_TYPE}" <c:if test="${oneResult.PZ_TYPE == VCtr.PZ_TYPE}">selected</c:if> >${oneResult.PZ_TYPE}</option>
						</c:forEach>
					</select>
			    </td>    
			</tr>      
