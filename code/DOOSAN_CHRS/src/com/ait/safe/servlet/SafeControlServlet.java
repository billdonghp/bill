...
public class SafeControlServlet extends ControlServlet {

	private CommandManager manager;

	private static final Logger logger = Logger
			.getLogger(SafeControlServlet.class);
	...
	public void init(ServletConfig config) throws ServletException {
		...
		manager = CommandManager.getInstance(config.getServletContext()
				.getRealPath("/"), "navigation.safe.path");
	...
	protected Command resolveCommand(HttpServletRequest request)
		...
		try {
			...
			logger.error("Resolve safe command Exception. ");
			throw new GlRuntimeException("Resolve safe command Exception. ", e);
		}
