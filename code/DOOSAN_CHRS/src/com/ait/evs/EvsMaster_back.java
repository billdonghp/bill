...
public class EvsMaster_back implements Serializable {
	...
	public Vector getEvEmpsByMaster(String period, String master, String deptID)
		...
		String sql = "select * from EVS_MASTER_EMP_V1 where 1=1  "
				 + deptSql + "order by OPERATABLE desc";
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		Vector v = new Vector();
...
//			typeSql = " and EV_TYPE_ID = '" + evTypeId + "'";
	...
	// 取得所以参与评价的员工的评价进行信息2
	public List getAllEvEmpsByMasterPeriod2(String period, String master,
			String deptID, String evTypeId) throws DataAccessException {
		String deptSql = "";
		String typeSql = "";
		...
		if (evTypeId.trim().length() != 0)
			typeSql = " and EVS_EMP.EV_TYPE_ID = '" + evTypeId + "'";
		String VIEW_SQL = "SELECT DISTINCT EVS_MASTER.EV_PERIOD_ID as EV_PERIOD_ID, " +
				"EVS_MASTER.EV_MASTER as EV_MASTER," +
				" EVS_EMP.EV_EMP_ID AS EV_EMP_ID," +
				"EVS_EMP.EV_EMP_NAME AS EV_EMP_NAME," +
				"EVS_EMP.EV_DEPT_ID AS EV_DEPT_ID," +
				...
				"SC1.CODE_NAME AS CURRENT_PROCESS_NAME, " +
				"(CASE " +
                 " WHEN EVS_EMP.EV_PROCESS_ID IN " +
                  "     (SELECT EV_PROCESS_ID " +
                    "      FROM EVS_MASTER EM " +
                   "      WHERE EM.EV_PERIOD_ID = EVS_EMP.EV_PERIOD_ID " +
                    "       AND EM.EV_EMP_ID = EVS_EMP.EV_EMP_ID " +
                     "      AND EM.EV_MASTER = EVS_MASTER.EV_MASTER) THEN " +
                   "(CASE " +
                  "WHEN (SELECT EV_PROCESS_SDATE " +
                  "        FROM EVS_TYPE_PROCESS " +
                   "      WHERE EV_PERIOD_ID = EVS_EMP.EV_PERIOD_ID " +
                    "       AND EV_TYPE_ID = EVS_EMP.EV_TYPE_ID " +
                     "      AND EV_PROCESS_ID = EVS_EMP.EV_PROCESS_ID) <= " +
                      " SYSDATE AND " +
                ...
                "sc3.code_name AS ev_grade_name, " +
                "r.setupcodeno, " +
                " get_evscodename(r.craft,'CRAFT','78000000') as CRAFT, " +
                "get_evscodename(r.skilltype,r.craft,'78000000') AS SKILLTYPE, " +
                " get_evscodename(r.line,r.craft,'78000000') AS LINE, " +
                "get_evscodename(r.aircraft,r.line,'78000000')  AS AIRCRAFT, " +
                "get_evscodename(r.process,r.line,'78000000') AS PROCESS," +
                "get_evscodename(r.jobcontent,r.process,'78000000') AS JOBCONTENT, " +
                " get_evscodename(r.typeoperation,'1','78000000') AS TYPEOPERATION, " +
                ...
                "get_sys_code(r.skilllevel) AS SKILLLEVAL, " +
                "get_sys_code(r.qualification) AS QUALIFICATION, " +
                "r.skillscore AS SKILLSCORE, " +
                "get_sys_code(r.purpose) AS PURPOSE, " +
                "r.remark AS REMARK " +
  ...
  "     evs_emp, " +
   "    SY_CODE SC1, " +
    "   SY_CODE SC3," +
     "  SY_CODE SC2, " +
      " evs_setupcode r " +
 "where EVS_MASTER.EV_PERIOD_ID = EVS_EMP.EV_PERIOD_ID " +
  " AND EVS_MASTER.EV_EMP_ID = EVS_EMP.EV_EMP_ID " +
   "and SC1.CODE_ID = EVS_EMP.EV_PROCESS_ID " +
   "AND SC1.PARENT_CODE = 'EVPROCESS' " +
   "and SC3.CODE_ID(+) = EVS_EMP.EV_GRADE_ID " +
   "AND SC3.PARENT_CODE(+) = 'EVEMPGRADE' " +
   "and SC2.CODE_ID(+) = EVS_EMP.EV_TYPE_ID " +
   "AND SC2.PARENT_CODE(+) = 'EVSTYPE' " +
   "and EVS_EMP.Ev_Period_Id = r.ev_period_id (+)" +
   "and EVS_EMP.Ev_Emp_Id = r.evs_empid (+)" +
   "and EVS_EMP.EV_TYPE_ID = r.ev_type_id(+) ";

		String sql = VIEW_SQL + " and EVS_MASTER.EV_PERIOD_ID = ? AND "
				+ " EVS_MASTER.EV_MASTER = ? " + deptSql + typeSql
				+ "order by  EV_EMP_ID,OPERATABLE desc";
		Connection con = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		List lEvsEmp = new Vector();
		try {
			...
			while (rs.next()) {
			    ...
			    evsEmp.setSKILLLEVEL(rs.getString("SKILLLEVAL"));
