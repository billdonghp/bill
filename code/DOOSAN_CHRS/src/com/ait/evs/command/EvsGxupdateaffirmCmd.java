...
public class EvsGxupdateaffirmCmd  implements Command{

				private static final Logger logger = Logger
				.getLogger(EvsGxupdateaffirmCmd.class);
   ...
   throws ServletException, IOException {
	MessageSource messageSource ;
			...
			try { 
				...
				///决裁操作
				int result = evsservices.doGxModifyAffirm(params,flag);
				...
				if(result > 0 )
				{
					if(flag == 1){
						...
						List list1 = evsservices.getGxModifyNextAffirmor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								List list2 = evsservices.getGxModifyApplyNoForMail(simpleMap2);
					...
					//开始
						...
						simpleMap.setString("applyStr", buffer.substring(0,buffer.length()-1));
						List list1 = evsservices.getGxApplyor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								simpleMap2.setString("applyStr", buffer.substring(0,buffer.length()-1));
								List list2 = evsservices.getGxApplyNoCreateBy(simpleMap2);
			...
			return "/evs0128_modify_affirm.jsp";
		}
	public void sendLeaveMail(int[]Applyno,String setTo,int flag)throws Exception{
		...
		List applyResult = (List) evsservices.applyGxResult(parameterObject);
		SimpleMap inputData = new SimpleMap();
		String emailTitle="";	
		StringBuffer content = new StringBuffer();
		if(flag==1){
			emailTitle="工序难度系数修改申请";
		}else if(flag==2){
			emailTitle="工序难度系数修改申请,被否决";
		}else{
			emailTitle="工序难度系数修改申请";
		}
		...
		for(int i=0;i<applyResult.size();i++){
			...
			content.append("<br><br>").append("工序:").append(simpleMap.getString("CODE_ID"));
			content.append("<br><br>").append("难度系数:").append(simpleMap.getString("DENGJI"));
	...
	public void sendArModifyMail(int[]Applyno,String setTo,int flag,String adminId)throws Exception{//比上面的方法多一个决裁意见
		...
		parameterObject.setString("applyNoStr", applyStr.substring(0,applyStr.length()-1));
		List applyResult = (List) evsservices.applyGxModifyResult(parameterObject);
		SimpleMap inputData = new SimpleMap();
		String emailTitle="";	
		StringBuffer content = new StringBuffer();
		if(flag==1){
		...
		for(int i=0;i<applyResult.size();i++){
			...
			content.append("<br>").append("工序:").append(StringUtil.checkNull(simpleMap.getString("CODE_ID")));
			content.append("<br>").append("难度系数:").append(StringUtil.checkNull(simpleMap.getString("DENGJI")));			
