...
public class EvszyzgdjupdateaffirmCmd  implements Command{
   ...
   throws ServletException, IOException {
	MessageSource messageSource ;
			...
			try { 
				...
				///决裁操作
				int result = evsservices.doZyezgdjModifyAffirm(params,flag);
				...
				if(result > 0 )
				{
					if(flag == 1){
						...
						List list1 = evsservices.getZyzgdjModifyNextAffirmor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								List list2 = evsservices.getArModifyApplyNoForMail(simpleMap2);
					...
					//开始
						...
						simpleMap.setString("applyStr", buffer.substring(0,buffer.length()-1));
						List list1 = evsservices.getLeaveApplyor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								simpleMap2.setString("applyStr", buffer.substring(0,buffer.length()-1));
								List list2 = evsservices.getLeaveApplyNoCreateBy(simpleMap2);
			...
			return "/evs0134_modify_affirm.jsp";
		}
	public void sendLeaveMail(int[]Applyno,String setTo,int flag)throws Exception{
		...
		if(flag==1){
			emailTitle="职业资格等级设置修改申请";
		}else if(flag==2){
			emailTitle="职业资格等级设置修改申请,被否决";
		}else{
			emailTitle="职业资格等级设置修改申请";
		}
		...
		for(int i=0;i<applyResult.size();i++){
			...
			content.append("<br><br>").append("工种:").append(simpleMap.getString("CODE_ID"));
			content.append("<br><br>").append("等级:").append(simpleMap.getString("DENGJI"));
			content.append("<br><br>").append("积分From:").append(simpleMap.getString("DJJFFROM"))
				   .append("<br><br>").append("积分To:").append(simpleMap.getString("DJJFTO"));
	...
	public void sendArModifyMail(int[]Applyno,String setTo,int flag,String adminId)throws Exception{//比上面的方法多一个决裁意见
		...
		for(int i=0;i<applyResult.size();i++){
			...
			content.append("<br>").append("修改日期:").append(StringUtil.checkNull(simpleMap.getString("APPLY_DATE")));
			content.append("<br>").append("工种:").append(StringUtil.checkNull(simpleMap.getString("CODE_ID")));
			content.append("<br>").append("等级:").append(StringUtil.checkNull(simpleMap.getString("DENGJI")));
			content.append("<br>").append("积分From:").append(StringUtil.checkNull(simpleMap.getString("DJJFFROM")));
			content.append("<br>").append("积分To:").append(StringUtil.checkNull(simpleMap.getString("DJJFTO")));
