...
public class EvsGjjsaffirmCmd  implements Command{

				private static final Logger logger = Logger
				.getLogger(EvsGjjsaffirmCmd.class);
   ...
   throws ServletException, IOException {
	MessageSource messageSource ;
			...
			try { 
				...
				///决裁操作
				int result = evsservices.doGjjsAffirm(params,flag);
				...
				if(result > 0 )
				{
					if(flag == 1){
						...
						List list1 = evsservices.getGjjsNextAffirmor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								List list2 = evsservices.getGjjsApplyNoForMail(simpleMap2);
						...
						simpleMap.setString("applyStr", buffer.substring(0,buffer.length()-1));
						List list1 = evsservices.getGjjsApplyor(simpleMap);
						...
						if(list1.size()>0){
							...
							for(;iterator.hasNext();){
								...
								simpleMap2.setString("applyStr", buffer.substring(0,buffer.length()-1));
								List list2 = evsservices.getGjjsApplyNoCreateBy(simpleMap2);
	...
	public void sendLeaveMail(int[]Applyno,String setTo,int flag)throws Exception{
		...
		if(flag==1){
			emailTitle="工匠技师提交决裁申请";
		}else if(flag==2){
			emailTitle="工匠技师提交决裁申请,被否决";
		}else{
			emailTitle="工匠技师提交决裁申请";
		}
		...
		for(int i=0;i<applyResult.size();i++){
			...
			content.append("<br>").append("姓名:").append(StringUtil.checkNull(simpleMap.getString("JOBCONTENT")));
			content.append("<br>").append("总分:").append(StringUtil.checkNull(simpleMap.getString("COMPOSITE")));
