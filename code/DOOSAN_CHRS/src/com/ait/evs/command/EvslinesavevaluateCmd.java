...
public class EvslinesavevaluateCmd  implements Command{
			...
			try { 
				...
				if (craft != null) {
					for (int j = 0; j < craft.length; j++) {        
						...
						evsCraft.setEvsparentcode(craft[j]);
					 ...
					 EntEvsCode Enty = BizEvsCode.selectid(evscodeid);
					 if(Enty!=null)
		        	 {
		        		 if(Enty.getStatus()!=null&&Enty.getStatus().equals("0")){
		        			
			        		 biz.Delete(evscodeid);
