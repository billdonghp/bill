...
public class Paformular {
  ...
  //插入新记录的sql语句，"?"代表参数变量
  String INSERT_SQL = "INSERT INTO kpa_formular ("
      + " formular_no, pa_item_no,"
      + " condition, formular,"
      + " condition_cn, formular_cn,"
      + " condition_seq"
      + " )VALUES("
      + " kpa_formular_seq.nextval, ?,"
      + " ?, ?,"
      + " ?, ?,"
	  + " (select case when max(condition_seq) is null "
	  + " then 1 else max(condition_seq)+1 end as condition_seq from kpa_formular where pa_item_no = ?))";

  //更新记录的sql语句，"?"代表参数变量
  String UPDATE_SQL = "UPDATE kpa_formular SET"
      + " condition = ?,formular = ?, "
      + " condition_cn = ?,formular_cn = ? "
      + " WHERE formular_no = ?";

  //删除记录的sql语句，"?"代表参数变量
  String DELETE_SQL = "DELETE kpa_formular WHERE formular_no = ?";
  ...
  private void formatData()
  {
	  ...
	  int size = statTypeList.size() ;
	  for (int i = 0 ; i < size ; i ++ )
	  {
		  codeBean = (SysCodeBean)statTypeList.get(i) ;	  
		  if(this.condition.indexOf(codeBean.getCodeName()) != -1)
		  {
	  ...
	  List pa_item_list = services.retrievePa_Item_List_Month() ;
