...
public class ExportReport0206Cmd extends ArAbstractCommand {
		...
		try {
			...
			// 建立SQL字段和Excel列名的映射(针对重用以前的SQL,SQL结果集必须为SimleMap)
			 ...
			 if (parameterObject.getString("cpnyId").equals("60000000")) {
				 columns.setString("部门", "PARTNAME");
			 }
			 else 			 
			 {
				 columns.setString("部门", "OFFICENAME"); 
			 }
//			 if (parameterObject.getString("cpnyId").equals("78000000")) {  20190715 sunying
//			 if (parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("63000000")) {
...
//			 }
			 ...
			 columns.setString("职责津贴", "JOB_ALLOWANCE"); //20190716增加职责津贴    sunying
			 columns.setString("住宅补助", "RESIDENTIAL_GRANTS");
			 columns.setString("地区补助", "REGIONAL_GRANTS");

//			 if (!parameterObject.getString("cpnyId").equals("63000000")){ //20190715dicc、disd统一 sunying
//				 columns.setString("特殊补助", "SPECIAL_GRANTS") ; 
//			 }
			 if (parameterObject.getString("cpnyId").equals("63000000")||parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("61000000")){
                 columns.setString("岗位补助", "POST_SUBSIDIES");
			 }
//			 if (parameterObject.getString("cpnyId").equals("60000000")){
//			     columns.setString("职位补贴", "POSITION_ALLOWANCE");
//			 }
			 //if(parameterObject.getString("cpnyId").equals("78000000")){//20190715dicc、disd统一 sunying
			 if (parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("63000000")){//20190715 sunying
...
//			 if (parameterObject.getString("cpnyId").equals("63000000")) {//20190715dicc、disd统一 sunying
			 if (parameterObject.getString("cpnyId").equals("63000000")||parameterObject.getString("cpnyId").equals("78000000")) {// 20190715 sunying dicc、disd统一
				 columns.setString("研发补助", "RESEARCH_SUBSIDIES");
				 columns.setString("高温补助", "PAYMENT_HT_ALLOWANCE");
			 }
			if (parameterObject.getString("cpnyId").equals("60000000")) {
			 ...
			 if (parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("63000000")||parameterObject.getString("cpnyId").equals("60000000")){
                 columns.setString("手机话费", "MOBILE_PHONE_FEE");
				 columns.setString("燃油费", "FUEL_SURCHARGE_FEE");
			 }
//			 if (parameterObject.getString("cpnyId").equals("63000000")){//20190715dicc、disd统一 sunying
			 if (parameterObject.getString("cpnyId").equals("63000000")||parameterObject.getString("cpnyId").equals("78000000")){//20190715 sunying dicc、disd统一
				 columns.setString("产假支给", "MATERNITY_PAY");
			 }
...
//			if (parameterObject.getString("cpnyId").equals("78000000")) {//20190715dicc、disd统一 sunying
			if (parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("63000000")) {//20190715 sunying dicc、disd 统一
				 columns.setString("放假方案减除", "HOLIDAY_PLAN");
				 columns.setString("岗位保留减除", "A_POST_RESERVATION_MINUS");
				 columns.setString("特补减除", "SPECIAL_GRANTS_MINUS");
			     columns.setString("改善提案奖", "MEND_OVERTURE_AWARD");
			} 
			
			 columns.setString("实习试用扣除", "TRIAL_MINUS");
...
//				if (parameterObject.getString("cpnyId").equals("78000000")) {//20190715dicc、disd统一 sunying
			 if (parameterObject.getString("cpnyId").equals("78000000")||parameterObject.getString("cpnyId").equals("63000000")) { //20190715 sunying dicc、disd统一
					columns.setString("餐费计税", "MEAL_DUTY");
					columns.setString("税后减除", "TAX_AFTER_MINUS");//原disd内容
				}
				
			 //columns.setString("当前月个税抵扣合计", "CAL_DQGSDK_TOTAL");
//			 if (parameterObject.getString("cpnyId").equals("63000000")) {//20190715dicc、disd统一 sunying
//	                columns.setString("餐税", "MEAL_DUTY");
