...
public class ExportReport0225Cmd extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			{
				recordList = service.retrieveReport0225PaList(parameterObject);
			}
			else
			{
				recordList = service.retrieveReport0225BonusList(parameterObject);
			}
			...
			// 建立SQL字段和Excel列名的映射(针对重用以前的SQL,SQL结果集必须为SimleMap)
			 ...
			 columns.setString("银行名称", "BANK_CARD_NAME");
