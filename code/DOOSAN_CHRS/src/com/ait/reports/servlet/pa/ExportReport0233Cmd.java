 ...
 * 
 */
public class ExportReport0233Cmd extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List recordList = service.retrieveReport0233List(parameterObject);
			...
			// 建立SQL字段和Excel列名的映射(针对重用以前的SQL,SQL结果集必须为SimleMap)
			 ...
			 columnType.setInt("POST_GRADE", ReportConstant.CELL_TYPE_TEXT);
			 
			// 设置报表参数
			ExcelParameterBean paramBean = new ExcelParameterBean();
			paramBean.setFileName("pa_summary_post_grade.xls");
			paramBean.setSheetname("PA_SUMMARY_POST_GRADE");
			...
			// 设置内嵌表头
			// LSFC考勤刷卡记录表
			paramBean.setInLineHeadContent("斗山（中国）投资有限公司  "+paMonth.substring(0,4)+"年"+paMonth.substring(4,6)+"月 工资汇总表（职级别）");
