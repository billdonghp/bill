 ...
 * 
 */
public class ExportReport0231Cmd extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			List recordList = service.retrieveReport0231List(parameterObject) ;

			// 建立SQL字段和Excel列名的映射(针对重用以前的SQL,SQL结果集必须为SimleMap)
			 SimpleMap columns = new SimpleMap();
			 columns.setString("序号", "NUM");
			 columns.setString("姓名", "CHINESENAME");
			 columns.setString("开户行", "BANK");
			 columns.setString("银行账号", "BANK_ID");
			...
			paramBean.setFileName("pa_real_wages_schedule_bank.xls");
			paramBean.setSheetname("PA_REAL_WAGES_DATA");
			...
			// 设置内嵌表头
			// LSFC考勤刷卡记录表
			paramBean.setInLineHeadContent("斗山（中国）投资有限公司  "+paMonth.substring(0,4)+"年"+paMonth.substring(4,6)+"月 实发工资明细表");
