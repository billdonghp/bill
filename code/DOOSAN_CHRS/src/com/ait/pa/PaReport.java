...
public class PaReport {
    ...
    // 选出所有记录列表的sql语句
    String SELECT_SQL = "SELECT * " + " FROM T_PA_RESULT ";
    
    /*
    String Column_SQL = "SELECT * FROM ( "
    				+ " SELECT 'PA_MONTH' AS COLUMN_ID,'工资月' AS COLUMN_NAME,'Pay Month' AS COLUMN_EN_NAME,'' AS COLUMN_KOR_NAME, 0 AS ORDER_NO FROM DUAL"
	    			+ " UNION ALL "
					+ " SELECT A.DISTINCT_FIELD, A.FIELD_NAME, A.FIELD_EN_NAME, A.FIELD_KOR_NAME, A.ID"
					+ "  FROM PA_DISTINCT_LIST A "
					+ " WHERE (A.LANG_GROUP = 0 OR A.LANG_GROUP = 1) AND A.TABLE_NAME IN ('PA_HR_V','PA_AR_V') "     
					+ " UNION ALL "
					+ " SELECT B.ITEM_ID, B.ITEM_NAME, B.ITEM_EN_NAME, B.ITEM_KOR_NAME, B.CALCU_ORDER FROM PA_ITEM B  WHERE (B.CPNY_ID=? OR B.CPNY_ID IS NULL)  "
					+ " UNION ALL "
					+ " SELECT C.PARAM_ID, C.PARAM_NAME, C.PARAM_EN_NAME, C.PARAM_KOR_NAME, C.ORDER_NO FROM PA_PARAM_ITEM C WHERE (C.CPNY_ID=? OR C.CPNY_ID IS NULL))  "
					+ " ORDER BY ORDER_NO " ;
    */
    String Column_SQL1 = "SELECT PD.*,PH.ISVIEW FROM (SELECT 'PA_MONTH' AS COLUMN_ID, '工资月' AS COLUMN_NAME, 'Pay Month' AS COLUMN_EN_NAME,"
               		+"'' AS COLUMN_KOR_NAME,0 AS ID_NO FROM DUAL UNION ALL SELECT A.DISTINCT_FIELD, A.FIELD_NAME, A.FIELD_EN_NAME,"
               		+"A.FIELD_KOR_NAME, A.ID AS ID_NO FROM PA_DISTINCT_LIST A WHERE (A.LANG_GROUP = 0 OR A.LANG_GROUP = 1)"
               		+"AND A.TABLE_NAME IN ('PA_HR_V', 'PA_AR_V')) PD, PA_HISTORY_COL PH WHERE PD.COLUMN_ID = PH.COL_EN"
               		+" AND PH.CPNY_ID = ? ORDER BY PD.ID_NO";
    
    String Column_SQL2 = "SELECT B.ITEM_ID AS COLUMN_ID, B.ITEM_NAME AS COLUMN_NAME, B.ITEM_EN_NAME AS COLUMN_EN_NAME," 
    				+" B.ITEM_KOR_NAME AS COLUMN_KOR_NAME, B.CALCU_ORDER , C.ISVIEW FROM PA_ITEM B, PA_HISTORY_COL C WHERE B.ITEM_ID = C.COL_EN"
    				+" AND C.CPNY_ID = ? AND  (B.CPNY_ID = ? OR B.CPNY_ID IS NULL) ORDER BY CALCU_ORDER";
    
    String Column_SQL3 = "SELECT C.PARAM_ID AS COLUMN_ID, C.PARAM_NAME AS COLUMN_NAME, C.PARAM_EN_NAME AS COLUMN_EN_NAME,"
    				+" C.PARAM_KOR_NAME AS COLUMN_KOR_NAME, C.ORDER_NO , D.ISVIEW FROM PA_PARAM_ITEM C, PA_HISTORY_COL D"
    				+" WHERE C.PARAM_ID = D.COL_EN AND D.ISVIEW = 0 AND D.CPNY_ID = ? "
    				+" AND (C.CPNY_ID = ? OR C.CPNY_ID IS NULL) ORDER BY ORDER_NO";
    
    
    
    String PaBonusColumn_SQL = "SELECT * FROM ( "
    	+ " SELECT 'PA_MONTH' AS COLUMN_ID,'工资月' AS COLUMN_NAME, 0 AS ORDER_NO FROM DUAL "
    	+ " UNION ALL "
    	+ " SELECT 'BONUS_TYPE' AS COLUMN_ID,'奖金类别' AS COLUMN_NAME, 0 AS ORDER_NO FROM DUAL "
		+ " UNION ALL "
		+ " SELECT A.DISTINCT_FIELD, A.FIELD_NAME, A.ID FROM PA_DISTINCT_LIST A WHERE (A.LANG_GROUP = 0 OR A.LANG_GROUP = 1) AND A.TABLE_NAME IN ('PA_HR_V') "     
		+ " UNION ALL "
		+ " SELECT C.PARAM_ID, C.PARAM_NAME, C.ORDER_NO FROM PA_BONUS_PARAM_ITEM C WHERE (C.CPNY_ID=? OR C.CPNY_ID IS NULL) "
		+ " UNION ALL "
		+ " SELECT B.ITEM_ID, B.ITEM_NAME, B.CALCU_ORDER FROM PA_BONUS_ITEM B WHERE  (B.CPNY_ID=? OR B.CPNY_ID IS NULL) ) " 
		+ " ORDER BY ORDER_NO " ;
    ...
    public List ColumnPaSelect() throws Exception { // 搜索信息方法(搜索方式,搜索信息,开始日期,结束日期,分页)
        PaReport report = null;
        Connection conn = Conn(); // 建立连接
        PreparedStatement pstmt = null; // 建立参数声明
        ResultSet rs = null; // 建立数据集
        ...
        try {
        	logger.debug("SQL1= " + Column_SQL1);
        	logger.debug("SQL2= " + Column_SQL2);
        	logger.debug("SQL3= " + Column_SQL3);
        	
            report = new PaReport();
            pstmt = conn.prepareStatement(Column_SQL1);
            pstmt.setString(1, admin.getCpnyId());
            rs = pstmt.executeQuery(); // 得到信息数据集
            while (rs.next()) { // 依次得到值
                report = new PaReport(); // 产生实体对象
                ...
                report.setIsView(rs.getInt("ISVIEW"));
                Vlist1.addElement(report); // 形成一条记录，插入记录列表
            }
            
            pstmt = conn.prepareStatement(Column_SQL2);
            pstmt.setString(1, admin.getCpnyId()) ;
            pstmt.setString(2, admin.getCpnyId()) ;
            rs = pstmt.executeQuery(); // 得到信息数据集
            while (rs.next()) { // 依次得到值
                report = new PaReport(); // 产生实体对象
                report.setColumnId(rs.getString("COLUMN_ID"));
                ...
                report.setColumnKoName(rs.getString("COLUMN_KOR_NAME"));
                report.setIsView(rs.getInt("ISVIEW"));
                Vlist2.addElement(report); // 形成一条记录，插入记录列表
            }
            
            pstmt = conn.prepareStatement(Column_SQL3);
            pstmt.setString(1, admin.getCpnyId()) ;
            pstmt.setString(2, admin.getCpnyId()) ;
            rs = pstmt.executeQuery(); // 得到信息数据集
            while (rs.next()) { // 依次得到值
                ...
                report.setColumnKoName(rs.getString("COLUMN_KOR_NAME"));
                report.setIsView(rs.getInt("ISVIEW"));
                Vlist3.addElement(report); // 形成一条记录，插入记录列表
            }
            list.add(Vlist1);
            list.add(Vlist2);
            list.add(Vlist3);
    ...
    public Vector paBonusColumnSelect() throws Exception { // 搜索信息方法(搜索方式,搜索信息,开始日期,结束日期,分页)
        PaReport report = null;
        Connection conn = Conn(); // 建立连接
        PreparedStatement pstmt = null; // 建立参数声明
        ResultSet rs = null; // 建立数据集
    ...
    public String PaConfirm(String adminid) throws Exception { // 搜索信息方法(搜索方式,搜索信息,开始日期,结束日期,分页)
        ...
        try {
            cs = conn.prepareCall("{call pa_confirm(?,?)}");
