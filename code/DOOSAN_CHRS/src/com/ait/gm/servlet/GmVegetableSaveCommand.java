...
public class GmVegetableSaveCommand extends ArAbstractCommand {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			int result = NumberUtil.parseInt(gm.validateGmVegetable(parameterObject));

			if(result!=0){
				messageSource = new MessageSource("gm",admin.getLocale(), "UTF-8");
				request.setAttribute("alert", "该菜品代码已存在!返回重新添加");
				request.setAttribute("reload","history.back();");
				return "/inc/alertMessage.jsp";
			}
					
			// insert GmVegetable
			gm.insertGmVegetable(parameterObject);
