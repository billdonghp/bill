...
function search(){
	document.form1.action="/gaControlServlet?menu_code=ga0211&operation=gaAffirm&content=viewDriverOtAffirm";
	document.form1.submit();
}
function checkAll()
{
...
var msg1='<ait:message messageID="alert.ess.common.nodatatopass" module="ess"></ait:message>';
    ...
    if (confirm(msg5)) {
	document.form1.action = "/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&flag=1&content=driverOtApprovalOKorNO";
	document.form1.submit();
	}
...
function doReject(){
	...
	if (confirm(msg6)) {
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&flag=2&content=driverOtApprovalOKorNO";
	document.form1.submit();
	}
}
function driverOtApprovalOKorNO(applyno,flag){
document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&content=driverOtApprovalOKorNO&flag="+flag+"&applyno="+applyno;
document.form1.submit();
}
function ImportExcel() {
    document.form1.action="/gaControlServlet?menu_code=ga0211&operation=gaAffirm&content=viewDriverOtAffirm&excelFlag=excel";
	document.form1.submit();
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<table width="100%" height="30" border="0" cellspacing="1" cellpadding="0" >
		  ...
		  <ait:message messageID="display.mutual.end_date" module="ess"></ait:message> 
		   ...
		   <ait:message messageID="display.mutual.status" module="ess"></ait:message>            	  
		     ...
		     <ait:message messageID="display.mutual.key_word" module="ess"></ait:message>                
		          ...
		          <td class="info_content_00" nowrap="nowrap">  <input type="text" name="key" value="${key}"  title="输入姓名"/></td>       
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF">
		    ...
		    <tr>
		    <c:forEach items="${driverOtList}" var="list">
		    <tr align="center">	
		    <td nowrap="nowrap" align="center" class="info_content_01" >
		      	&nbsp;<c:if test="${ list.ACTIVE == 0 && list.AFFIRM_FLAG==0 }">
		      	 <input type="checkbox" name="applyno" value="${list.DRIVER_OT_APPLYID}" />
		      	</c:if>&nbsp;
			  </td>		
			  <td nowrap="nowrap" align="center" class="info_content_01" >
		      ...
		      <td nowrap="nowrap" align="center" class="info_content_01" >
    			...
    			<input type="hidden" name="applerId_${list.DRIVER_OT_APPLYID }" value="${list.DRIVER_OT_APPLYID }"> 
    			<input type="hidden" name="appler_Name_${list.DRIVER_OT_APPLYID }" value="${list.DRIVER_CHINESENAME }">
    			<input type="hidden" name="DRIVER_OT_TYPE_${list.DRIVER_OT_APPLYID }" value="${list.DRIVER_OT_TYPE_NAME }">
    			<input type="hidden" name="DRIVER_OT_APPLY_STARTDATE_${list.DRIVER_OT_APPLYID }" value="${list.APPLY_STARTDATE }">
    			<input type="hidden" name="DRIVER_OT_APPLY_STARTTIME_${list.DRIVER_OT_APPLYID }" value="${list.APPLY_STARTTIME }">
    			<input type="hidden" name="DRIVER_OT_APPLY_ENDDATE_${list.DRIVER_OT_APPLYID }" value="${list.APPLY_ENDDATE }">
    			<input type="hidden" name="DRIVER_OT_APPLY_ENDTIME_${list.DRIVER_OT_APPLYID }" value="${list.APPLY_ENDTIME }">
   			 ...
   			 ${list.APPLY_ENDTIME }&nbsp;</td>
   			  ...
   			  <td class="info_content_00" nowrap="nowrap">
		    	<a style="cursor:hand" id="leftnewstd" title='查看详细信息' onClick="showMemo('${list.DRIVER_OT_APPLYID}');">
             ...
             <td nowrap="nowrap" align="center" class="info_content_01" >
   			 ...
   			 <td nowrap="nowrap" align="center" class="info_content_01" >
   			 <c:forEach items="${list.driverOtAffiirmList}" var="view" varStatus="i">
		      	...
		      	<c:if test="${view.AFFIRM_FLAG==2}"><font color="#00CC00">已否决</font></c:if>
   			 	...
   			 	<c:if test="${view.AFFIRMOR_ID == adminId}">
   			 	<input type="hidden" name="affirmNo_${list.DRIVER_OT_APPLYID}" value="${view.ID}">
   			 	<input type="hidden" name="MAX_AFFIRM_FLAG_${list.DRIVER_OT_APPLYID}" value="${view.MAX_AFFIRM_FLAG}">
   			 	<input type="hidden" name="AFFIRM_LEVEL_${list.DRIVER_OT_APPLYID}" value="${view.AFFIRM_LEVEL}">
   			 	<font color="#990099">${view.AFFIRM_LEVEL }${view.CHINESENAME }</font>
   			 		<c:if test="${view.AFFIRM_FLAG==0}"><font color="#990099">未决裁</font></c:if>
   			 		<c:if test="${view.AFFIRM_FLAG==1}"><font color="#00CC00">通过</font></c:if>
					<c:if test="${view.AFFIRM_FLAG==2}"><font color="#00CC00">已否决</font></c:if>
			      	...
			      	<c:if test="${list.ACTIVE==0 && view.AFFIRM_FLAG==0}">|
			      		<a href="#" onclick="driverOtApprovalOKorNO('${list.DRIVER_OT_APPLYID}','1')"><font color="red">通过</font></a>
			      	</c:if>
			      	<c:if test="${list.ACTIVE==0 && view.AFFIRM_FLAG==0}">
<!--			      		<a href="&affirmNo_${list.VOITURE_APPLYID}=${view.ID}&MAX_AFFIRM_FLAG_${list.VOITURE_APPLYID}=${view.MAX_AFFIRM_FLAG}&applyno=${list.VOITURE_APPLYID}&content=visiterApprovalOKorNO&affirmorIdea_=${list.VOITURE_APPLYID}">-->
			      			<a href="#" onclick="driverOtApprovalOKorNO('${list.DRIVER_OT_APPLYID}','2')"><font color="red">否决</font></a>
			      	</c:if>
   			 	</c:if>
   			 	<br />
   			 </c:forEach>
   			 <td nowrap="nowrap"  align="center" class="info_content_06"><textarea name="affirmorIdea_${list.DRIVER_OT_APPLYID}" style=" height: 40px;width:100px " type="_moz" onfocus="this.style.height='50px'" onblur="this.style.height='40px';">${list.AFFIRMORIDEA}</textarea></td>
	...
	<ait:page       
             ...
             parameters="&operation=gaAffirm&content=viewDriverOtAffirm&menu_code=${param.menu_code}&qryType=${qryType }&deptid=${deptid }&endDate=${endDate }&startDate=${startDate }&key=${key }"
             total="${driverOtInt}"
