...
<SCRIPT type="text/javascript">
 ...
 function search(){
 document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=view&content=temporarycardapplicationview";
 ...
 function doBatchConfirm(){ 
   ...
   if(!selectFlag){
		  ...
		  document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=view&content=oingTempCardConfirm&Flag=1";
		  document.form1.submit();
  }
 ...
 function doBatchReject(){
   if(confirm("确认对已选择的信息进行否决吗？")){  
	   ...
	   if(!selectFlag){
			  ...
			  document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=view&content=oingTempCardConfirm&Flag=2";;
			  document.form1.submit();
	  }
  }
}
 ...
 function ImportExcel() {
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=view&content=exceltemporarycardapplicationview";
	document.form1.submit();
}
  
 function tempCardProvide(applyNo){
  document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=view&content=tempCardProvideView&applyNo="+applyNo;
  document.form1.submit(); 
 }
</SCRIPT>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		        ...
		        <tr>
		         ...
		         <td  class="info_content_00"><ait:codeClass codeClass="EMP_DIFF" name="cpnyId" selected="${cpnyId}" onChange="search();"/></td> 
		          ...
		          <td class="info_content_00" nowrap="nowrap">  <input type="text" name="key" value="${key}"  title="输入姓名或者职号"/></td>    
		           ...
		           <td  class="info_title_01">申请类型
		          </td>
		         <td  class="info_content_00">
		        <select name="provideFlag">
		        <option value="">请选择</option>
				<option value="Local">本社员工</option>
				<option value="external">外来人员</option>		
				</select>
		         </td>        
		        </tr>      
		        </table>
		...
		<table width="100%"  border="1" cellspacing="0" cellpadding="0" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF">
		    ...
		    <c:forEach items="${TempCardConfirmList}" var="test" varStatus="i">		
		    <tr align="center">
		      ...
		      <td nowrap="nowrap" align="center" class="info_content_01" >${test.USER_NAME} &nbsp;</td>
		      <td nowrap="nowrap" align="center" class="info_content_01" >${test.USER_BUSINESS}&nbsp;</td>
		      ...
		      <td nowrap="nowrap" align="center" class="info_content_01" >${test.permissionsTypeName}&nbsp;</td>
		      ...
		      <td nowrap="nowrap"  align="center" class="info_content_06"><textarea name="confirmorIdea_${i.index}" style=" height: 40px;width:100px " type="_moz" onfocus="this.style.height='50px'" onblur="this.style.height='40px';">${test.CONFIRMOR_IDEA}</textarea></td>
		       <td nowrap="nowrap">    
		          <c:if test="${test.ISCONFIRM==2}"><font color="#00CC00">临时卡详情</font></c:if>    
		          <c:if test="${test.ISCONFIRM!=2}"><span style="color:red; cursor:pointer;" onclick="tempCardProvide(${test.APPLY_NO})">临时卡详情</span></c:if>   
		      </td>		  
		      </tr>
		   </c:forEach>	
		   <input type="hidden" name="currentPage" value="${currentPage}">	    
					...
					<ait:page       
		               ...
		               parameters="&operation=view&content=temporarycardapplicationview&menu_code=${param.menu_code}&startDate=${startDate}&endDate=${endDate}&qryType=${qryType}&deptid=${deptid}&key=${key}" 
		               total="${resultCount}"
