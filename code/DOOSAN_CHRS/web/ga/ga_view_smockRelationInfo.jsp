...
function Add() {
	...
	document.form1.action="/ga/ga_add_smockRelation.jsp?menu_code=${param.menu_code}";
...
function Delete() {
	...
	if(confirm('确定删除吗？')){
		document.form1.action="/gaControlServlet?operation=deleteSmockRelation&menu_code=${param.menu_code}";
		document.form1.submit();
	}
}
function Search() {
	...
	document.form1.action="/gaControlServlet?operation=retrieveSmockRelation&menu_code=${param.menu_code}";
	document.form1.submit();
}
...
{
    form1.action = "/gaControlServlet?operation=exportSmockRelationReport&menu_code=${param.menu_code}";
    form1.submit();    
}

function importExcel()
{
	url="/gaControlServlet?operation=importSmockRelationReport&menu_code=${param.menu_code}";
...
{
	form1.action = "/gaControlServlet?operation=downloadSmockRelationTemplate&menu_code=${param.menu_code}";
    form1.submit();
}
	...
	<tr>
		...
		<table width="100%" height="30" border="0" cellspacing="1"
			...
			<tr>
				...
				<table width="100%" height="30" border="0" cellpadding="0"
					...
					<tr>
						<td width="10%" class="info_title_01" nowrap="nowrap">开始日期</td>
						...
						<td width="10%" class="info_content_00">
							<ait:codeClass codeClass="smockInfo" name="smockName" selected="${smockName}" all="all"/>
						</td>
					...
					<tr>
						<td width="10%" class="info_title_01">职号\姓名</td>
						<td width="10%" class="info_content_00">
		...
		<table width="100%" border="1" cellspacing="0" cellpadding="2"
			...
			<c:forEach items="${recordList}" var="oneResult" varStatus="i">
				<tr align="center" onClick="band('#E7F0EF','black',${oneResult.SEQNO});">
					...
					<td align="center" style="color: #666666;">${oneResult.FOURTHIDEPTNAME}&nbsp;</td>
		...
		<ait:page link="/gaControlServlet"
			parameters="&operation=retrieveSmockRelation&menu_code=${param.menu_code}&startDate=${startDate}&endDate=${endDate}&smockName=${smockName}&personId=${personId}&DeptId=${DeptId}"
