...
function Search(){
	...
	document.form1.action="/gaControlServlet?operation=retrievePresentAffirmList&menu_code=${param.menu_code}";
...
function affirm(affirmFlag, index){
	
	$('SEQ_NO').value = $('applyNo_'+index).value;;
	$('affirmNo').value = $('affirmNo_'+index).value;;
	$('AFFIRM_FLAG').value = affirmFlag;
	$('AFFIRM_OPITION').value = $('opinion_'+index).value;
	$('AFFIRM_LEVEL').value = $('AFFIRM_LEVEL_'+index).value;
	$('op').value = "single";
	
	if (confirm("确定对当前记录进行决裁？")) {
		document.form1.action="/gaControlServlet?operation=affirmPresent&menu_code=${param.menu_code}";
		document.form1.submit();
	}
}
...
function doApproval(){
    ...
    if (confirm(msg5)) {
    	$('AFFIRM_FLAG').value = 1;
    	$('op').value = "batch";
		document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=affirmPresent";
		document.form1.submit();
	}
}
...
function ScenePhotos(presentId){
...
<input type="hidden" name="AFFIRM_LEVEL" value="" />
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
				...
				<table width="100%" height="30" border="0" cellpadding="0"
					...
					<tr>
						...
						<td width="8%" class="info_content_00"><ait:codeClass
							name="affirmType" codeClass="AffirmType" selected="${affirmType}"
							all="all" />
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
			<tr>
				<td align="left" class="title1" colspan="10">礼品决裁</td>
			</tr>
		</table>
		<table width="100%" border="1" cellpadding="0" cellspacing="0"
			...
			<tr>
				...
				<!--<td width="8%" class="info_title_01" nowrap>更新日期</td>-->
			...
			<c:forEach items="${recordList}" var="oneResult" varStatus="i">
				<tr align="center">
					...
					<input type="hidden" name="affirmNo_${i.index}" value="${oneResult.AFFIRM_NO}">
					<input type="hidden" name="AFFIRM_LEVEL_${i.index}" value="${oneResult.AFFIRM_LEVEL}" />
					<td class="info_content_01" height="27">&nbsp;
						<c:if test="${oneResult.AFFIRM_OP_FLAG == 0}">
					...
					<td class="info_content_01">
						...
						<c:forEach items="${oneResult.detailList}" var="detail" varStatus="j">
								<tr>
									...
									<td class="info_content_07"><span style="color:red; cursor:pointer;" onClick="ScenePhotos('${detail.PRESENT_ID}')" title="查看礼品照片">查看照片</span></td>
									<td class="info_content_07">数量：${detail.QUENTITY}${detail.UNIT_NAME}</td>
									<td class="info_content_07">单价：${detail.UNIT_PRICE}</td>
									<td class="info_content_07">价格：${detail.AMOUNT}</td>
				...
				<!-- <td class="info_content_01">
					...
					&nbsp;
						<c:if test="${oneResult.AFFIRM_OP_FLAG == 0}">
							<a href="#" onClick="affirm(1,'${i.index}')"><font color="red">通过</font></a>
							<a href="#" onClick="affirm(2,'${i.index}')"><font color="red">否决</font></a>
						</c:if>
					</td>
					<td class="info_content_01">
						<c:if test="${oneResult.AFFIRM_OP_FLAG == 0}">
		...
		<!-- Page Navigation Start--> <ait:page link="/gaControlServlet"
			parameters="&operation=retrievePresentApplyInfo&menu_code=${param.menu_code}&personId=${personId}&empId=${empId}&startDate=${startDate}&endDate=${endDate}&affirmType=${affirmType}&deptId=${deptId}"
