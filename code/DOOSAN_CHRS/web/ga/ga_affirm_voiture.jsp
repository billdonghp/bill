...
function search(){
	document.form1.action="/gaControlServlet?menu_code=ga0201&operation=gaAffirm&content=viewVoitureAffirm";
	document.form1.submit();
}
...
var msg1='<ait:message messageID="alert.ess.common.nodatatopass" module="ess"></ait:message>';
    ...
    if (confirm(msg5)) {
	document.form1.action = "/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&flag=1&content=visiterApprovalOKorNO";
	document.form1.submit();
	}
...
function doReject(){
	...
	if (confirm(msg6)) {
	document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&flag=2&content=visiterApprovalOKorNO";
	document.form1.submit();
	}
}
function visiterApprovalOKorNO(applyno,flag){
document.form1.action="/gaControlServlet?menu_code=${param.menu_code}&operation=gaAffirm&content=visiterApprovalOKorNO&flag="+flag+"&applyno="+applyno;
document.form1.submit();
}
</SCRIPT>
...
<!--		      <td nowrap="nowrap" class="info_title_01">
		    ...
		    <tr>
		    <c:forEach items="${voitureList}" var="list">
		    <tr align="center">	
		    <td nowrap="nowrap" align="center" class="info_content_01" >
		      	&nbsp;<c:if test="${ list.ACTIVE == 0 && list.AFFIRM_FLAG==0 }">
		      	 <input type="checkbox" name="applyno" value="${list.VOITURE_APPLYID}" />
		      	</c:if>&nbsp;
			  </td>		     
		      <td nowrap="nowrap" align="center" class="info_content_01" >
    			${list.CHINESENAME }
    			<input type="hidden" name="applerId_${list.VOITURE_APPLYID }" value="${list.APPLYER_ID }"> 
    			<input type="hidden" name="appler_Name_${list.VOITURE_APPLYID }" value="${list.CHINESENAME }">
    			<input type="hidden" name="VISITER_DATE_${list.VOITURE_APPLYID }" value="${list.APPLY_DATE }">
    			<input type="hidden" name="VISITER_COME_TIME_${list.VOITURE_APPLYID }" value="${list.APPLY_STARTTIME }">
    			<input type="hidden" name="APPLY_ENDDATE_${list.VOITURE_APPLYID }" value="${list.APPLY_ENDDATE }">
    			<input type="hidden" name="VISITER_END_TIME_${list.VOITURE_APPLYID }" value="${list.APPLY_ENDTIME }">
    			<input type="hidden" name="VISITER_PEOPLE_NUM_${list.VOITURE_APPLYID }" value="${list.APPLY_USERSCOUNT }">
    			</td>
   			<td nowrap="nowrap" align="center" class="info_content_01" >
   			 ...
   			 区分:<c:if test="${detail.DISTINCTION==1}">接</c:if>
   			 <c:if test="${detail.DISTINCTION==2}">送</c:if>
   			 &nbsp;&nbsp;使用时间:${detail.APPLY_DATE }&nbsp;${detail.APPLY_STARTTIME }
   			 &nbsp;&nbsp;使用时间:${detail.END_DATE }&nbsp;${detail.APPLY_ENDTIME }
   			 ...
   			 <td nowrap="nowrap" align="center" class="info_content_01" >
   			 <c:forEach items="${list.voitureAffiirmList}" var="view" varStatus="i">
		      	...
		      	<c:if test="${view.AFFIRM_FLAG==2}"><font color="#00CC00">已否决</font></c:if>
   			 	...
   			 	<c:if test="${view.AFFIRMOR_ID == adminId}">
   			 	<input type="hidden" name="affirmNo_${list.VOITURE_APPLYID}" value="${view.ID}">
   			 	<input type="hidden" name="MAX_AFFIRM_FLAG_${list.VOITURE_APPLYID}" value="${view.MAX_AFFIRM_FLAG}">
   			 	<input type="hidden" name="AFFIRM_LEVEL_${list.VOITURE_APPLYID}" value="${view.AFFIRM_LEVEL}">
   			 	<font color="#990099">${view.AFFIRM_LEVEL }${view.CHINESENAME }</font>
   			 		<c:if test="${view.AFFIRM_FLAG==0}"><font color="#990099">未决裁</font></c:if>
   			 		<c:if test="${view.AFFIRM_FLAG==1}"><font color="#00CC00">通过</font></c:if>
					<c:if test="${view.AFFIRM_FLAG==2}"><font color="#00CC00">已否决</font></c:if>
			      	...
			      	<c:if test="${list.ACTIVE==0 && view.AFFIRM_FLAG==0}">|
			      		<a href="#" onclick="visiterApprovalOKorNO('${list.VOITURE_APPLYID}','1')"><font color="red">通过</font></a>
			      	</c:if>
			      	<c:if test="${list.ACTIVE==0 && view.AFFIRM_FLAG==0}">
<!--			      		<a href="&affirmNo_${list.VOITURE_APPLYID}=${view.ID}&MAX_AFFIRM_FLAG_${list.VOITURE_APPLYID}=${view.MAX_AFFIRM_FLAG}&applyno=${list.VOITURE_APPLYID}&content=visiterApprovalOKorNO&affirmorIdea_=${list.VOITURE_APPLYID}">-->
			      			<a href="#" onclick="visiterApprovalOKorNO('${list.VOITURE_APPLYID}','2')"><font color="red">否决</font></a>
			      	</c:if>
   			 	</c:if>
   			 	<br />
   			 </c:forEach>
   			 <td nowrap="nowrap"  align="center" class="info_content_06"><textarea name="affirmorIdea_${list.VOITURE_APPLYID}" style=" height: 40px;width:100px " type="_moz" onfocus="this.style.height='50px'" onblur="this.style.height='40px';">${list.AFFIRMORIDEA}</textarea></td>
<!--		    <td nowrap="nowrap" class="info_content_01" width='200' style='word-break:break-all' align="left">
	      		<c:if test="${list.MODIFY_FLAG eq '1'}">
	      		车辆名称:${list.VOITURE_BRAND }<br />
	      		车牌号码:${list.VOITURE_NUMBER} <br />
	      		司机:${list.carStr }<br />
	      		<a href="#" onclick="Expel('${list.VOITURE_APPLYID}')" style="color:red">打印派车单</a>
	      		</c:if>
	      		<c:if test="${list.MODIFY_FLAG ne '1'}">&nbsp;</c:if>
	...
	<ait:page       
             ...
             parameters="&operation=gaAffirm&content=viewVoitureAffirm&menu_code=${param.menu_code}&qryType=${qryType }&deptid=${deptid }&endDate=${endDate }&startDate=${startDate }&key=${key }"
             total="${voitureInt}"
