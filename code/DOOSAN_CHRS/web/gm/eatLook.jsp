	...
	<tr>
	    ...
	    <tr>
	      ...
	      <tr>
	          ...
	          <input type="text" name="listDDS" value="${YestedateDate}" onClick="setday(this);" readonly style="width:70px">
	          <input type="hidden" name="hhS" value="${listHHS}" style="width:30px"/>
	          <input type="hidden" name="mmS" value="${listMMS}" style="width:30px"/>
	          <select name="listHHS" style="width:50px">
	          <option value="">请选择时</option>
	          	<c:forEach items="${listHH}" var="lhh" varStatus="i"> 
	          		<option value="${lhh}"  <c:if test="${lhh == '08'}">selected</c:if>>${lhh}</option>
	          	</c:forEach>
	          </select>
	          :
	          ...
	          <option value="">选择分钟</option>
	          	<c:forEach items="${listMM}" var="lmm" varStatus="i">
	          		<option value="${lmm}" <c:if test="${lmm == '00'}">selected</c:if>>${lmm}</option>
	          	</c:forEach>
	          </select>
	          </td>
	          ...
	          <td nowrap="nowrap" class="info_content_00">
	          <input type="text" name="listDDE" value="${Todaydate}" onClick="setday(this);" readonly style="width:70px">
	          ...
	          <input type="hidden" name="mmE" value="${listMME}"/>
	          	...
	          	<option value="">请选择时</option>
	          		<c:forEach items="${listHH}" var="lhhe" varStatus="i"> 
	          			<option value="${lhhe}" <c:if test="${lhhe == '07'}">selected</c:if>>${lhhe}</option>
	          		</c:forEach>
	          	</select>
	          	:
	          	...
	          	<option value="">选择分钟</option>
	          		<c:forEach items="${listMM}" var="lmme" varStatus="i">
	          			<option value="${lmme}" <c:if test="${lmme == '59'}">selected</c:if>>${lmme}</option>
	          		</c:forEach>
	          	</select>
	          </td>
	          ...
	          <td nowrap="nowrap" class="info_content_00">
	         	 ...
	         	 <option value="">请选择</option>
		         	 <c:forEach items="${gmType}" var="oneResult" varStatus="i">
		          		...
		          		<option value="${oneResult.GM_TYPE}">${oneResult.GM_TYPE}</option>
	          		</c:forEach>
	          	</select>
	          </td>
	          ...
	          <td nowrap="nowrap" class="info_content_00">
	          	...
	          	<option value="">请选择就餐卡类别</option>
		         	 <c:forEach items="${cardType}" var="oneResult" varStatus="i">
		          		...
		          		<option value="${oneResult.TYPE_NAME}">${oneResult.TYPE_NAME}</option>
	          		</c:forEach>
	          	</select>
			  </td>
