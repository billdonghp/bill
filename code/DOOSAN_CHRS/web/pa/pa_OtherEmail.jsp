...
function Search(){
	document.form1.action="/paControlServlet?operation=viewOtherEmailcmd&menu_code=${param.menu_code}&paMonth=" + document.form1.year.value + document.form1.month.value ;
	document.form1.submit();
}

function sendMailAll(){//按条件发送
	document.form1.action="/paControlServlet?operation=paOtherEmaiSendCmd&menu_code=${param.menu_code}&paMonth=" + document.form1.year.value + document.form1.month.value ;
...
{
	url="/paControlServlet?operation=importOtherEmailReport";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td background="../images/tablbk01_r1_c2.gif">
			...
			<!-- display toolbar -->
		<%@ include file="../inc/toolbar_send_otherMail.jsp"%>
		</td>
	...
	<tr>
		      	 ...
		      	 <tr>
		      		...
		      		<td class="info_content_00" nowrap="nowrap">
			        	<select name="tableName">
			        		<option value="JJ" <c:if test="${tableName == 'JJ'}">selected</c:if> >奖金</option>
			        		<option value="FLJ" <c:if test="${tableName == 'FLJ'}">selected</c:if>>福利金</option>
			        	</select>	        
			        </td>
	        		...
	        		<td class="info_content_00"><a target="_blank" href="/reports/template/send_othermail.xls"><ait:image src="/images/btn/Template.gif"  border="0" align="absmiddle" /></a></td>
		        </tr>
	      		<tr>
	        		...
	        		<td class="info_content_00" nowrap="nowrap">
	        			<select name="activity">
							...
							<option value="Y" <c:if test="${activity == 'Y'}">selected</c:if> >是</option>
							<option value="N" <c:if test="${activity == 'N'}">selected</c:if> >否</option>
						</select>
	        		</td>
					<td class="info_content_00"><ait:image src="/images/btn/Excel_Imp.gif"  border="0" align="absmiddle" onclick="javascript:importExcel();" style="cursor:hand"/></td>				 
				 </tr>
			</table>
	      </td>
		</tr>
		...
		<!-- Page Navigation Start-->
					<ait:page       
		               ...
		               parameters="operation=viewOtherEmailcmd&menu_code=${param.menu_code}&paMonth=${paMonth}&key=${key}&year=${year}&month=${month}&tableName=${tableName}&bonusTypeCode=${bonusTypeCode}&bonusNo=${bonusNo}&statTypeCode=${statTypeCode}&activity=${activity}&deptId=${deptId}&supplyFlag=${supplyFlag}" 
		               total="${resultCount}"
