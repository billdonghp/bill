<%@ page contentType="text/html; charset=UTF-8" language="java" import="java.util.*,com.ait.pa.PaDistinct"%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form method="post" name="sf" action="/paControlServlet?operation=paBonusParamItemConfigureList&method=update&menu_code=${param.menu_code}" onSubmit="return CheckForm()">
		<input type="hidden" name="CONFIGURE_NO" value="${paParamItimConfigure.CONFIGURE_NO}">
		<tr>
		<td width="150" height="26" class="info_title_01"><!--法人区分-->法人区分</td>
		<td colspan="3" style="padding-left:40px">${paParamItimConfigure.CPNY_NAME}
			<input type="hidden" name="CPNY_ID" value="${paParamItimConfigure.CPNY_ID}">
		</td>
		</tr>
		<tr>
		<td width="150" height="26" class="info_title_01"><!--项目名称-->项目名称</td>
		<td colspan="3" style="padding-left:40px">
			<input type="hidden" name="PARAM_NO" value="${paParamItimConfigure.PARAM_NO}">
			${paParamItimConfigure.PARAM_ID} / ${paParamItimConfigure.PARAM_NAME}
		</td>
		</tr>
		<tr>
		<td width="150" height="26" class="info_title_01"><!--员工区分类型-->员工区分类型</td>
		<td colspan="3" style="padding-left:40px">
			<ait:empDiff name="STAT_TYPE_CODE"  cpnyDiff="${cpnyDiff}" selected="${paParamItimConfigure.STAT_TYPE_CODE}"/>
		</td>
		</tr>
		<tr>
		...
		<td colspan="3" style="padding-left:40px">
			<ait:codeClass name="GENERATE_TYPE_CODE" codeClass="ParamTypeID" selected="${paParamItimConfigure.GENERATE_TYPE_CODE}" />
		</td>
		</tr>
		<tr>
			...
			<select name="DISTINCT_FIELD">
				...
				<c:forEach items="${padistinctv}" var="padistinctv">					
					<option value="${padistinctv.distinct_field}" 
						<c:if test="${padistinctv.distinct_field eq  paParamItimConfigure.DISTINCT_FIELD}">selected</c:if>
					>
		...
		<td colspan="3" style="padding-left:40px">${paParamItimConfigure.PA_MONTH_STR}<input name="PA_MONTH_STR" type="hidden" value="${paParamItimConfigure.PA_MONTH_STR}"></td>
		</tr>
		<tr>
		<td width="150" height="26" class="info_title_01"><!--数据类型-->
					<ait:message  messageID="display.mutual.data_type"  /></td>
		<td colspan="3" style="padding-left:40px">
			<ait:codeClass name="DATA_TYPE" codeClass="DataType" selected="${paParamItimConfigure.DATA_TYPE}" onChange="javascript:if(this.selectedIndex==1){ document.sf.DEFAULT_VAL.value='';}else{document.sf.DEFAULT_VAL.value='0'}"/>
		</td>
		</tr>
		<tr>
		...
		<td colspan="3" style="padding-left:40px"><input name="DEFAULT_VAL" size="25" value="${paParamItimConfigure.DEFAULT_VAL}"></td>
		</tr>
	</form>
	</table>
