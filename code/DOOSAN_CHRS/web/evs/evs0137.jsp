              ...
              if(status=='2'){
               ...
               return;
            	  ...
            	  if( confirm("提交后,相关评价成绩将交由相关领导审批 !\n\n      确定要提交吗?") ) {
                      document.form1.action="/evsControlServlet?operation=evssubmitgjjs&evscodeid="+ID+"&status="+status+"&menu_code=<%=request.getParameter("menu_code")%>";
...
{
   	// alert(ID)
   //	 var  ramark = document.LGFORM.code_name.value;
       document.form1.action="/evsControlServlet?operation=evsGjjsdjupdateR&menu_code=<%=request.getParameter("menu_code")%>";
  	document.form1.submit();
...
<table width="100%" border="0"   cellpadding="0"
	...
	<tr>
		...
		<td background="../images/tablbk01_r1_c2.gif">
			<!-- display toolbar -->
 		<%@ include file="../evs/inc/evs0137toolbar_v.jsp"%>  
		</td>
	            ...
	            List levStypecodeid1 = evsGxjsdj.getEvsGxjsDJRemark(code_name,code_name1);
				  ...
				  if(levStypecodeid1!=null){ 
				   ...
				   String statuss="";
	                if(code_name!=null)
	                {
					  List levStypecodeid = evsGxjsdj.getEvsGxjsDJCode(code_name,code_name1);
					  ...
					  if(levStypecodeid!=null){ 
					     for(int i=0;i<levStypecodeid.size();i++){
					    	 ...
					    	 if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("2"))
			                     checkState =  "disabled";
			                else if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("3"))
			                     checkState =  "disabled";
			                if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("1"))
			                	statuss =  "已保存";
			                else if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("2"))
			                	statuss =  "已提交";
			                else if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("3"))
			                	statuss =  "已决裁";
			                else if(evsGxjndj0.getStatus()!=null&&evsGxjndj0.getStatus().equals("4"))
			                	statuss =  "已否决";
	                
				 %>
						<tr	onClick="HighLightTR('#F0F1F4','black','<%=evsGxjndj0.getCodeNo()%>','<%=evsGxjndj0.getStatus()%>','<%=menu_code%>')">
							<td class="info_content_09" >
							 	<input type="checkbox" name="affirmNo_${num}" value="<%=evsGxjndj0.getCodeNo()%>" <%=checkState%> class="check">
							 	<input type="hidden" name="setupcodeNo_${num}" value="<%=evsGxjndj0.getCodeNo()%>" >
							...
							<td>
							<div align="center" height="30" ><%=statuss%>&nbsp;
							<input name="status" type="hidden" value="<%=evsGxjndj0.getStatus()%>"/>&nbsp;</div>							
							</td>
						</tr>
						<%}}}%>
			<tr>
			...
			<tr>
				...
				<td align="right" valign="middle" style="padding:3 0 3 0 ">								
					<img src="/images/btn/Modify.gif"
					 border="0" align="absmiddle" onclick="javascript:UpdateR();" style="cursor:hand;">				
					</td>	
			</tr>	
			<tr>
			
				<td colspan="13"><textarea name="gjjsRemark"
