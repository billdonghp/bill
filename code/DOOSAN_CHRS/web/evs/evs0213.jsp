...
function Return(){
	...
	if(CheckForm()){
		document.save.flag.value="return";
...
String evProcessId=Constants.EVPROCESS009;
List lEvsDept=null;
...
try{
	lEvsMaster=evsMaster.getEvsMasterByEvsProcess(Constants.EVPROCESS015,StringUtil.checkNull(map.getString("PERIODID")));
...
%>
<%@ include file="inc/evstoolbar_0213.jsp"%>
<%@ include file="inc/evs_nav.jsp"%>

<table width="99%"  border="0" align="center" cellpadding="0" cellspacing="1">
<form action="/evs/evs0213.jsp?menu_code=evs0213" method="Post" name="searchForm">
...
<input type="hidden" name="PROCESSID" value="<%=evProcessId%>" >
	...
	<tr>
		...
		&nbsp;
		<select name="PROCESSMASTERID" onChange="searchForm.submit();">
			<%
             if(lEvsMaster!=null){
                 	int lEvsMasterSize=lEvsMaster.size();
                 	for(int i=0;i<lEvsMasterSize;i++){
                 		EvsMaster em=(EvsMaster)lEvsMaster.get(i);
                              %>
			<option value="<%=em.getEvMaster()%>">
				<%=em.getEvMasterName()%>
			</option>
			<%}}%>
		</select>
...
<form action="/evs/evs0213_t.jsp?menu_code=evs0213" method="Post" name="save" >     
...
<input type="hidden" name="PROCESSMASTERID_CON" value="<%=map.getString("PROCESSMASTERID")%>" >
...
<input type="hidden" name="POSITION_CON" value="<%=map.getString("POSITION")%>" >
         ...
         <tr align="center" bgcolor="#F5F5F5">
          ...
          if(null!=lPreEvsProcess){
	           for(int i=0,j=lPreEvsProcess.size();i<j;i++){
	        	...
	        	for(int i=0,j=lPreEvsProcess.size();i<j;i++){
	        		EvsProcess ep=lPreEvsProcess.get(i);
	        		if(!StringUtil.checkNull(ep.getEvProcessID()).equals(Constants.EVPROCESS015)){
		        		out.print("<td bgcolor=\"#F5F5F5\">");
        ...
        <tr align="center">
             ...
             --%><%
	           ...
	           for(int k=0,m=lPreMaster.size();k<m;k++){
	        	...
	        	if(null!=lPreEvsProcess&&null!=lPreMaster){
	        	 ...
	        	 for(int k=0,m=lPreMaster.size();k<m;k++){
	        		EvsMaster em=lPreMaster.get(k);
	        		if(!em.getEvProcessID().equals(Constants.EVPROCESS015)){
		        		out.print("<td >");
             ...
             <td ><%=e.getEvMark()%>&nbsp;</td>
             <td ><%=e.getEvOrder()%>&nbsp;</td>
             <td ><%=e.getEvDefautlGradeName()%>&nbsp;</td>
             <td >
             
             <%if(!StringUtil.checkNull(e.getEvDefaultGrade()).equals(StringUtil.checkNull(e.getEvGradeID()))){
             	out.print("<font color='red'>");
             	out.print(e.getEvGradeName());
             	out.print("</font>");
             }else{
             	out.print(e.getEvGradeName());
             }%>&nbsp;
