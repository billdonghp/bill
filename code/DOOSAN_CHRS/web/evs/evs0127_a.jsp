...
{
  var str="";
  if(document.LGFORM.evscodeid.value=="")
  {//请输入基本代码名称
  	alert('请输入基本代码名称');
	return;
  }
  if(document.LGFORM.evscodename.value=="")
  {//"请输入 Line的名称"
  	alert('请输入 Line的名称');
	return;
  }
  document.LGFORM.action="/evsControlServlet?operation=evsircraftsave&menu_code=<%=request.getParameter("menu_code")%>";
       ...
       <input type="hidden" name="menu_code" value="<%=menu_code%>">
		  ...
		  <table width="100%" border="1"cellspacing="0" cellpadding="10" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
		    <tr>
		      ...
		      <td width="75%" class="info_content_00"><input name=evscodeid type="text" size="15" ></td>
		    </tr>
		    <tr>
		      <td width="15%" class="info_title_01"><!-- Line-->
					  机种名称</td>
		      <td width="75%" class="info_content_00"><input name=evscodename type="text" size="20" ></td>
		    </tr>
		    <tr>
		      <td width="15%" class="info_title_01"><!-- Line-->
					 机种英文名称</td>
		      <td width="75%" class="info_content_00"><input name=evsencodename type="text" size="20" ></td>
		    </tr>
		    <tr>
		      <td class="info_title_01"><!-- 工序-->
		        ...
		        <td class="info_content_00"> 
		            ...
		            <c:forEach items="${searchExpiredIrcraft}" var="EVSCT1"
					    varStatus="s">
					    <c:choose>
			              			<c:when test="${s.count % 5 == 0}">
							 <input
								type="checkbox" name="isChecked" value="${EVSCT1.CODE_ID}"
								class="check"  /> 
							 ${EVSCT1.CODE_NAME} 
							 <c:if test="${s.count != searchExpiredIrcraftSize}">
					              		<br/>
					              		</c:if>
							 </c:when>
							 <c:otherwise>
