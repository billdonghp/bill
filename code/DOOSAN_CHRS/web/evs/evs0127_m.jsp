...
function Save()
{
  document.LGFORM.action="/evsControlServlet?operation=evsircraftsave2&menu_code=<%=request.getParameter("menu_code")%>";
  document.LGFORM.submit();
	...
	<body>
		...
		<%String evscodeid = "";
			evscodeid = request.getParameter("evscodeis") != null ? request
					.getParameter("evscodeid") : "";
		    ...
		    <tr>
		      <td class="info_title_01"><!-- 工序-->
		        ...
		        <td class="info_content_00"> 
		             ...
		             <c:forEach items="${search2Line2}" var="vList" varStatus="i">
			              
			              		<c:choose>
			              			<c:when test="${i.count % 5 == 0}">
					              		 
					              		<input name="isChecked" id="isChecked_${vList.CODE_ID}" value="${vList.CODE_ID}" type="checkbox" style="border:0px"
											<c:forEach items="${search3Line3}" var="search3Line3">
												<c:if test="${search3Line3.PARENTID eq vList.CODE_ID}"> 
													checked="checked"
												</c:if> 
											</c:forEach> 
										/>
					              		...
					              		<c:if test="${i.count != search2Line2Size}">
