...
<form action="/evs/evs0205_t.jsp" method="POST" name="Evs0205">
  ...
  <c:forEach items="${itemList}" var="oneResult" varStatus="i" >
		  ...
		  <tr >
		    ...
		    <!-- 担当业务、培训情况和综合意见不显示比重 -->
		    <c:if test="${itemVector[i.index].ItemID != 'EVITEM001' and itemVector[i.index].ItemID != 'EVITEM004' and itemVector[i.index].ItemID != 'EVITEM006'}">
		    	 <td align="center" width="30" bgcolor="#F5F5F5" bgcolor="#F5F5F5" nowrap>比重%</td>
		    </c:if>
		   
		    <!-- 不显示内容输入(“详细输入”操作、培训、综合意见除外) -->
		    <c:if test="${operate == 'EVOPERATE010' or 
		    		      (itemVector[i.index].ItemID == 'EVITEM004' and operate == 'EVOPERATE009') or 
		    		      (itemVector[i.index].ItemID == 'EVITEM006' and operate == 'EVOPERATE009') or
		    		      (itemVector[i.index].ItemID == 'EVITEM004' and operate == 'EVOPERATE006') or 
				...
				<tr >
				    ...
				    <c:forEach items="${detailMap.detailContent}" var="oneContent" varStatus="m">
				    	...
				    	<c:if test="${m.count != fn:length(detailMap.detailContent) 
				    			and (itemVector[i.index].ItemID == 'EVITEM001' or itemVector[i.index].ItemID == 'EVITEM004' or itemVector[i.index].ItemID == 'EVITEM006')}">
				    		<!-- 细则项目名称不转行 -->
					    	<td align="left" 
					    	<c:if test="${operate == 'EVOPERATE010'}">
					    		style="width: 120px; word-wrap: break-word"
