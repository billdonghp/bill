...
function Modify()
{
    ...
    {
            url='/evsControlServlet?operation=evstypeopaupdate&menu_code='+MENU_CODE+'&ID='+ID+'&ID2='+ID2;
            location.href=url;
    }
}
function Delete()
{
    ...
    else
    {
            if( confirm("删除后,相关信息也将被清空!\n\n      确定要删除吗?") ) {
                    url='/evsControlServlet?operation=evstypeopadelete&menu_code='+MENU_CODE+'&ID='+ID+'&ID2='+ID2;
                    location.href=url;
            }
            else
            {
