...
function Search() {
	...
	document.ApplyForm.action="/essControlServlet?operation=view&content=attendanceconfirm&menu_code=ess0915";
    ...
    if (document.ApplyForm.details) {
		if (document.ApplyForm.details[0]) {
			...
			for (i=0;i<document.ApplyForm.details.length;i++){
			     ...
			     if( document.ApplyForm.details[i].checked == true){
			   
				    if(dept0 == null){
				      dept0 = document.getElementById("deptid" + document.ApplyForm.details[i].value).value;
				    }
			        ///alert(document.getElementById("deptid" + document.ApplyForm.details[i].value).value);
			       ///针对ic0600076决裁两个支社信息时，不能同时决裁合肥支社(EX)和武汉支社(EX)
			       if((document.getElementById("deptid" + document.ApplyForm.details[i].value).value) != dept0){
			         if(((document.getElementById("deptid" + document.ApplyForm.details[i].value).value) == "780S5A00")||((document.getElementById("deptid" + document.ApplyForm.details[i].value).value) == "780S5500")){
			          alert('不能同时确认多个部门考勤信息');
				      return false;
				      }
				   }
...
<input type="hidden" name="content" value="attendanceconfirm" />
<input type="hidden" name="menu_code" value="ess0915" />
		    ...
		    <tr align="center">
		      ...
		      <input name="night_<%=detail.getPkNo()%>" type="hidden" value="N">
		       ...
		       <td class="info_content_09" nowrap>
				<font color="<%=(String) colorMap.get(String.valueOf(detail.getArFlag()))%>">
					<%=(String) confirmMap.get(String.valueOf(detail.getArFlag()))%>
		    		<%if (detail.getArFlag() == 0) {%>
			...
			<c:if test="${fn:length(detailList) < 6}">
		      ...
		      <!-- Page Navigation Start-->
			        <ait:page       
			               ...
			               parameters="&operation=view&content=attendanceconfirm&menu_code=ess0915&sDate=${sDate}&eDate=${eDate}&key=${key}&deptid=${deptid}&itemNo=${itemNo}&personId=${personId}&qryType=${qryType}" 
			               total="${resultCount}"
