...
<div align="left">
<span class="title1">决裁信息&gt;漏卡申请决裁</span></div>
<form name="ApplyForm" action="/essControlServlet">
	<input type="hidden" name="operation" value="view" />
	<input type="hidden" name="content" value="cardaffirm" />
...
<%for(int i=0;i<applyList.size();i++){
    ...
    String remark = StringUtil.checkNull(essCardApplyBean.getRemark(),"");
    essAffirmorList = essCardApplyBean.getAffirmorList();
    int applyNo = Integer.parseInt(essCardApplyBean.getApplyID());
    int affirmNo = 0;
    int opFlag = essCardApplyBean.getOpFlag();
    if (opFlag >= 0) {
...
%>
  <tr align="center">
    <td align="center" nowrap><%if (essCardApplyBean.getOpFlag()>=0) {%><input type="checkbox" name="affirmNo" value="<%=affirmNo%>" <%=checkState%>>
