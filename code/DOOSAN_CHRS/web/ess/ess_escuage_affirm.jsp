...
<form name="familyForm" action="/EssContorlServlet">
	...
	<input type="hidden" name="content" value="InfoAffirmEscuageContentImp"/>
	<table>
		<tr>
...
%>
  <tr align="center">
    ...
    <td  height="30"  align="center"><%=(String)ht.get("MILITARY_TYPE")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)ht.get("MILITARY_LEVEL")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)ht.get("MILITARY_AREA")%>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)ht.get("START_DATE")!=null && (String)ht.get("START_DATE")!=""?((String)ht.get("START_DATE")).substring(0,((String)ht.get("START_DATE")).indexOf(" ")):"" %>&nbsp;</td>
    <td  height="30"  align="center"><%=(String)ht.get("END_DATE")!=null && (String)ht.get("END_DATE")!=""?((String)ht.get("END_DATE")).substring(0,((String)ht.get("END_DATE")).indexOf(" ")):""%>&nbsp;</td>
      <%
      int activity=Integer.parseInt((String)ht.get("ACTIVITY")); 
 %>
      <%if(activity==0){%>
      <td  height="30"  align="center"><a href="/EssContorlServlet?command=InfoAffirmCommandImp&content=InfoAffirmEscuageViewContentImp&militaryNO=<%=(String)ht.get("MILITARY_NO")%>&flag=1&menu_code=ess0504"><font color="#00CC00">通过</font></a><font color="#00CC00">&nbsp;/&nbsp;</font><a href="/EssContorlServlet?command=InfoAffirmCommandImp&content=InfoAffirmEscuageViewContentImp&militaryNO=<%=(String)ht.get("MILITARY_NO") %>&flag=2&menu_code=ess0504"><font color="#00CC00">不通过</font></a></td>
      <%}else if(activity==1){%>
...
%>  
 ...
 <table border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <td width="11%">
  <a href="/EssContorlServlet?command=InfoAffirmCommandImp&content=InfoAffirmEscuageContentImp&menu_code=ess0504&pageno=<%=(p.getPagenum()/10)*10-10+1%>"><img src="/images/btn/p_last101.gif" width="18" height="12" border="0" align="absmiddle"></a>
  </td>
  <%}%>
  <td>
 <%for(int i=(((p.getCurrent()-1)/10*10)+1);i<=(((p.getCurrent()-1)/10)+1)*10 && i<=p.getPagenum();i++){%>
	<a href="/EssContorlServlet?command=InfoAffirmCommandImp&content=InfoAffirmEscuageContentImp&menu_code=ess0504&pageno=<%=i%>"><font color="#666666" ><b><%=i%></b></font></a>&nbsp;
 <%}%>
 </td>
  <%if(p.getPagenum()/10!=0 && (((p.getCurrent()-1)/10*10)+1)+10<p.getPagenum()){%>
  <td width="11%">
  <a href="/EssContorlServlet?command=InfoAffirmCommandImp&content=InfoAffirmEscuageContentImp&menu_code=ess0504&pageno=<%=(p.getPagenum()/10)*10+1%>"><img src="/images/btn/p_next101.gif" width="18" height="12" border="0" align="absmiddle"></a>
  </td>
  <%}%>
  </tr>
</table>
