...
<jsp:useBean id="affirmSetList" class="java.util.ArrayList" scope="request" />
<jsp:useBean id="evaluateAffirm" class="com.ait.ev.bean.EvaluateAffirm" scope="page" />
<jsp:useBean id="evAffirmorList" class="java.util.ArrayList" scope="page" />
<jsp:useBean id="evAffirmor" class="com.ait.ev.bean.EvaluateAffirm" scope="page" />
...
{	
    ...
    document.form1.action="/evControlServlet?operation=evaluateSet&content=evaluateAffirmSetModify&menu_code=${param.menu_code}";
...
tableUtil.appendRow = function(){	
		...
		td = nTr.insertCell() ;
		td.className = "info_content_01" ;
		td.innerHTML = "<select name='level"+i+"'><option value='1'>1级</option><option value='2'>2级</option><option value='3'>3级</option><option value='4'>4级</option></select>";	     		
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<table id="operateTable" width="100%" border="1" cellspacing="0" cellpadding="5"
		    ...
		    %>
		    <input type="hidden" name="temp1" value="<%=evAffirmorList.size() %>">
			<tr>
			    
				<td width="10%" class="info_title_01">部门</td>
				<td width="10%" class="info_content_00"><%=evaluateAffirm.getDeptName() %>&nbsp; (<%=evaluateAffirm.getFourthDept() %>)
				<input type="hidden" name="DEPTID_SEQ" value="<%=evaluateAffirm.getDeptId()%>" />
				<input type="hidden" name="DeptId" value="<%=evaluateAffirm.getDeptId()%>" />
               	</td>
			</tr>
			<tr>
				<td width="10%" class="info_title_01">职位</td>
				<td class="info_content_00">
					<%=evaluateAffirm.getPositionName() %>
					 <input type="hidden" name="POSITION_SEQ" value="<%=evaluateAffirm.getPosition()%>" />
					 <input type="hidden" name="selectPosition" value="<%=evaluateAffirm.getPosition()%>" />
				</td>
		...
		<!-- 	<tr>
		 		...
		 		<td width="10%" class="info_title_01">决裁人</td>
				<td class="info_content_00">
					<input id="empID" name="empID" size="8" onKeyUp="SearchContent(this.value,this.id)" value="${result.EMPID}" />
			...
			<tr>
				...
				<td class="info_content_00">
					<select name="level">
						    ...
						    <option value="1" <c:if test="${result.AFFIRM_LEVEL == '1'}">selected</c:if>>1</option>
						    <option value="2" <c:if test="${result.AFFIRM_LEVEL == '2'}">selected</c:if>>2</option>
						    <option value="3" <c:if test="${result.AFFIRM_LEVEL == '3'}">selected</c:if>>3</option>
						    <option value="4" <c:if test="${result.AFFIRM_LEVEL == '4'}">selected</c:if>>4</option>
						</select>
				</td>
			</tr>
			-->
			 <%	for(int j=0;j<evAffirmorList.size();j++){            
				...
				<td class="info_content_01">
					<input id="empID<%=j%>" name="empID<%=j%>" size="8" onKeyUp="SearchContent(this.value,this.id,<%=j%>)" value="<%=evAffirmor.getAffirmorId() %>" />
				...
				<td class="info_content_01">
					<select name="level<%=j%>">
						    ...
						    <option value="1" <% if(evAffirmor.getLevel() == 1){ %> selected <%}%> >1级</option>
						    <option value="2" <% if(evAffirmor.getLevel() == 2){ %> selected <%}%> >2级</option>
						    <option value="3" <% if(evAffirmor.getLevel() == 3){ %> selected <%}%> >3级</option>
						    <option value="4" <% if(evAffirmor.getLevel() == 4){ %> selected <%}%> >4级</option>
						</select>
				</td>
