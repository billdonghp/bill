...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form action="hrmControlServlet" method="post" name="searchForm" onkeydown="if(event.keyCode==13) searchForm.submit();" >
		  ...
		  <input type="hidden" name="operation" value="searchHortation">
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d" style="word-break:break-all">
			<form name="save" action="hrmControlServlet" method="post">
			  <input type="hidden" name="operation" value="delHortation"/>
			  <input type="hidden" name="menu_code" value="${toolbarInfo.menu_code}" />
		  ...
		  <c:forEach items="${list}" var="oneResult" varStatus="i">
		      <tr>
		        <td class="info_content_01" width="5%">
		        	...
		        	<input type="hidden" name="rewardNo_${i.index}" value="${oneResult.rewardNo}"/>
		        	<input type="hidden" name="activity_${i.index}" value="${oneResult.activity}"/>
		        </td>
		        <td class="info_content_01" width="5%" nowrap>${oneResult.transNo}</td>
		        ...
		        <td class="info_content_01" width="10%" nowrap>${oneResult.rewardDate}</td>
		        ...
		        <td class="info_content_01" width="5%" nowrap>${oneResult.rewardAmount}</td>
		        <td class="info_content_01" width="10%">${oneResult.rewardContents}</td>
		        <td class="info_content_01" width="10%" nowrap>${oneResult.rewardBonus}</td>
		          <td class="info_content_01" nowrap="nowrap">
		    	<a href="/hrmControlServlet?operation=exportInfo&type=reward&empid=${oneResult.empID}" target="_blank"><!-- 查看内容 -->
		    	    excel
		    	</a>                 
		     </td>    
		        <!--  
		...
		<ait:page       
		         ...
		         parameters="&operation=searchHortation&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}&TRANSCODE=${TRANSCODE}&TRANSNO=${TRANSNO}&ACTIVITY=${ACTIVITY}" 
		         total="${resultCount}"
