...
<script language="javascript" src="../js/hr_modify_reward.js"></script>
<SCRIPT type="text/javascript">
function Save(){
	
	if(CheckForm()) {
	
		document.form1.action="hrmControlServlet?menu_code=hr0104&operation=updateReward&empID=<c:out value='${empID}'/>";
    ...
    <td>
        <table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d" >
         ...
         <c:forEach items="${rewardList}" var="oneResult" varStatus="i">
	        <tr>
	            <td class="info_content_01" height="30" nowrap><input type="text" size="10" class="content" name="rewardDate_${i.count}" id="rewardDate_${i.count}" value="${oneResult.rewardDate}" onClick="setday(this);" date mask="9999-99-99"/></td>
	            <td class="info_content_01" nowrap><ait:codeClass name="rewardTypeCode_${i.count}" codeClass="RewardTypeCode" all="ALL" selected="${oneResult.rewardTypeCode}" /></td>
	            <td class="info_content_01" nowrap><ait:codeClass name="rewardSortCode_${i.count}" codeClass="RewardSortCode" all="ALL" selected="${oneResult.rewardSortCode}"/></td>
	            <td class="info_content_01" nowrap><input type="text" class="content" name="rewardAmount_${i.count}" id="rewardAmount_${i.count}" value="${oneResult.rewardAmount}" /></td>
	            <td class="info_content_01" nowrap><ait:codeClass name="achievementTypeCode_${i.count}" codeClass="AchievementTypeCode" all="ALL" selected="${oneResult.achievementTypeCode}"/></td>
	            <td class="info_content_01" nowrap><input type="text" class="content" name="rewardContents_${i.count}" id="rewardContents_${i.count}" value="${oneResult.rewardContents}" /></td>
	            <td class="info_content_01" nowrap><input type="text" class="content" name="rewardBonus_${i.count}" id="rewardBonus_${i.count}" value="${oneResult.rewardBonus}" /></td>
	            <input type="hidden" name="updatedBy_${i.count}" value="${adminId}">
