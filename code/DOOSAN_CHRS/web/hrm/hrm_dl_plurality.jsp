...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<form action="hrmControlServlet" method="post" name="searchForm" onkeydown="if(event.keyCode==13) searchForm.submit();" >
		  ...
		  <input type="hidden" name="operation" value="viewPlurality">
					...
					<tr>
						...
						<td width="10%" class="info_title_01" nowrap><!-- 发令类型 -->
		          		...
		          		<td width="20%" class="info_content_00"><ait:codeClass  
							style="content" codeClass="TransPlurality" name="transCode"
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="1">
		  ...
		  <table width="100%"  border="0" cellpadding="0" cellspacing="0">
			  ...
			  <input type="hidden" name="operation" value="insertPlurality"/>
			  <input type="hidden" name="menu_code" value="${toolbarInfo.menu_code}"/>
		  	...
		  	<tr>
				  ...
				  <table width="100%" border="0" cellpadding="0" cellspacing="1" class="dr_d">
				    <tr align="center">
				      ...
				      <td class="info_title_01" nowrap="nowrap"><!--工号--><ait:message  messageID="display.mutual.emp_id"/></td>
				      <td class="info_title_01" nowrap="nowrap"><!-- 姓名 --><ait:message  messageID="display.mutual.name"/></td>
				      <!-- 去除兼职事由、增加部门、职位、岗位职务 -->
				      <td class="info_title_01" nowrap="nowrap"><!-- 部门 --><ait:message  messageID="display.mutual.dept"/></td>
				      <td class="info_title_01" nowrap="nowrap"><!-- 职位 --><ait:message  messageID="display.mutual.position"/></td>
				      <td class="info_title_01" nowrap="nowrap"><!-- 岗位职务 --><ait:message  messageID="display.mutual.post"/></td>
				      <td class="info_title_01" nowrap="nowrap"><!-- 人事令号 --><ait:message  messageID="display.mutual.regulation_no"/></td>
				      
				      
				      <!-- 
				      <td class="info_title_01">兼职事由</td>
				       -->
				      ...
				      <td class="info_title_01" nowrap="nowrap"><!-- 兼职内容 -->
					<ait:message  messageID="display.emp.staff_info.contents_of_concurrent_job" module="hrm"/>
				</td>
				    </tr>
				    
				    <c:choose>
						    <c:when test="${transCode eq 'TransPlurality02'}">
						    	<c:forEach items="${emp_list}" var="emp" varStatus="j">
									<tr>
										<td class="info_content_01" height="30">
											...
											<input type="hidden" name="pluralityNo_${j.index}" value="${emp.pluralityNo}"/>
											
										</td>
										<td class="info_content_01" nowrap="nowrap">${emp.empID}</td>
										<td class="info_content_01" nowrap>
							<ait:content enContent='${emp.pinyin}' zhContent='${emp.chineseName}' koContent='${emp.korName}'/>
							</td>
										<td class="info_content_01" nowrap="nowrap">
							<ait:content enContent='${emp.enDept}' zhContent='${emp.department}' koContent='${emp.korDept}'/>
							</td>
										<td class="info_content_01" nowrap="nowrap">
							<ait:content enContent='${emp.enPosition}' zhContent='${emp.position}' koContent='${emp.korPosition}'/>
							</td>
										<td class="info_content_01" nowrap="nowrap">
							<ait:content enContent='${emp.enPost}' zhContent='${emp.post}' koContent='${emp.korPost}'/>
							</td>
										<td class="info_content_01">${emp.transNo}</td>
										<!--
										<td class="info_content_01">${emp.reason}</td>
										 -->
										<td class="info_content_01" nowrap="nowrap">${emp.startDate}</td>
										<td class="info_content_01" nowrap="nowrap">
										...
										<td class="info_content_01" nowrap="nowrap">${emp.contents}</td>
									</tr>
							    </c:forEach>
							...
							<ait:content enContent='${emp.englishPost}' zhContent='${emp.post}' koContent='${emp.korPost}'/>
									  ...
									  <!--
								      <td class="info_content_01">
								      	<ait:codeClass style="content" codeClass="PluralityReason" name="reasonCode_${j.index}" onChange="setCheckboxChecked('selectedTag',${j.index});selMuli('reasonCode_',this.id)"/>
								      </td>
								        -->
								      <td class="info_content_01">
								      ...
								      <td class="info_content_01">
								      	<input style="content" type="text" size="20" id="contents_${j.index}" name="contents_${j.index}" 
											      		onChange="setCheckboxChecked('selectedTag',${j.index});;textMuli('contents_',this.id);" maxlength="200"/>
								      </td>
								    </tr>
							    </c:forEach>
							    <c:if test="${fn:length(emp_list) < 6}">
	...
	<ait:page       
         ...
         parameters="&operation=viewPlurality&menu_code=${toolbarInfo.menu_code}&DEPTID=${DEPTID}&CONDITION=${CONDITION}&transCode=${transCode }" 
         total="${resultCount}"
