<%@ page contentType="text/html; charset=UTF-8"  import="java.util.*,com.ait.sqlmap.util.*,com.ait.util.StringUtil,com.ait.hrm.bean.Department"%>
...
<body>
 <%
		...
		response.setHeader("Content-Disposition", "attachment; filename=dailyDetailStatisticsReport.xls");
...
<table border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr align="center" height="30">
    <td >             
	    <table width="100%">  
	    	<tr>
	    		<td align="center" colspan="13"><b><font size="+2"><!-- 部门考勤统计表 --><%= parameterObject.getString("dateStarted") %> <ait:message messageID="display.att.view.report.detail" module="ar"/></font></b></td>
	    	</tr>
	    </table>    	
    </td>
  </tr>
	   ...
	   <%
			String date = parameterObject.getString("dateStarted") ;
	   		SimpleMap dataMap = new SimpleMap() ;
   		   	List oneList = reportDAO.getDept_PayDaily_head(date,"Z000000") ;//根据顶级部门ID，取出一级部门．
           	for (int i = 0 ; i < oneList.size() ; i ++)        
           	{
           		...
           		for (int j = 0 ; j < twoList.size() ; j ++)
           		{
           			...
           			List threeList = reportDAO.getDept_PayDaily(date,twoDept.getDeptID(),twoDept.getDeptLevel()) ;//根据一级部门ID，取出二级部门
           			
   					for (int k = 0 ; k < threeList.size() ; k ++)
   					{
   						Department threeDept = (Department)threeList.get(k) ;    
   						
   						dataMap = reportDAO.getData_arDailyDetailStatistics(date,threeDept.getDeptID(),threeDept.getDeptLevel()) ;//三级部门的数据
   						if( j == 0 && k == 0) //判断是否，为一级部门的第一行
   						{
   		%>
	   		  <tr align="center">
	      			<td rowspan="<%= reportDAO.getDeptRowspan_paDaily(date,oneDept.getDeptID(),oneDept.getDeptLevel()) %>"  align="center">
	   				...
	   				<td rowspan="<%= reportDAO.getDeptRowspan_paDaily(date,twoDept.getDeptID(),twoDept.getDeptLevel()) %>"  align="center">
	   		...
	   		<tr align="center">	
	   			<%			
	   						dataMap = reportDAO.getData_arDailyDetailStatistics(date,twoDept.getDeptID(),4); //二级部门的数据
	   			%>
	      			 <td ><!--                科小计 -->
	   			...
	   			<%
	   	        		...
	   	        		dataMap = reportDAO.getData_arDailyDetailStatistics(date,oneDept.getDeptID(),4); //一级部门的数据
	   		    %>
	           <tr align="center">
	           ...
	           <%		
	   	        	...
	   	        	dataMap = reportDAO.getData_arDailyDetailStatistics(date,"Z000000",4)  ;
	   		%>
	   		<tr align="center">       
