...
String sql_admin= " AND (DATE_LEFT IS NULL OR DATE_LEFT BETWEEN TO_DATE('" + from + "','YYYY-MM-DD') AND TO_DATE('" + to + "','YYYY-MM-DD') ) " 
				+ " AND EXISTS (SELECT S.SUPERVISED_DEPTID FROM AR_SUPERVISOR_INFO S WHERE H.DEPTID = S.SUPERVISED_DEPTID AND S.AR_SUPERVISOR_ID = '" + admin.getAdminID() + "') ";
                
String sql_statement="";

	if(!"".equals(deptid)){
		sql_statement="and EXISTS(" +
							"SELECT deptid " +
							"FROM hr_department WHERE HR_DEPARTMENT.DEPTID = H.DEPTID " +
...
while(fromDate.compareTo(toDate)<=0){
	...
	sqlField.append(" ,AR_GET_SHIFTNO(person_id,'"+format1.format(fromDate)+"','" + admin.getCpnyId() + "') AS \""+format2.format(fromDate)+ "\"");
	ca.setTime(fromDate);   
    ca.add(Calendar.DATE,1);
    fromDate=(java.util.Date)ca.getTime();
    cols++;
}
sql=sql_name+sqlField.toString()+"  FROM HR_EMP_V H,SY_CODE SY  WHERE h.Cpny_Id='"+ adminBean.getCpnyId() +"' and H.POST_CODE = SY.CODE_ID(+) "+sqlCon.toString()+sql_admin+sql_statement+" order by h.deptid DESC,H.POSITION_CODE,h.person_id ";
}
