...
<%
 String cpnyId=request.getParameter("cpnyid");
	...
	//Vector vlist = Biz.getDept(cpnyId);
	Vector vlist = Biz.getCompanyId();
...
%>
        <tr>
          ...
          <td nowrap  class="left_menu"><%=Ent.getDeptName()%></td>
          <td nowrap valign="middle" class="left_menu">
            <input type="hidden" name="parentDptId" value="<%=Ent.getDeptId()%>">
            ...
            <input type="checkbox" name="dp" value="<%=Ent.getDeptId()%>" <%=checkFlag%>></td>
        </tr>
        <!-- language.equals("en") -->
       <%}%>
      </table></td>
