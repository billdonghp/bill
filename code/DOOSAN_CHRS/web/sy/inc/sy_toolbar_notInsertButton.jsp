...
<%
	...
	session.setAttribute("toolbarInfo", map);
         ...
         <td align="left" valign="middle" style="padding:3 0 3 0 ">
          ...
          <c:if test="${toolbarInfo.updateMenu == 1}">
            <ait:image src="/images/btn/Modify.gif"  border="0" align="absmiddle"
          	onclick="javascript:Update(msg);" style="cursor:hand" title="修改" enTitle="update" />
          </c:if>
          <c:if test="${toolbarInfo.deleteMenu == 1}">
          <ait:image src="/images/btn/Delete.gif"  border="0" align="absmiddle"
          	onclick="javascript:Delete(msg);" style="cursor:hand" title="删除" enTitle="delete" />
          </c:if>
          </td>
        </tr>
      </table>
