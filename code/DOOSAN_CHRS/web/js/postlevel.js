...
function Add() {
     url = "/sy/postgradeleveladd.jsp?menu_code=" + MENU_CODE;         
...
function Update(msg) {   
    ...
    {
        url = "/syControlServlet?operation=retrievedataforupdatepostlevelcmd&postlevelid=" + ID + "&menu_code=" + MENU_CODE;
        location.href = url;       
    }
}
function Active() {
...
function Delete(msg) {
    if (ID == "") {
        ...
        if (confirm(msg[2])) {
            url = "/syControlServlet?operation=deletepostlevelcmd&postlevelid="+ ID + "&menu_code=" + MENU_CODE;
            location.href = url;  
        } else {   
            return;
        }
