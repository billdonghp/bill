...
function Save()
{
  ...
  {//"请正确输入2位分钟时间"
	...
	document.form1.action="/arControlServlet?operation=updateEateryRecord&menu_code=${param.menu_code}";
		...
		<form name="form1" method="post" action="">
		  ...
		  <table width="100%" border="1"cellspacing="0" cellpadding="5" bordercolorlight="#E7E7E7" bordercolordark="#FFFFFF" style="padding: 2px 2px 2px 2px;">
		    <tr>
		      ...
		      <td width="75%" class="info_content_00">${result.CARD_NO}&nbsp;(&nbsp;<ait:content enContent="${result.CHINESE_PINYIN}" zhContent="${result.CHINESENAME}" koContent="${result.KOREANNAME}"/>&nbsp;)</td>
		      <input type="hidden" name="key" value="${result.CARD_NO}" />
		    </tr>
		    <tr>
		      <td class="info_title_01"><!--就餐时间-->
		    ...
		    <tr>
		      <td class="info_title_01"><!--就餐类型-->
					<ait:message  messageID="display.att.view.dining.dining_type" module="ar"/></td>
		      <td class="info_content_00"><ait:codeClass name="REPAST_TYPE" codeClass="RepastType" selected="${result.REPAST_TYPE }"/></td>
		    </tr>
		    <tr>
