...
ArShift010 ar=null;
ArrayList groups=ArGroup.getGroupNo(admin.getCpnyId());
request.setAttribute("groups",groups);
ArDynamicGroup arg=null;
request.setAttribute("person_id",request.getParameter("shiftID"));
...
function Add()
{
	...
	{//"请选择组号"
		alert('<ait:message  messageID="alert.att.maintenance.rotation.select_group" module="ar"/>');
		return ;
	}
	...
	document.form1.action="/arControlServlet?operation=shiftset&adminID=<%=admin.getAdminID()%>&empID="+empID+"&from_date="+from+"&to_date="+to+"&d_count="+d_count+"&shift_no="+str+"&type=group";
...
function viewGroup(groupNo){
	...
	if(groupNo == "empty")	
	//"未选定组!\n\n请在左边组列表中选择组！"
	alert('<ait:message  messageID="alert.att.maintenance.rotation.group_left_column" module="ar"/>');
}
function excel(){
	var from=document.form1.from_date.value;
	var to=document.form1.to_date.value;
...
function checkGroup(){
	if(document.form1.person_id.value==''){
	//'请选择一个动态组'
		alert('<ait:message  messageID="alert.att.maintenance.rotation.select_pattern" module="ar"/>');
	}else{
		viewGroup(document.form1.person_id.value);
	}
}
</script>
</head>
...
<!--      <img src="../images/calendar/calender.gif" align="absMiddle" border="0" height="18" width="18" onclick="showCalendar('form1','from_date');" Style="cursor:hand"></td>-->
    ...
    <tr>
      <td class="info_title_01" nowrap="nowrap"><!-- 动态组-->
	  ...
	  <c:forEach items="${groups}" var="groups">
		  <c:choose>
		  	<c:when test="${groups.groupNo==person_id}">
		  		<option value="${groups.groupNo}" selected="selected">
		  			<ait:content enContent="${groups.groupEnName}" zhContent="${groups.groupName}" koContent="${groups.groupKorName}"/>
		  		</option>
		  	</c:when>
		  	<c:otherwise>
		  		<option value="${groups.groupNo}">
		  			<ait:content enContent="${groups.groupEnName}" zhContent="${groups.groupName}" koContent="${groups.groupKorName}"/>
		  		</option>
		  	</c:otherwise>
		  </c:choose>	  	
      ...
      <td><ait:image src="/images/btn/Member.gif"  border="0" align="absmiddle"
          	onclick="javascript:checkGroup();" style="cursor:hand"/>
      </td>
      <td><ait:image src="/images/btn/Save.gif"  border="0" align="absmiddle" onclick="javascript:Add();" style="cursor:hand" />
      </td>
    </tr>
	<tr>
